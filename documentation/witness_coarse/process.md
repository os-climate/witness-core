...
The next section described the notion of _Utility per capita_.

#### Utility per capita

The utility per capita relies on two variables available in witness, _Consumption per capita_ and _Energy price_. The next two sections gives a quick explanation of these variables.

In our optimization formulation, we want to maximize the quantity of things consumed. For that, we can see _Consumption per capita_ can be seen as
