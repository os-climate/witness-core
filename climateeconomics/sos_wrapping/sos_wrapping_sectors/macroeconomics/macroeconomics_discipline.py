...
class MacroeconomicsDiscipline(ClimateEcoDiscipline):
        ...
        if GlossaryCore.SectorListValue in self.get_data_in():
            ...
            if sector_list is not None:
                for sector in sector_list:
                    ...
                    dynamic_inputs[f"{sector}.{GlossaryCore.ProductionDfValue}"] = GlossaryCore.get_dynamic_variable(
                        GlossaryCore.ProductionDf
                    )
                    dynamic_inputs[f"{sector}.{GlossaryCore.InvestmentDfValue}"] = GlossaryCore.get_dynamic_variable(
    ...
    def run(self):
        ...
        outputs_dict = {
            ...
            GlossaryCore.DamageDfValue: self.macro_model.damage_df[
    ...
    def get_post_processing_list(self, chart_filters=None):
        ...
        if GlossaryCore.OutputNetOfDamage in chart_list:
            ...
            for sector in sector_list:
                ...
                new_series = InstanciatedSeries(years, sector_net_output, sector, "bar", True)
        ...
        if GlossaryCore.Capital in chart_list:
            ...
            legend = {GlossaryCore.Capital: "capital stock", GlossaryCore.UsableCapital: "usable capital stock"}
            years = list(economics_detail_df.index)

            chart_name = "Total capital stock and usable capital"
            new_chart = TwoAxesInstanciatedChart(
                GlossaryCore.Years, "capital stock [T$]", chart_name=chart_name, y_min_zero=True
            )

            for key in to_plot:
                ordonate_data = list(economics_detail_df[key])
                new_series = InstanciatedSeries(years, ordonate_data, legend[key], "lines", True)
                new_chart.add_series(new_series)
        ...
        if "share capital" in chart_list:
            ...
            new_chart = TwoAxesInstanciatedChart(
                GlossaryCore.Years, "share of total capital stock [%]", stacked_bar=True, chart_name=chart_name
            )
            ...
            for sector in sector_list:
                ...
                new_series = InstanciatedSeries(years, ordonate_data, sector, "bar", True)
                new_chart.add_series(new_series)
        ...
        if "share output" in chart_list:
            ...
            new_chart = TwoAxesInstanciatedChart(
                GlossaryCore.Years, "share of total net output [%]", stacked_bar=True, chart_name=chart_name
            )
        ...
        if "output growth" in chart_list:
            ...
            for key in to_plot:
                ordonate_data = list(economics_detail_df[key])
                new_series = InstanciatedSeries(years, ordonate_data, key, "lines", True)
                new_chart.add_series(new_series)
        ...
        if "Investments breakdown by sector" in chart_list:
            ...
            new_chart = TwoAxesInstanciatedChart(
                GlossaryCore.Years, "Sectors investments [G$]", chart_name=chart_name, stacked_bar=True
            )
            ...
            for sector in sector_list:
                ...
                new_series = InstanciatedSeries(years, ordonate_data, f"{sector} investments", "bar")
                new_chart.add_series(new_series)
            ...
            new_series = InstanciatedSeries(years, ordonate_data, f"energy investments", "bar")
            new_chart.add_series(new_series)
            ...
            new_series = InstanciatedSeries(years, ordonate_data, f"total investments", "lines")
            new_chart.add_series(new_series)
        ...
        if GlossaryCore.Damages in chart_list:
            ...
            new_series = InstanciatedSeries(years, list(applied_damages), "Total", "lines", True)
        ...
        if GlossaryCore.Damages in chart_list:
            ...
            for sector in sector_list:
                damage_detailed_df = self.get_sosdisc_inputs(f"{sector}.{GlossaryCore.DamageDetailedDfValue}")
                sector_damage = (
                    damage_detailed_df[GlossaryCore.EstimatedDamagesFromClimate].values
        ...
        if GlossaryCore.DamagesFromClimate in chart_list:
            ...
            new_series = InstanciatedSeries(years, list(damages_from_climate), "Total", "lines", True)
        ...
        if GlossaryCore.DamagesFromProductivityLoss in chart_list:
            ...
            new_series = InstanciatedSeries(years, list(damages_from_productivity_loss), "Total", "lines", True)
