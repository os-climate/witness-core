...
class SectorDiscipline(ClimateEcoDiscipline):
    ...
    DESC_IN = {
        ...
        GlossaryCore.EnergyProductionValue: {
            "type": "dataframe",
            "unit": GlossaryCore.EnergyProductionDf["unit"],
        ...
        "prod_function_fitting": {
            "type": "bool",
            "default": False,
            "visibility": "Shared",
            "unit": "-",
            "namespace": GlossaryCore.NS_MACRO,
            "structuring": True,
        },
    ...
    DESC_OUT = {
        ...
        GlossaryCore.EnergyWastedObjective: {"type": "array", "unit": "-", "namespace": GlossaryCore.NS_FUNCTIONS},
    }
        ...
        if GlossaryCore.YearStart in self.get_data_in() and GlossaryCore.YearEnd in self.get_data_in():
            ...
            if year_start is not None and year_end is not None:
                ...
                if f"{self.sector_name}.{GlossaryCore.SectionGdpPercentageDfValue}" in self.get_data_in():
                    variable_value = self.get_sosdisc_inputs(
                    ...
                    if variable_value is None:
                        ...
                        section_gdp_percentage_df_default = pd.read_csv(
                            join(
                                global_data_dir, f"weighted_average_percentage_{self.sector_name.lower()}_sections.csv"
                            )
                        )
                        section_gdp_percentage_dict = {
                            ...
                            **dict(
                                zip(
                                    section_gdp_percentage_df_default.columns[1:],
                        ...
                        self.set_dynamic_default_values(
                            {f"{self.sector_name}.{GlossaryCore.SectionGdpPercentageDfValue}": new_variable_value}
                        )
                if f"{self.sector_name}.{GlossaryCore.SectionEnergyConsumptionPercentageDfValue}" in self.get_data_in():
                    ...
                    if variable_value is None:
                        ...
                        section_energy_consumption_percentage_dict = {
                            ...
                            **dict(
                                zip(
                                    section_energy_consumption_percentage_df_default.columns[1:],
                        ...
                        self.set_dynamic_default_values(
                            {
                                f"{self.sector_name}.{GlossaryCore.SectionEnergyConsumptionPercentageDfValue}": new_variable_value
                            }
                        )
    ...
    def setup_sos_disciplines(self):
        ...
        if GlossaryCore.WorkforceDfValue in self.get_sosdisc_inputs():
            ...
            if workforce_df is not None and self.sector_name not in workforce_df.columns:
                raise Exception(
                    f"Data integrity : workforce_df does should have a column for sector {self.sector_name}"
                )
        if "prod_function_fitting" in self.get_sosdisc_inputs():
            ...
            if prod_function_fitting:
                ...
                dynamic_inputs["hist_sector_investment"] = {
                    "type": "dataframe",
                    "unit": "-",
                ...
                dynamic_outputs["range_energy_eff_constraint"] = {
                    "type": "array",
                    "unit": "-",
                    "dataframe_descriptor": {},
                    "dynamic_dataframe_columns": True,
        ...
        section_energy_consumption_percentage_var = GlossaryCore.get_dynamic_variable(
            GlossaryCore.SectionEnergyConsumptionPercentageDf
        )
        section_energy_consumption_percentage_var.update({"namespace": GlossaryCore.NS_SECTORS})
        dynamic_inputs[f"{self.sector_name}.{GlossaryCore.SectionEnergyConsumptionPercentageDfValue}"] = (
            section_energy_consumption_percentage_var
        )
        ...
        dynamic_outputs[f"{self.sector_name}.{GlossaryCore.DetailedCapitalDfValue}"] = (
            GlossaryCore.get_dynamic_variable(GlossaryCore.DetailedCapitalDf)
        )
        ...
        section_energy_consumption_df_variable["dataframe_descriptor"].update(
            {section: ("float", [0.0, 1e30], True) for section in GlossaryCore.SectionDictSectors[self.sector_name]}
        )
        dynamic_outputs[f"{self.sector_name}.{GlossaryCore.SectionEnergyConsumptionDfValue}"] = (
            section_energy_consumption_df_variable
        )
    ...
    def run(self):
        ...
        dict_values = {
            ...
            f"{self.sector_name}.{GlossaryCore.DamageDfValue}": self.model.damage_df[
                GlossaryCore.DamageDf["dataframe_descriptor"].keys()
            ],
            f"{self.sector_name}.{GlossaryCore.DamageDetailedDfValue}": self.model.damage_df[
                GlossaryCore.DamageDetailedDf["dataframe_descriptor"].keys()
            ],
            f"{self.sector_name}.{GlossaryCore.ProductionDfValue}": self.model.production_df[
                GlossaryCore.ProductionDf["dataframe_descriptor"].keys()
            ],
            f"{self.sector_name}.{GlossaryCore.CapitalDfValue}": self.model.capital_df[
                [
                    GlossaryCore.Years,
    ...
    def compute_sos_jacobian(self):
        ...
        d_estimated_damage_from_climate_d_workforce = self.model.d_estimated_damages_from_climate_d_user_input(
            d_gross_output_d_workforce, d_net_output_d_workforce
        )
        d_damage_from_productivity_loss_d_workforce, d_estimated_damage_from_productivity_loss_d_workforce = (
            self.model.d_damages_from_productivity_loss_d_user_input(d_gross_output_d_workforce)
        )
        ...
        d_estimated_damages_d_workforce = self.model.d_estimated_damages_d_user_input(
            d_estimated_damage_from_productivity_loss_d_workforce, d_estimated_damage_from_climate_d_workforce
        )
        ...
        d_gross_output_d_damage_frac_output = self.model.doutput_ddamage(dproductivity_ddamage)
        d_net_output_d_damage_frac_output = self.model.dnetoutput_ddamage(d_gross_output_d_damage_frac_output)
        ...
        d_estimated_damages_from_climate_d_damage_frac_output = (
            self.model.d_estimated_damages_from_climate_d_damage_frac_output(
                d_gross_output_d_damage_frac_output, d_net_output_d_damage_frac_output
            )
        )
        (
            d_damages_from_productivity_loss_d_damage_frac_output,
            d_estimated_damages_from_productivity_loss_d_damage_frac_output,
        ) = self.model.d_damages_from_productivity_loss_d_damage_fraction_output(d_gross_output_d_damage_frac_output)
        ...
        d_estimated_damages_d_damage_frac_output = self.model.d_damages_d_user_input(
            d_estimated_damages_from_climate_d_damage_frac_output,
            d_estimated_damages_from_productivity_loss_d_damage_frac_output,
        )
        ...
        d_damage_from_climate_d_invests = self.model.d_damages_from_climate_d_user_input(
            d_gross_output_d_invests, d_net_output_d_invests
        )
        d_estimated_damage_from_climate_d_invests = self.model.d_estimated_damages_from_climate_d_user_input(
            d_gross_output_d_invests, d_net_output_d_invests
        )
        d_damage_from_productivity_loss_d_invests, d_estimated_damage_from_productivity_loss_d_invests = (
            self.model.d_damages_from_productivity_loss_d_user_input(d_gross_output_d_invests)
        )
        ...
        d_estimated_damages_d_invests = self.model.d_estimated_damages_d_user_input(
            d_estimated_damage_from_productivity_loss_d_invests, d_estimated_damage_from_climate_d_invests
        )

        self.set_partial_derivative_for_other_types(
            (GlossaryCore.EnergyWastedObjective,), (invest_df, GlossaryCore.InvestmentsValue), d_EWO_d_invests
        )
        self.set_partial_derivative_for_other_types(
        ...
        d_estimated_damage_from_climate_d_energy_production = self.model.d_estimated_damages_from_climate_d_user_input(
            d_gross_output_d_energy_production, d_net_output_d_energy_production
        )
        (
            d_damage_from_productivity_loss_d_energy_production,
            d_estimated_damage_from_productivity_loss_d_energy_production,
        ) = self.model.d_damages_from_productivity_loss_d_user_input(d_gross_output_d_energy_production)
        ...
        d_estimated_damages_d_energy_production = self.model.d_estimated_damages_d_user_input(
            d_estimated_damage_from_productivity_loss_d_energy_production,
            d_estimated_damage_from_climate_d_energy_production,
        )
    ...
    def get_post_processing_list(self, chart_filters=None):
        ...
        if GlossaryCore.UsableCapital in chart_list:
            ...
            new_chart = TwoAxesInstanciatedChart(
                GlossaryCore.Years, "Capital stock [trillion dollars]", chart_name=chart_name, y_min_zero=True
            )
        ...
        if GlossaryCore.Damages in chart_list:
                ...
                to_plot.update(
                    {
                        ...
                        + "not " * (not damages_to_productivity)
            ...
            new_series = InstanciatedSeries(years, list(all_damages), "All damages", "lines", True)
        ...
        if GlossaryCore.Capital in chart_list:
            ...
            new_chart = TwoAxesInstanciatedChart(
                GlossaryCore.Years, "Capital stock [Trillion dollars]", chart_name=chart_name, stacked_bar=True
            )
        ...
        if GlossaryCore.Productivity in chart_list:
            ...
            new_chart = TwoAxesInstanciatedChart(
                GlossaryCore.Years, "Total Factor Productivity [no unit]", chart_name=chart_name, y_min_zero=True
            )
        ...
        if GlossaryCore.EnergyEfficiency in chart_list:
            ...
            new_chart = TwoAxesInstanciatedChart(
                GlossaryCore.Years, "Capital energy efficiency [-]", chart_name=chart_name, y_min_zero=True
            )
        ...
        if "output growth" in chart_list:
            ...
            new_chart = TwoAxesInstanciatedChart(GlossaryCore.Years, " growth rate [-]", chart_name=chart_name)
            for key in to_plot:
                visible_line = True
        ...
        if GlossaryCore.SectionGdpPart in chart_list:
            ...
            new_chart = TwoAxesInstanciatedChart(
                GlossaryCore.Years, GlossaryCore.SectionGdpPart, chart_name=chart_name, stacked_bar=True
            )
            ...
            instanciated_charts.append(
                InstantiatedPlotlyNativeChart(fig, chart_name=chart_name, default_title=True, default_legend=False)
            )

        if GlossaryCore.SectionEnergyConsumptionPart in chart_list:
            sections_energy_consumption = self.get_sosdisc_outputs(
                f"{self.sector_name}.{GlossaryCore.SectionEnergyConsumptionDfValue}"
            )
            ...
            new_chart = TwoAxesInstanciatedChart(GlossaryCore.Years, "PWh", chart_name=chart_name, stacked_bar=True)
