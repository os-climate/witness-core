...
class ObjectivesDiscipline(ClimateEcoDiscipline):
    ...
    DESC_IN = {
        ...
        "weights_df": {
            "type": "dataframe",
            "unit": "-",
            "default": default_weight_df,
            "dataframe_descriptor": {GlossaryCore.Years: ("float", None, False), "weight": ("float", None, True)},
            "dataframe_edition_locked": False,
        },
        "data_for_earlier_energy_eff": {
            "type": "dataframe",
            "unit": "-",
            "default": default_earlier_energy_eff,
            "dataframe_descriptor": {
                GlossaryCore.Years: ("float", None, False),
        ...
        if GlossaryCore.SectorListValue in self.get_data_in():
            ...
            for sector in sector_list:
                dynamic_inputs[f"{sector}.{GlossaryCore.DetailedCapitalDfValue}"] = {
                    "type": "dataframe",
                    "unit": MacroeconomicsModel.SECTORS_OUT_UNIT[sector],
                    "visibility": ClimateEcoDiscipline.SHARED_VISIBILITY,
                    "namespace": GlossaryCore.NS_SECTORS,
                    "dataframe_descriptor": {
                        GlossaryCore.Years: ("float", None, False),
                        GlossaryCore.Capital: ("float", None, False),
                        GlossaryCore.UsableCapital: ("float", None, False),
    ...
    def run(self):
        ...
        # -- compute
        error_pib_total, sectors_gdp_errors, energy_eff_errors, hist_energy_eff, year_min = (
            self.objectives_model.compute_all_errors(inputs_dict)
        )

        # store outputs in a dict
        outputs_dict = {"error_pib_total": np.array([error_pib_total]), "year_min_energy_eff": year_min}
    ...
    def get_post_processing_list(self, chart_filters=None):
        ...
        if "Total output" in chart_list:
            ...
            new_series = InstanciatedSeries(years, list(simu), "simulated values", "lines", visible_line)
            new_chart.series.append(new_series)
        ...
        if "sectors" in chart_list:
            ...
            for sector in sector_list:
                ...
                new_chart.series.append(new_series)
                new_series = InstanciatedSeries(years, list(simu_gdp_sector), "simulated values", "lines", visible_line)
                new_chart.series.append(new_series)
                instanciated_charts.append(new_chart)
                ...
                years_energ_eff = list(
                    hist_energy_eff_sector[GlossaryCore.Years][
                        hist_energy_eff_sector[GlossaryCore.Years] >= year_min_energy_eff[sector]
                    ]
                )
                hist_energy_eff_s = list(
                    hist_energy_eff_sector[GlossaryCore.EnergyEfficiency][
                        hist_energy_eff_sector[GlossaryCore.Years] >= year_min_energy_eff[sector]
                    ]
                )
                new_series = InstanciatedSeries(
                ...
                if len(years_energ_eff) > len(years):
                    new_series = InstanciatedSeries(
                        list(simu_energy_eff_sector_lt[GlossaryCore.Years]),
                        list(simu_energy_eff_sector_lt[GlossaryCore.EnergyEfficiency]),
                        "simulated values",
                        "lines",
                        visible_line,
                    )
