...
class DemandDiscipline(SoSWrapp):
    ...
    def setup_sos_disciplines(self):
        ...
        if GlossaryCore.SectorListValue in self.get_data_in():
            ...
            for sector in sector_list:
                dynamic_inputs[f'{sector}.{GlossaryCore.SectorDemandPerCapitaDfValue}'] = GlossaryCore.get_dynamic_variable(GlossaryCore.SectorDemandPerCapitaDf)
                dynamic_outputs[f'{sector}.{GlossaryCore.SectorGDPDemandDfValue}'] = GlossaryCore.get_dynamic_variable(GlossaryCore.SectorGDPDemandDf)
    ...
    def get_chart_filter_list(self):
        ...
        chart_list = [GlossaryCore.SectorGDPDemandDf,
    ...
    def get_post_processing_list(self, filters=None):
        ...
        if all_filters or GlossaryCore.SectorGDPDemandDf:
            ...
            for sector in sector_list:
                demand_sector_per_capita = all_sectors_demand_df[sector].values
                share_sector = demand_sector_per_capita / total_demand * 100.
                new_series = InstanciatedSeries(years,
                                                list(share_sector),
                                                sector, 'bar', True)
                new_chart.series.append(new_series)
        ...
        if all_filters or GlossaryCore.SectorGDPDemandDf:
            ...
            for sector in sector_list:
                demand_sector_per_capita_df = self.get_sosdisc_inputs(f'{sector}.{GlossaryCore.SectorDemandPerCapitaDfValue}')
