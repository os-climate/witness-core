...
class GHGemissionsDiscipline(ClimateEcoDiscipline):
    ...
    DESC_IN = {
        "cheat_var_to_update_ns_dashboard_in_ms_mdo": {
            "type": "float",
            "namespace": GlossaryCore.NS_GHGEMISSIONS,
            "visibility": "Shared",
            "default": 0.0,
            "unit": "-",
            "user_level": 3,
        ...
        if GlossaryCore.SectorListValue in self.get_data_in():
            ...
            if sectorlist is not None:
                for sector in sectorlist:
                    ...
                    dynamic_inputs[f"{sector}.{GlossaryCore.SectionEnergyConsumptionDfValue}"] = (
                        section_energy_consumption_df_variable
                    )

                    section_non_energy_emissions_gdp_var = GlossaryCore.get_dynamic_variable(
                    ...
                    dynamic_inputs[f"{sector}.{GlossaryCore.SectionNonEnergyEmissionGdpDfValue}"] = (
                        section_non_energy_emissions_gdp_var
                    )

                    section_gdp_var = GlossaryCore.get_dynamic_variable(GlossaryCore.SectionGdpDf)
                    section_gdp_var["dataframe_descriptor"].update(
                        {section: ("float", [0.0, 1e30], True) for section in GlossaryCore.SectionDictSectors[sector]}
                    )
                    dynamic_inputs[f"{sector}.{GlossaryCore.SectionGdpDfValue}"] = section_gdp_var

                    section_energy_emissions_df_variable = GlossaryCore.get_dynamic_variable(
                        GlossaryCore.SectionEnergyEmissionDf
                    )
                    section_energy_emissions_df_variable["dataframe_descriptor"].update(
                        {section: ("float", [0.0, 1e30], True) for section in GlossaryCore.SectionDictSectors[sector]}
                    )
                    ...
                    dynamic_outputs[f"{sector}.{GlossaryCore.SectionEnergyEmissionDfValue}"] = (
                        section_energy_emissions_df_variable
                    )

                    section_non_energy_emissions_df_variable = GlossaryCore.get_dynamic_variable(
                        GlossaryCore.SectionNonEnergyEmissionDf
                    )
                    section_non_energy_emissions_df_variable["dataframe_descriptor"].update(
                        {section: ("float", [0.0, 1e30], True) for section in GlossaryCore.SectionDictSectors[sector]}
                    )
                    ...
                    dynamic_outputs[f"{sector}.{GlossaryCore.SectionNonEnergyEmissionDfValue}"] = (
                        section_non_energy_emissions_df_variable
                    )
                    ...
                    section_emissions_df_variable["dataframe_descriptor"].update(
                        {section: ("float", [0.0, 1e30], True) for section in GlossaryCore.SectionDictSectors[sector]}
                    )
    ...
    def run(self):
        ...
        dict_values = {
            ...
            GlossaryCore.CO2EmissionsGtValue: self.emissions_model.GHG_total_energy_emissions[
                [GlossaryCore.Years, GlossaryCore.TotalCO2Emissions]
            ],
            GlossaryCore.GHGEmissionsDfValue: self.emissions_model.ghg_emissions_df[
                GlossaryCore.GHGEmissionsDf["dataframe_descriptor"].keys()
            ],
            ...
            GlossaryCore.EconomicsEmissionDfValue: self.emissions_model.total_economics_emisssions[
                GlossaryCore.EmissionDf["dataframe_descriptor"].keys()
            ],
        ...
        for sector in self.emissions_model.new_sector_list:
            dict_values.update(
                {
                    f"{sector}.{GlossaryCore.SectionEnergyEmissionDfValue}": self.emissions_model.dict_sector_sections_energy_emissions[
                        sector
                    ]
                }
            )
            dict_values.update(
                {
                    f"{sector}.{GlossaryCore.SectionNonEnergyEmissionDfValue}": self.emissions_model.dict_sector_sections_non_energy_emissions[
                        sector
                    ]
                }
            )
            dict_values.update(
                {
                    f"{sector}.{GlossaryCore.SectionEmissionDfValue}": self.emissions_model.dict_sector_sections_emissions[
                        sector
                    ]
                }
            )
            dict_values.update(
                {
                    f"{sector}.{GlossaryCore.EmissionsDfValue}": self.emissions_model.dict_sector_emissions[sector][
                        GlossaryCore.EmissionDf["dataframe_descriptor"].keys()
                    ]
                }
            )
    ...
    def compute_sos_jacobian(self):
        ...
        inputs_dict = self.get_sosdisc_inputs()
        years = np.arange(
            inputs_dict[GlossaryCore.YearStart],
            inputs_dict[GlossaryCore.YearEnd] + 1,
            inputs_dict[GlossaryCore.TimeStep],
        )
        ...
        self.set_partial_derivative_for_other_types(
            ...
            ("GHG_total_energy_emissions", GlossaryCore.TotalCO2Emissions),
            np.identity(len(years)),
        )
        self.set_partial_derivative_for_other_types(
            (GlossaryCore.CO2EmissionsObjectiveValue,),
            ("GHG_total_energy_emissions", GlossaryCore.TotalCO2Emissions),
        ...
        d_section_energy_emissions_d_section_energy_consumption = (
            self.emissions_model.d_section_energy_emissions_d_section_energy_consumption()
        )
        for sector in self.emissions_model.economic_sectors_except_agriculture:
            ...
            for section in GlossaryCore.SectionDictSectors[sector]:
                ...
                d_section_energy_emissions_d_energy_prod = self.emissions_model.d_section_energy_emissions_d_user_input(
                    section_name=section,
                    sector_name=sector,
                    d_carbon_intensity_d_user_input=d_carbon_intensity_d_energy_prod,
                )
                ...
                d_sector_section_non_energy_emissions_d_section_gdp = (
                    self.emissions_model.d_section_non_energy_emissions_d_gdp_section(sector=sector, section=section)
                )
                ...
                for ghg in GlossaryCore.GreenHouseGases:
                    d_sections_energy_emissions_d_ghg_emisssions[ghg].append(
                        self.emissions_model.d_section_energy_emissions_d_user_input(
                            section_name=section,
                            sector_name=sector,
                            d_carbon_intensity_d_user_input=d_energy_carbon_intensity_d_ghg_total_emissions[ghg],
                        )
                    )
                    self.set_partial_derivative_for_other_types(
            ...
            d_sector_energy_emissions_d_energy_prod[sector] = np.sum(
                d_energy_emissions_sections_d_energy_prod_list, axis=0
            )

            for ghg in GlossaryCore.GreenHouseGases:
                d_sector_energy_emissions_d_ghg_emissions[sector][ghg] = np.sum(
                    d_sections_energy_emissions_d_ghg_emisssions[ghg], axis=0
                )
    ...
    def get_chart_filter_list(self):
        ...
        chart_list = [
            "Global Warming Potential (20-year basis) emissions per GHG",
        ...
        selected_values = [
            "Global Warming Potential (20-year basis) emissions per GHG",
            "Global Warming Potential (100-year)",
            "Global Warming Potential (100-year basis) emissions per source" "Total CO2 emissions",
        ]

        chart_filters.append(ChartFilter("Charts", chart_list, selected_values, "charts"))
    ...
    def get_post_processing_list(self, chart_filters=None):
        ...
        if GlossaryCore.EnergyCarbonIntensityDfValue in charts:
            ...
            new_chart = TwoAxesInstanciatedChart(
                GlossaryCore.Years, GlossaryCore.EnergyCarbonIntensityDf["unit"], chart_name=chart_name
            )

            new_serie = InstanciatedSeries(
                list(carbon_intensity_df[GlossaryCore.Years].values),
    ...
    def get_chart_gwp(self, gwp_year):
        ...
        for ghg in GHGEmissions.GHG_TYPE_LIST:
            new_serie = InstanciatedSeries(
                list(GWP_emissions[GlossaryCore.Years].values),
    ...
    def get_chart_emission_per_source(self, ghg):
        GHG_emissions_detail_df = self.get_sosdisc_outputs(GlossaryCore.GHGEmissionsDetailedDfValue)

        chart_name = f"{ghg} emissions per source"
        new_chart = TwoAxesInstanciatedChart(
            GlossaryCore.Years,
            f"{ghg} Emissions [{GlossaryCore.GHGEmissionsDf['unit']}]",
            chart_name=chart_name,
            stacked_bar=True,
        )

        emission_type = [
            GlossaryCore.insertGHGAgriLandEmissions,
        ...
        for e_t in emission_type:
            new_serie = InstanciatedSeries(
                ...
                list(GHG_emissions_detail_df[e_t.format(ghg)].values),
                e_t.format(ghg),
                "bar",
            )
