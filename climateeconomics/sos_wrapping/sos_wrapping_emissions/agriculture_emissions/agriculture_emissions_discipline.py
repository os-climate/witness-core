...
class AgricultureEmissionsDiscipline(ClimateEcoDiscipline):
    ...
    DESC_IN = {
        ...
        "co2_eq_20_objective_ref": {"type": "float", "default": 1000.0},
        "co2_eq_100_objective_ref": {"type": "float", "default": 500.0},
    ...
    def run(self):
        ...
        other_land_CO2_emissions = self.get_sosdisc_inputs("other_land_CO2_emissions")
        ...
        co2_eq_20 = (
            CO2_emissions_land_use_df["Forest"].sum()
            + CO2_emissions_land_use_df["Crop"].sum()
            + CH4_emissions_land_use_df["Crop"].sum() * self.GWP_20_default[GlossaryCore.CH4]
            + N2O_emissions_land_use_df["Crop"].sum() * self.GWP_20_default[GlossaryCore.N2O]
        ) / (co2_eq_20_ref * l_years)

        co2_eq_100 = (
            CO2_emissions_land_use_df["Forest"].sum()
            + CO2_emissions_land_use_df["Crop"].sum()
            + CH4_emissions_land_use_df["Crop"].sum() * self.GWP_100_default[GlossaryCore.CH4]
            + N2O_emissions_land_use_df["Crop"].sum() * self.GWP_100_default[GlossaryCore.N2O]
        ) / (co2_eq_100_ref * l_years)
        ...
        for techno in techno_list:
            ...
            if techno == "Crop":
                self.set_partial_derivative_for_other_types(
                    ...
                    (f"{techno}.CH4_land_emission_df", "emitted_CH4_evol_cumulative"),
                    np.identity(np_years),
                )
                self.set_partial_derivative_for_other_types(
                    (GlossaryCore.insertGHGAgriLandEmissions.format(GlossaryCore.N2O), f"{techno}"),
                    (f"{techno}.N2O_land_emission_df", "emitted_N2O_evol_cumulative"),
                    np.identity(np_years),
                )
        self.set_partial_derivative_for_other_types(
            ("co2_eq_20",),
    ...
    def get_chart_ghg_emissions(self):
        ...
        CO2_emissions_land_use = CO2_emissions_land_use_df.drop(GlossaryCore.Years, axis=1).sum(axis=1).values.tolist()
        CH4_emissions_land_use = CH4_emissions_land_use_df.drop(GlossaryCore.Years, axis=1).sum(axis=1).values.tolist()
        N2O_emissions_land_use = N2O_emissions_land_use_df.drop(GlossaryCore.Years, axis=1).sum(axis=1).values.tolist()

        serie = InstanciatedSeries(year_list, CO2_emissions_land_use, "CO2 Emissions [GtCO2]", "bar")
        new_chart.series.append(serie)

        serie = InstanciatedSeries(year_list, CH4_emissions_land_use, "CH4 Emissions [GtCH4]", "bar")
        new_chart.series.append(serie)

        serie = InstanciatedSeries(year_list, N2O_emissions_land_use, "N2O Emissions [GtN2O]", "bar")
        new_chart.series.append(serie)
