...
class AgricultureEmissionsDiscipline(ClimateEcoDiscipline):
    ...
    DESC_OUT = {
        GlossaryCore.insertGHGLandEmissions.format(GlossaryCore.CO2): {'type': 'dataframe', 'unit': 'GtCO2',
                                       'visibility': ClimateEcoDiscipline.SHARED_VISIBILITY, 'namespace': GlossaryCore.NS_WITNESS},
        GlossaryCore.insertGHGLandEmissions.format(GlossaryCore.CH4): {'type': 'dataframe', 'unit': 'GtCH4',
                                       'visibility': ClimateEcoDiscipline.SHARED_VISIBILITY, 'namespace': GlossaryCore.NS_WITNESS},
        GlossaryCore.insertGHGLandEmissions.format(GlossaryCore.N2O): {'type': 'dataframe', 'unit': 'GtN2O',
                               'visibility': ClimateEcoDiscipline.SHARED_VISIBILITY, 'namespace': GlossaryCore.NS_WITNESS},
    ...
    def run(self):
        ...
        # write outputs
        outputs_dict = {
            GlossaryCore.insertGHGLandEmissions.format(GlossaryCore.CO2): CO2_emissions_land_use_df,
            GlossaryCore.insertGHGLandEmissions.format(GlossaryCore.CH4): CH4_emissions_land_use_df,
            GlossaryCore.insertGHGLandEmissions.format(GlossaryCore.N2O): N2O_emissions_land_use_df,
    ...
    def compute_sos_jacobian(self):
        ...
        for techno in techno_list:
            self.set_partial_derivative_for_other_types(
                (GlossaryCore.insertGHGLandEmissions.format(GlossaryCore.CO2), f'{techno}'), (
                    f'{techno}.CO2_land_emission_df', 'emitted_CO2_evol_cumulative'),
                np.identity(np_years))
            if techno == 'Crop':
                self.set_partial_derivative_for_other_types(
                    (GlossaryCore.insertGHGLandEmissions.format(GlossaryCore.CH4), f'{techno}'),
                    (f'{techno}.CH4_land_emission_df', 'emitted_CH4_evol_cumulative'), np.identity(np_years))
                self.set_partial_derivative_for_other_types(
                    (GlossaryCore.insertGHGLandEmissions.format(GlossaryCore.N2O), f'{techno}'),
    ...
    def get_chart_ghg_emissions(self):
        ...
        CO2_emissions_land_use_df = self.get_sosdisc_outputs(
            GlossaryCore.insertGHGLandEmissions.format(GlossaryCore.CO2))
        CH4_emissions_land_use_df = self.get_sosdisc_outputs(
            GlossaryCore.insertGHGLandEmissions.format(GlossaryCore.CH4))
        N2O_emissions_land_use_df = self.get_sosdisc_outputs(
            GlossaryCore.insertGHGLandEmissions.format(GlossaryCore.N2O))
