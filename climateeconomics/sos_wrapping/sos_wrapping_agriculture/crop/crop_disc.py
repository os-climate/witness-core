...
class CropDiscipline(ClimateEcoDiscipline):
    ...
    default_n2o_emissions = {
        ...
        GlossaryCore.Fish: 0.0,  # no crop or livestock related
    ...
    for food in default_ghg_emissions:
        default_co2_emissions[food] = max(
            0.0,
            (
                default_ghg_emissions[food]
                - default_ch4_emissions[food] * ch4_gwp_100
                - default_n2o_emissions[food] * n2o_gwp_100
            )
            / co2_gwp_100,
        )

    year_range = default_year_end - default_year_start + 1
    total_kcal = 1067961.0  # kcal/person/year
    ...
    milk_eggs_average_ca_daily_intake = (
        default_kg_to_kcal["eggs"] * diet_df_default["eggs"].values[0] / 365
        + default_kg_to_kcal["milk"] * diet_df_default["milk"].values[0] / 365
    )
    white_meat_average_ca_daily_intake = (
        default_kg_to_kcal["white meat"] * diet_df_default["white meat"].values[0] / 365
    )
    ...
    vegetables_and_carbs_average_ca_daily_intake = (
        diet_df_default["fruits and vegetables"].values[0] / 365 * default_kg_to_kcal["fruits and vegetables"]
    ...
    fish_average_ca_daily_intake = (
        default_kg_to_kcal[GlossaryCore.Fish] * diet_df_default[GlossaryCore.Fish].values[0] / 365
    )
    other_average_ca_daily_intake = (
        default_kg_to_kcal[GlossaryCore.OtherFood] * diet_df_default[GlossaryCore.OtherFood].values[0] / 365
    )
    ...
    default_fish_ca_per_day = pd.DataFrame(
        {GlossaryCore.Years: default_years, GlossaryCore.FishDailyCal: [fish_average_ca_daily_intake] * year_range}
    )
    default_other_ca_per_day = pd.DataFrame(
        {GlossaryCore.Years: default_years, GlossaryCore.OtherDailyCal: [other_average_ca_daily_intake] * year_range}
    )
    ...
    techno_infos_dict_default = {
        ...
        "residue_percentage_for_energy": 0.0,  # TODO delete all use of residues in crop : residue will be computed in organic waste
        "efficiency": 1.0,
        "techno_evo_eff": "no",
    ...
    initial_age_distribution = pd.DataFrame(
        {
            "age": np.arange(1, lifetime),
            "distrib": [
                0.16,
                0.24,
                0.31,
    ...
    DESC_IN = {
        ...
        "diet_df": {
            "type": "dataframe",
            "unit": "kg_food/person/year",
            "default": diet_df_default,
        ...
        "red_meat_calories_per_day": {
            "type": "dataframe",
            "default": default_red_meat_ca_per_day,
        ...
        "white_meat_calories_per_day": {
            "type": "dataframe",
            "default": default_white_meat_ca_per_day,
            "dataframe_descriptor": {
                GlossaryCore.Years: ("float", None, False),
        ...
        "vegetables_and_carbs_calories_per_day": {
            ...
            "dataframe_descriptor": {
                GlossaryCore.Years: ("float", None, False),
                "vegetables_and_carbs_calories_per_day": ("float", None, True),
            },
            "unit": "kcal",
            "visibility": ClimateEcoDiscipline.SHARED_VISIBILITY,
            "namespace": "ns_crop",
            "description": "number of calories coming from vegetables and fruits " "consumed per day per person",
        },
        "milk_and_eggs_calories_per_day": {
            "type": "dataframe",
            "default": default_milk_and_eggs_calories_per_day,
            "dataframe_descriptor": {
                GlossaryCore.Years: ("float", None, False),
                "milk_and_eggs_calories_per_day": ("float", None, True),
            },
            "unit": "kcal",
            "visibility": ClimateEcoDiscipline.SHARED_VISIBILITY,
            "namespace": "ns_crop",
        ...
        GlossaryCore.FishDailyCal: {
            "type": "dataframe",
            "default": default_fish_ca_per_day,
            "dataframe_descriptor": {
                GlossaryCore.Years: ("float", None, False),
        ...
        GlossaryCore.OtherDailyCal: {
            "type": "dataframe",
            "default": default_other_ca_per_day,
            "dataframe_descriptor": {
                GlossaryCore.Years: ("float", None, False),
        ...
        "margin": {
            ...
            "dataframe_descriptor": {GlossaryCore.Years: ("float", None, True), "margin": ("float", None, True)},
        },
        "transport_cost": {
            "type": "dataframe",
            "unit": "$/t",
        ...
        "techno_infos_dict": {"type": "dict", "unit": "defined in dict", "default": techno_infos_dict_default},
        "initial_production": {"type": "float", "unit": "TWh", "default": initial_production},
        "initial_age_distrib": {
            "type": "dataframe",
            "unit": "%",
            "default": initial_age_distribution,
            "dataframe_descriptor": {
                GlossaryCore.Years: ("float", None, True),
                "age": ("float", None, True),
    ...
    DESC_OUT = {
        ...
        "food_land_surface_df": {"type": "dataframe", "unit": "Gha"},
    ...
    def compute_sos_jacobian(self):
        ...
        food_waste_percentage = inputs_dict[GlossaryCore.FoodWastePercentageValue][
            GlossaryCore.FoodWastePercentageValue
        ].values
        ...
        for food_type in food_types:
            self.set_partial_derivative_for_other_types(
                ("calories_per_day_constraint",),
                (food_type, food_type),
                np.identity(l_years) * (1 - food_waste_percentage / 100.0) / self.crop_model.constraint_calories_ref,
            )

            self.set_partial_derivative_for_other_types(
                (GlossaryCore.CaloriesPerCapitaValue, "kcal_pc"),
                (food_type, food_type),
                np.identity(l_years) * (1 - food_waste_percentage / 100.0),
            )
        ...
        for ghg in [GlossaryCore.CO2, GlossaryCore.CH4, GlossaryCore.N2O]:
            ...
            for food in self.get_sosdisc_inputs("co2_emissions_per_kg"):
                ...
                if food == GlossaryCore.Fish:
                    ...
                    d_food_surface_d_red_meat_percentage = model.compute_d_food_surface_d_red_meat_percentage(
                        population_df, food
                    )
                    d_food_surface_d_white_meat_percentage = model.compute_d_food_surface_d_white_meat_percentage(
                        population_df, food
                    )
                    d_food_surface_d_fish_percentage = model.compute_d_food_surface_d_fish_percentage(
                        population_df, food
                    )
                    d_food_surface_d_other_food_percentage = model.compute_d_food_surface_d_other_food_percentage(
                        population_df, food
                    )

                    dco2_dpop += dland_emissions_dfood_land_surface[ghg] * dland_dpop
                    dco2_dfish += dland_emissions_dfood_land_surface[ghg] * d_food_surface_d_fish_percentage
                    ...
                    dco2_dother_cal += dland_emissions_dfood_land_surface[ghg] * d_food_surface_d_other_food_percentage

                    if food == "rice and maize":
                        dco2_dinvest += (
                            dland_emissions_dfood_land_surface[ghg]
                            * dprod_dinvest
                            * scaling_factor_crop_investment
                            * (1 - residue_density_percentage)
                            / density_per_ha
            ...
            for food in vegetables_column_names:
                dland_emissions_dfood_land_surface_oth = model.compute_dland_emissions_dfood_land_surface_df(food)[ghg]
            ...
            for food in ["eggs", "milk"]:
                dland_emissions_dfood_land_surface_oth = model.compute_dland_emissions_dfood_land_surface_df(food)[ghg]
                dco2_deggs_milk += (
                    dland_emissions_dfood_land_surface_oth
                    * model.d_surface_d_milk_eggs_calories_percentage(population_df, food)
                )
    ...
    def get_chart_filter_list(self):
        ...
        chart_filters = []
        chart_list = [CropDiscipline.CROP_CHARTS, "Crop Productivity Evolution", "Crop Energy", "Crop Emissions"]
    ...
    def get_post_processing_list(self, chart_filters=None):
        ...
        if CropDiscipline.CROP_CHARTS in chart_list:
            ...
            new_chart = TwoAxesInstanciatedChart(
                GlossaryCore.Years,
                "surface [Gha]",
                chart_name="Surface taken to produce food over time",
                stacked_bar=True,
            )
            ...
            surface_percentage_df = self.get_sosdisc_outputs("food_land_surface_percentage_df")
            ...
            for key in updated_diet_df.keys():
                ...
                if key == GlossaryCore.Years:
                    ...
                    new_series = InstanciatedSeries(years, l_values.tolist(), key, InstanciatedSeries.BAR_DISPLAY)

                    series_to_add.append(new_series)

            new_chart = TwoAxesInstanciatedChart(
                GlossaryCore.Years,
                "food calories [kcal / person / day]",
            ...
            new_series = InstanciatedSeries(years, total_consumed_pc_per_day, "Total", InstanciatedSeries.LINES_DISPLAY)
            ...
            food_waste_percentage = (
                self.get_sosdisc_inputs(GlossaryCore.FoodWastePercentageValue)[GlossaryCore.FoodWastePercentageValue]
                / 100.0
            )
            ...
            new_chart.add_series(new_series)
            new_series = InstanciatedSeries(years, wasted_food.tolist(), "Wasted food", InstanciatedSeries.BAR_DISPLAY)
            new_chart.add_series(new_series)
            new_series = InstanciatedSeries(
                years,
                [minimum_kcal_daily_apport] * len(years),
                "Minimum daily kcalories",
        ...
        kg_to_m2_dict, co2_emissions_per_kg, ch4_emissions_per_kg, n2o_emissions_per_kg = self.get_sosdisc_inputs(
            ["kg_to_m2_dict", "co2_emissions_per_kg", "ch4_emissions_per_kg", "n2o_emissions_per_kg"]
        )
        ...
        for key in starting_diet:
            if key == "fruits and vegetables" or key == "cereals" or key == "rice and maize":
                ...
                proportion = starting_diet[key].values[0] / (
                    starting_diet["fruits and vegetables"].values[0]
                    + starting_diet["cereals"].values[0]
                ...
                co2_emissions_per_kcal_dict_mean["vegetables_and_carbs"] += (
                    proportion * co2_emissions_per_kg[key] / kg_to_kcal_dict[key]
                )
                ch4_emissions_per_kcal_dict_mean["vegetables_and_carbs"] += (
                    ch4_gwp_100 * proportion * ch4_emissions_per_kg[key] / kg_to_kcal_dict[key]
                )
                n2o_emissions_per_kcal_dict_mean["vegetables_and_carbs"] += (
                    n2o_gwp_100 * proportion * n2o_emissions_per_kg[key] / kg_to_kcal_dict[key]
                )
                ...
                co2_emissions_per_kcal_dict_mean["eggs_milk"] += (
                    proportion * co2_emissions_per_kg[key] / kg_to_kcal_dict[key]
                )
                ch4_emissions_per_kcal_dict_mean["vegetables_and_carbs"] += (
                    ch4_gwp_100 * proportion * ch4_emissions_per_kg[key] / kg_to_kcal_dict[key]
        ...
        for food in ["red meat", "white meat", GlossaryCore.Fish, GlossaryCore.OtherFood]:
            ...
            ch4_emissions_per_kcal_dict_mean[food] = ch4_gwp_100 * ch4_emissions_per_kg[food] / kg_to_kcal_dict[food]
            n2o_emissions_per_kcal_dict_mean[food] = n2o_gwp_100 * n2o_emissions_per_kg[food] / kg_to_kcal_dict[food]

        for category in list_categories:
            ghg_emissions_per_kcal[category] = (
                co2_emissions_per_kcal_dict_mean[category]
        ...
        new_chart = TwoAxesInstanciatedChart(
            GlossaryCore.Years, "m2 per category per kcal [m2/kcal]", chart_name=chart_name
        )

        for key in list_categories:
            new_series = InstanciatedSeries([key], [m2_per_kcal_dict[key]], f"m2 per kcal of category {key}", "bar")
        ...
        new_chart = TwoAxesInstanciatedChart(
            GlossaryCore.Years, "ghg per category per kcal [co2eq/kcal]", chart_name=chart_name
        )
        ...
        if "Crop Productivity Evolution" in chart_list:
            prod_df = self.get_sosdisc_outputs("crop_productivity_evolution")
            years = list(prod_df[GlossaryCore.Years])

            chart_name = "Crop productivity evolution"

            new_chart = TwoAxesInstanciatedChart(
                GlossaryCore.Years, " productivity evolution [%]", chart_name=chart_name
            )

            visible_line = True
            ordonate_data = list(prod_df["productivity_evolution"] * 100)

            new_series = InstanciatedSeries(years, ordonate_data, "productivity_evolution", "lines", visible_line)
        ...
        if "Crop Energy" in chart_list:
            ...
            mix_detailed_production = deepcopy(self.get_sosdisc_outputs("mix_detailed_production"))
            land_use_required = deepcopy(self.get_sosdisc_outputs("land_use_required"))
            ...
            crop_investment = deepcopy(
                self.get_sosdisc_inputs("crop_investment") * self.get_sosdisc_inputs("scaling_factor_crop_investment")
            )
            years = list(prod_df[GlossaryCore.Years])
            ...
            new_chart = TwoAxesInstanciatedChart(GlossaryCore.Years, "Investments [M$]", chart_name=chart_name)
            ...
            for investment in crop_investment:
                if investment != GlossaryCore.Years:
                    ...
                    new_series = InstanciatedSeries(years, ordonate_data, investment, "bar", visible_line)
                    new_chart.series.append(new_series)
            ...
            new_chart = TwoAxesInstanciatedChart(
                GlossaryCore.Years, "Crop mass for energy production [Mt]", chart_name=chart_name
            )
            ...
            new_chart = TwoAxesInstanciatedChart(
                GlossaryCore.Years, "Crop for Energy production [TWh]", chart_name=chart_name
            )
            ...
            land_demand_chart = TwoAxesInstanciatedChart(GlossaryCore.Years, "Land demand [Gha]", chart_name=chart_name)
            ordonate_data = list(land_use_required["Crop (Gha)"])
            land_demand_serie = InstanciatedSeries(years, ordonate_data, "Crop", "lines", visible_line)
            ...
            new_chart = TwoAxesInstanciatedChart(GlossaryCore.Years, "Crop prices [$/MWh]", chart_name=chart_name)
            ...
            new_chart = TwoAxesInstanciatedChart(GlossaryCore.Years, "Crop prices [$/t]", chart_name=chart_name)
        ...
        if "Crop Emissions" in chart_list:
            ...
            for key in co2_emissions_detailed_df.columns:
                ...
                if key != GlossaryCore.Years:
                    new_series = InstanciatedSeries(
                        years,
                        (co2_emissions_detailed_df[key]).values.tolist(),
                        key.replace(" (Gt)", ""),
            ...
            for key in ch4_emissions_detailed_df.columns:
                ...
                if key != GlossaryCore.Years:
                    new_series = InstanciatedSeries(
                        years,
                        (ch4_emissions_detailed_df[key]).values.tolist(),
                        key.replace(" (Gt)", ""),
                        InstanciatedSeries.BAR_DISPLAY,
                    )
            ...
            for key in n2o_emissions_detailed_df.columns:
                ...
                if key != GlossaryCore.Years:
                    new_series = InstanciatedSeries(
                        years,
                        (n2o_emissions_detailed_df[key]).values.tolist(),
                        key.replace(" (Gt)", ""),
                        InstanciatedSeries.BAR_DISPLAY,
                    )
            ...
            for year in years_list:
                values = [
                    co2_emissions_detailed_df.loc[co2_emissions_detailed_df[GlossaryCore.Years] == year][food].values[0]
            ...
            for year in years_list:
                values = [
                    ch4_emissions_detailed_df.loc[ch4_emissions_detailed_df[GlossaryCore.Years] == year][food].values[0]
                    for food in food_names
                ]
            ...
            for year in years_list:
                values = [
                    n2o_emissions_detailed_df.loc[n2o_emissions_detailed_df[GlossaryCore.Years] == year][food].values[0]
                    for food in food_names
                ]
