...
def post_processings(execution_engine, namespace, filters):
    ...
    year_start_dict, year_end_dict = get_variables_values_per_scenario(execution_engine, df_paths, selected_scenarios)
    year_start, year_end = year_start_dict[selected_scenarios[0]], year_end_dict[selected_scenarios[0]]
    """
        -------------
        -------------
    ...
    if "CO2 Emissions vs Utility min" in selected_chart_list:
        ...
        (co2_emissions_df_dict, utility_df_dict) = get_variables_values_per_scenario(execution_engine, df_paths)
    ...
    if "PPM vs Utility" in selected_chart_list:
        ...
        (carboncycle_detail_df_dict, utility_df_dict) = get_variables_values_per_scenario(execution_engine, df_paths)
    ...
    if "Consumption vs Mean Energy Price" in selected_chart_list:
        ...
        df_paths = [
            f"{GlossaryCore.EconomicsDetailDfValue}",
            f"{GlossaryCore.EnergyMeanPriceValue}",
        ]
        (economics_df_dict, energy_detail_df_dict) = get_variables_values_per_scenario(execution_engine, df_paths)
