...
def post_processing_filters(execution_engine, namespace):
    ...
    chart_list = [
        "Temperature vs Welfare",
        "GHG Emissions vs Welfare",
        "GHG Emissions vs min(Utility)",
    ...
    filters.append(ChartFilter("Scenarios", scenario_list, scenario_list, "Scenarios"))
...
def post_processings(execution_engine, namespace, filters):
    ...
    if filters is not None:
        ...
        graphs_list = [
            "Temperature vs Welfare",
            "GHG Emissions vs Welfare",
            "GHG Emissions vs min(Utility)" "CO2 tax per scenario",
            "Temperature per scenario",
            "Welfare per scenario",
            "Utility per scenario",
            "GHG emissions per scenario",
    ...
    df_paths = [
        f"{OPTIM_NAME}.{COUPLING_NAME}.{EXTRA_NAME}.{GlossaryCore.YearStart}",
        f"{OPTIM_NAME}.{COUPLING_NAME}.{EXTRA_NAME}.{GlossaryCore.YearEnd}",
    ]
    year_start_dict, year_end_dict = get_df_per_scenario_dict(execution_engine, df_paths, scenario_list)
    year_start, year_end = year_start_dict[scenario_list[0]], year_end_dict[scenario_list[0]]
    ...
    if "Temperature vs Welfare" in graphs_list:
        ...
        x_axis_name = f"Temperature increase since industrial revolution" + "[\u00B0C]"
        y_axis_name = "Welfare [-]"
        ...
        (temperature_df_dict, utility_df_dict) = get_df_per_scenario_dict(execution_engine, df_paths, scenario_list)
        ...
        new_pareto_chart = get_chart_pareto_front(
    ...
    if "GHG Emissions vs Welfare" in graphs_list:
        ...
        (gwp_emissions_df_dict, utility_df_dict) = get_df_per_scenario_dict(execution_engine, df_paths)

        summed_gwp_emissions_dict, welfare_dict = {}, {}
        for scenario in scenario_list:
            summed_gwp_emissions_dict[scenario] = (
                gwp_emissions_df_dict[scenario][["N2O_20", "CO2_20", "CH4_20"]].sum().sum()
            )
            welfare_dict[scenario] = utility_df_dict[scenario][GlossaryCore.Welfare][year_end]
        namespace_w = f"{execution_engine.study_name}.{scatter_scenario}"

        new_pareto_chart = get_chart_pareto_front(
            summed_gwp_emissions_dict,
            welfare_dict,
            scenario_list,
            namespace_w,
            chart_name=chart_name,
    ...
    if "GHG Emissions vs min(Utility)" in graphs_list:
        ...
        for scenario in scenario_list:
            ...
            min_utility_dict[scenario] = min(utility_df_dict[scenario][GlossaryCore.DiscountedUtility])
        namespace_w = f"{execution_engine.study_name}.{scatter_scenario}"
    ...
    if "CO2 ppm(mean) vs Welfare" in graphs_list:
        ...
        df_paths = [
            f"{OPTIM_NAME}.{COUPLING_NAME}.{EXTRA_NAME}.GHGCycle.ghg_cycle_df_detailed",
            f"{OPTIM_NAME}.{COUPLING_NAME}.{EXTRA_NAME}.{GlossaryCore.UtilityDfValue}",
        ]
        (ghgcycle_detail_df_dict, utility_df_dict) = get_df_per_scenario_dict(execution_engine, df_paths)
    ...
    if "CO2 tax per scenario" in graphs_list:
        ...
        df_paths = [
            f"{OPTIM_NAME}.{COUPLING_NAME}.{EXTRA_NAME}.{GlossaryCore.CO2TaxesValue}",
        ]
    ...
    if "GWP emissions per scenario" in graphs_list:
        ...
        (gwp_emissions_df_dict,) = get_df_per_scenario_dict(execution_engine, df_paths)

        co2_emissions_dict = {}
        for scenario in scenario_list:
            co2_emissions_dict[scenario] = (
                gwp_emissions_df_dict[scenario][["N2O_20", "CO2_20", "CH4_20"]].sum().values.tolist()
            )

        new_chart = get_scenario_comparison_chart(
            years,
            gwp_emissions_df_dict,
            chart_name=chart_name,
            x_axis_name=x_axis_name,
            y_axis_name=y_axis_name,
            selected_scenarios=selected_scenarios,
    ...
    if "ppm per scenario" in graphs_list:
        ...
        df_paths = [f"{OPTIM_NAME}.{COUPLING_NAME}.{EXTRA_NAME}.ghg_cycle_df_detailed"]
        (ghgcycle_detail_df_dict,) = get_df_per_scenario_dict(execution_engine, df_paths)
    ...
    if "Total production per scenario" in graphs_list:
        ...
        df_paths = [f"{OPTIM_NAME}.{COUPLING_NAME}.{EXTRA_NAME}.EnergyMix.{GlossaryCore.EnergyProductionDetailedValue}"]
    ...
    if "invest per scenario" in graphs_list:
        ...
        df_paths = [f"{OPTIM_NAME}.{COUPLING_NAME}.{EXTRA_NAME}.{GlossaryCore.EnergyInvestmentsValue}"]
...
def get_scenario_comparison_chart(x_list, y_dict, chart_name, x_axis_name, y_axis_name, selected_scenarios):
    ...
    for scenario, y_values in y_dict.items():
        if scenario in selected_scenarios:
            new_series = InstanciatedSeries(x_list, y_values, scenario, "lines", True)
...
def get_chart_pareto_front(
    x_dict, y_dict, scenario_list, namespace_w, chart_name="Pareto Front", x_axis_name="x", y_axis_name="y"
):
    """
    ...
    new_pareto_chart = InstantiatedParetoFrontOptimalChart(
        ...
        primary_ordinate_axis_range=[min_y - max_y * 0.03, max_y * 1.03],
        chart_name=chart_name,
    )

    for scenario in scenario_list:
        new_serie = InstanciatedSeries(
            [x_dict[scenario]], [y_dict[scenario]], scenario, "scatter", custom_data=f"{namespace_w}.{scenario}"
        )
    ...
    sorted_list = sorted([[x_dict[scenario], y_dict[scenario]] for scenario in sorted_scenarios])
...
def get_df_per_scenario_dict(execution_engine, df_paths, scenario_list=None):
    ...
    for scenario in scenario_list:
        for i, df_path in enumerate(df_paths):
            df_per_scenario_dicts[i][scenario] = execution_engine.dm.get_value(f"{namespace_w}.{scenario}.{df_path}")
