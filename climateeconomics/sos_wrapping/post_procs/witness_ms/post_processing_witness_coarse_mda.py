...
def post_processing_filters(execution_engine, namespace):
    ...
    filters.append(
        ChartFilter(END_YEAR_NAME, years_list, year_end, END_YEAR_NAME, multiple_selection=False)
    )  # by default shows all years
    # filter on effects applies on the list of scenarios already filtered (ie it's a logical AND between the filters)
    filters.append(ChartFilter(SCENARIO_NAME, scenario_list, scenario_list, SCENARIO_NAME))
    filters.append(
        ChartFilter(EFFECT_NAME, effects_list, ALL_SCENARIOS, EFFECT_NAME, multiple_selection=False)
    )  # by default shows all studies, ie does not apply any filter
    ...
    if True in [TIPPING_POINT in scenario for scenario in scenario_list]:
        filters.append(ChartFilter(TIPPING_POINT, TIPPING_POINT_LIST, TIPPING_POINT_LIST, TIPPING_POINT))
...
def post_processings(execution_engine, namespace, filters):
    ...
    if filters is not None:
        for (
            ...
            filters
        ):  # filter on "scenarios" must occur before filter on "Effects" otherwise filter "Effects" does not work
                ...
                if effect != ALL_SCENARIOS:
                    if effect == DAMAGE_AND_TAX_NAME:
                        selected_scenarios = [
                            ...
                            if (
                                damage_tax_activation_status_dict[scenario][TAX_NAME]
                                and damage_tax_activation_status_dict[scenario][DAMAGE_NAME]
                            )
                        ]
                    else:
                        selected_scenarios = [
    ...
    if "Temperature" in graphs_list:
        ...
        (temperature_detail_df_dict,) = get_df_per_scenario_dict(execution_engine, df_paths)
        temperature_dict = {}
        for scenario in scenario_list:
            temperature_dict[scenario] = temperature_detail_df_dict[scenario][GlossaryCore.TempAtmo].values.tolist()

        new_chart = get_scenario_comparison_chart(
            years,
            temperature_dict,
    ...
    if "GDP" in graphs_list:
        ...
        df_paths = [
            "Macroeconomics." + GlossaryCore.EconomicsDetailDfValue,
        ]
        (gdp_df_dict,) = get_df_per_scenario_dict(execution_engine, df_paths)
        gdp_dict = {}
        for scenario in scenario_list:
    ...
    if "CO2 emissions" in graphs_list:
        ...
        df_paths = [GlossaryCore.GHGEmissionsDfValue]
        (co2_emissions_df_dict,) = get_df_per_scenario_dict(execution_engine, df_paths)

        co2_emissions_dict = {}
        for scenario in scenario_list:
            co2_emissions_dict[scenario] = co2_emissions_df_dict[scenario][
    ...
    if "Population" in graphs_list:
        ...
        df_paths = [
            "Population.population_detail_df",
        ]
        (pop_df_dict,) = get_df_per_scenario_dict(execution_engine, df_paths)
        pop_dict = {}
        for scenario in scenario_list:
    ...
    if "Cumulative climate deaths" in graphs_list:
        ...
        df_paths = [
            "Population.death_dict",
        ]
        (death_dict_dict,) = get_df_per_scenario_dict(execution_engine, df_paths)
        death_dict = {}
        for scenario in scenario_list:
    ...
    if "Invesments in energy" in graphs_list:
        ...
        df_paths = [f"{GlossaryCore.EnergyInvestmentsValue}"]
        (energy_investment_df_dict,) = get_df_per_scenario_dict(execution_engine, df_paths)

        energy_investment_dict = {}
        for scenario in scenario_list:
            energy_investment_dict[scenario] = energy_investment_df_dict[scenario][
    ...
    if "Investements in energy" in graphs_list:
        ...
        df_paths = [
            GlossaryEnergy.EnergyInvestmentsWoTaxValue,
        ]
        ...
        for energy in energy_list + ccs_list:
            ...
            if energy != BiomassDry.name:
                ...
                for techno in techno_list:
                    df_paths = [
                        f"{energy_disc}.{energy}.{techno}.{GlossaryEnergy.InvestLevelValue}",
                    ]
                    (invest_df_dict,) = get_df_per_scenario_dict(execution_engine, df_paths)
                    invest_dict = {}
                    for scenario in scenario_list:
                ...
                invest_per_energy = {}
                for scenario in scenario_list:
                    invest_per_energy[scenario] = list(
                        np.sum([invest_dict[scenario] for invest_dict in list_energy], axis=0)
                    )
    ...
    if "CO2 tax" in graphs_list:
        ...
        (co2_taxes_df_dict,) = get_df_per_scenario_dict(execution_engine, df_paths)
        co2_tax_dict = {}
        for scenario in scenario_list:
    ...
    if "Welfare" in graphs_list:
        ...
        new_chart = TwoAxesInstanciatedChart("", y_axis_name, [], [min_y * 0.95, max_y * 1.05], chart_name)

        for scenario, welfare in welfare_dict.items():
            if scenario in selected_scenarios:
                serie = InstanciatedSeries([""], [welfare], scenario, "bar")
    ...
    if "Utility" in graphs_list:
        ...
        utility_dict = {}
        for scenario in scenario_list:
            utility_dict[scenario] = utility_df_dict[scenario][
                GlossaryCore.DiscountedUtilityQuantityPerCapita
            ].values.tolist()
    ...
    if "ppm" in graphs_list:
        ...
        df_paths = [GlossaryCore.GHGCycleDfValue]
        (carboncycle_detail_df_dict,) = get_df_per_scenario_dict(execution_engine, df_paths)
    ...
    if "Total energy production" in graphs_list:
        ...
        (energy_production_detailed_df_dict,) = get_df_per_scenario_dict(execution_engine, df_paths)

        energy_production_detailed_dict = {}
        for scenario in scenario_list:
            energy_production_detailed_dict[scenario] = energy_production_detailed_df_dict[scenario][
    ...
    if "Fossil production" in graphs_list:
        ...
        for scenario in scenario_list:
            energy_production_brut_detailed_dict[scenario] = energy_production_brut_detailed_df_dict[scenario][
    ...
    if "Renewable production" in graphs_list:
        ...
        if "Mean energy price" in graphs_list:
            ...
            for scenario in scenario_list:
                mean_energy_price_dict[scenario] = mean_energy_price_df_dict[scenario][
        ...
        if "Consumption" in graphs_list:
            ...
            consumption_dict = {}
            for scenario in scenario_list:
                consumption_dict[scenario] = economics_df_dict[scenario][GlossaryCore.Consumption].values.tolist()
...
def get_scenario_damage_tax_activation_status(execution_engine, scenario_list):
    ...
    df_paths = [
        "ccs_price_percentage",
    ]
    (ccs_price_dict,) = get_df_per_scenario_dict(execution_engine, df_paths)
    df_paths = [
        "co2_damage_price_percentage",
    ]
    (co2_damage_price_dict,) = get_df_per_scenario_dict(execution_engine, df_paths)
    df_paths = [
        "Macroeconomics.damage_to_productivity",
    ]
    (damage_to_productivity_dict,) = get_df_per_scenario_dict(execution_engine, df_paths)
    status_dict = {}
    for scenario in scenario_list:
        status_dict[scenario] = {}
        status_dict[scenario][DAMAGE_NAME] = (
            damage_to_productivity_dict[scenario]
            and assumption_dict[scenario]["compute_climate_impact_on_gdp"]
            and assumption_dict[scenario]["activate_climate_effect_population"]
        )
        status_dict[scenario][TAX_NAME] = ccs_price_dict[scenario] > 25.0 or (
            co2_damage_price_dict[scenario] > 0 and status_dict[scenario][DAMAGE_NAME]
        )
...
def get_scenario_comparison_chart(
    x_list, y_dict, chart_name, x_axis_name, y_axis_name, selected_scenarios, status_dict=None
):
    min_x = min(x_list)
    max_x = max(x_list)
    ...
    new_chart = TwoAxesInstanciatedChart(
        x_axis_name, y_axis_name, [min_x - 5, max_x + 5], [min_y - max_y * 0.05, max_y * 1.05], chart_name
    )
