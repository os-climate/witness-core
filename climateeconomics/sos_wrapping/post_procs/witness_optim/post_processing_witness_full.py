...
def post_processing_filters(execution_engine, namespace):
    ...
    chart_list = ["Breakdown price energies", "Energies CO2 intensity", "Global CO2 breakdown sankey"]
...
def post_processings(execution_engine, namespace, filters):
    ...
    if "Energies CO2 intensity" in graphs_list:
        ...
        new_chart = get_chart_green_energies(execution_engine, namespace, chart_name=chart_name)
        if new_chart is not None:
            instanciated_charts.append(new_chart)

        chart_name = f"Energies CO2 intensity by years"
        new_chart = get_chart_green_energies(execution_engine, namespace, chart_name=chart_name, summary=False)
        if new_chart is not None:
            instanciated_charts.append(new_chart)
    # ---
    if "Global CO2 breakdown sankey" in graphs_list:
        chart_name = f"Global CO2 breakdown sankey summary"
        new_chart = get_chart_Global_CO2_breakdown_sankey(execution_engine, namespace, chart_name=chart_name)
        if new_chart is not None:
            instanciated_charts.append(new_chart)
...
def get_chart_green_energies(execution_engine, namespace, chart_name="Energies CO2 intensity", summary=True):
    ...
    if summary:
        ...
        EnergyMix = execution_engine.dm.get_disciplines_with_name(f"{namespace}.EnergyMix")[0]
        CO2_taxes, CO2_taxes_array = (
            EnergyMix.get_sosdisc_inputs(GlossaryEnergy.CO2Taxes["var_name"])[GlossaryCore.CO2Tax].values,
            [],
        )
        for i, energy in enumerate(energy_list):
            # energies level
            production += [
                np.sum(multilevel_df.groupby(level=0)["production"].sum()[energy]),
            ]
            invest += [
                np.sum(multilevel_df.groupby(level=0)[GlossaryCore.InvestValue].sum()[energy]),
            ]
            total_CO2 += [
                np.sum((multilevel_df.loc[energy]["CO2_per_kWh"] * multilevel_df.loc[energy]["production"]).sum()),
            ]
            total_price += [
                np.sum((multilevel_df.loc[energy]["price_per_kWh"] * multilevel_df.loc[energy]["production"]).sum()),
            ]
            total_price_wotaxes += [
                np.sum(
                    (multilevel_df.loc[energy]["price_per_kWh_wotaxes"] * multilevel_df.loc[energy]["production"]).sum()
                ),
            ]
            CO2_per_kWh += [
                np.divide(total_CO2[i], production[i]),
            ]
            price_per_kWh += [
                np.divide(total_price[i], production[i]),
            ]
        ...
        marker_sizes = np.multiply(production, 20.0) / pintmax + 10.0
        scatter_mean = go.Scatter(
            x=list(price_per_kWh_wotaxes),
            y=list(CO2_per_kWh),
        ...
        for i_year in range(len(years)):
            ...
            for i, energy in enumerate(energy_list):
                # energies level
                production += [
                    multilevel_df.groupby(level=0)["production"].sum()[energy][i_year],
                ]
                invest += [
                    multilevel_df.groupby(level=0)[GlossaryCore.InvestValue].sum()[energy][i_year],
                ]
                total_CO2 += [
                    (multilevel_df.loc[energy]["CO2_per_kWh"] * multilevel_df.loc[energy]["production"]).sum()[i_year],
                ]
                total_price += [
                    (multilevel_df.loc[energy]["price_per_kWh"] * multilevel_df.loc[energy]["production"]).sum()[
                        i_year
                    ],
                ]
                total_price_wotaxes += [
                    (
                        multilevel_df.loc[energy]["price_per_kWh_wotaxes"] * multilevel_df.loc[energy]["production"]
                    ).sum()[i_year],
                ]
                CO2_per_kWh += [
                    np.divide(total_CO2[i], production[i]),
            ...
            marker_sizes = np.multiply(production, 20.0) / pmax + 10.0
            scatter = go.Scatter(
                x=list(price_per_kWh_wotaxes),
                y=list(CO2_per_kWh),
                customdata=list(np.asarray(customdata).T),
        ...
        for i in range(int(len(fig.data)) - 1):
            step = dict(method="update", args=[{"visible": [False] * len(fig.data)}], label=str(2020 + i))
            step["args"][0]["visible"][i] = True
            steps.append(step)
        sliders = [
            dict(active=0, currentvalue={"prefix": "Year: "}, steps=steps),
        ]
    ...
    new_chart = InstantiatedPlotlyNativeChart(fig, chart_name=chart_name, default_title=True)
    return new_chart
...
def get_multilevel_df(execution_engine, namespace, columns=None):
    ...
    years = np.arange(
        EnergyMix.get_sosdisc_inputs(GlossaryCore.YearStart), EnergyMix.get_sosdisc_inputs(GlossaryCore.YearEnd) + 1, 1
    )
    ...
    for energy in energy_list:
        ...
        for techno in techno_list:
            ...
            production_techno = techno_disc.get_sosdisc_outputs("techno_production")[
                f"{energy} (TWh)"
            ].values * techno_disc.get_sosdisc_inputs(
            ...
            if "Forest" in techno:
                ...
                invest_techno = techno_disc.get_sosdisc_inputs("forest_investment")[
                    "forest_investment"
                ].values * techno_disc.get_sosdisc_inputs("scaling_factor_forest_investment")
                carbon_emissions = techno_disc.get_sosdisc_outputs("CO2_emissions")
            elif "Crop" in techno:
                data_fuel_dict = techno_disc.get_sosdisc_inputs("data_fuel_dict")
                invest_techno = techno_disc.get_sosdisc_inputs("crop_investment")[
                    "investment"
                ].values * techno_disc.get_sosdisc_inputs("scaling_factor_crop_investment")
                carbon_emissions = techno_disc.get_sosdisc_outputs("CO2_emissions")
            else:
                data_fuel_dict = techno_disc.get_sosdisc_inputs("data_fuel_dict")
                invest_techno = techno_disc.get_sosdisc_inputs(GlossaryCore.InvestLevelValue)[
                    GlossaryCore.InvestValue
                ].values * techno_disc.get_sosdisc_inputs("scaling_factor_invest_level")
            ...
            if "CO2_per_use" in data_fuel_dict and "high_calorific_value" in data_fuel_dict:
                if data_fuel_dict["CO2_per_use_unit"] == "kg/kg":
                    CO2_per_use = (
                        np.ones(len(carbon_emissions[GlossaryCore.Years]))
                        * data_fuel_dict["CO2_per_use"]
                        / data_fuel_dict["high_calorific_value"]
                    )
                elif data_fuel_dict["CO2_per_use_unit"] == "kg/kWh":
                    CO2_per_use = np.ones(len(carbon_emissions[GlossaryCore.Years])) * data_fuel_dict["CO2_per_use"]
            for emission_type in carbon_emissions:
                if emission_type == GlossaryCore.Years:
                    ...
                    total_carbon_emissions = CO2_per_use + carbon_emissions[techno].values
                else:
            ...
            price_per_kWh_techno = techno_disc.get_sosdisc_outputs("techno_prices")[f"{techno}"].values
            price_per_kWh_wotaxes_techno = techno_disc.get_sosdisc_outputs("techno_prices")[f"{techno}_wotaxes"].values
            idx = pd.MultiIndex.from_tuples([(f"{energy}", f"{techno}")], names=["energy", "techno"])
            columns_techno = [
                "energy",
                "technology",
                "production",
            ...
            techno_df = pd.DataFrame(
                [
                    (
                        ...
                        invest_techno,
                        CO2_per_kWh_techno,
                        price_per_kWh_techno,
                        price_per_kWh_wotaxes_techno,
                        CO2_from_production,
                        CO2_per_use,
                        CO2_after_use,
                        CO2_from_other_consumption,
...
def get_chart_Global_CO2_breakdown_sankey(execution_engine, namespace, chart_name, summary=True):
    ...
    label_col1 = ["CO2 from production", "CO2 per use", "CO2 from other consumption"]
    ...
    if summary:
        ...
        for i, energy in enumerate(energy_list):
            # energies level
            production = np.sum(multilevel_df.groupby(level=0)["production"].sum()[energy])
            # per kWh
            CO2_from_production = np.mean(
                (multilevel_df.loc[energy]["CO2_from_production"] * multilevel_df.loc[energy]["production"]).sum()
                / multilevel_df.groupby(level=0)["production"].sum()[energy]
            )
            CO2_per_use = np.mean(
                (multilevel_df.loc[energy]["CO2_per_use"] * multilevel_df.loc[energy]["production"]).sum()
                / multilevel_df.groupby(level=0)["production"].sum()[energy]
            )
            CO2_after_use = np.mean(
                (multilevel_df.loc[energy]["CO2_after_use"] * multilevel_df.loc[energy]["production"]).sum()
                / multilevel_df.groupby(level=0)["production"].sum()[energy]
            )
            CO2_from_other_consumption = np.mean(
                (
                    multilevel_df.loc[energy]["CO2_from_other_consumption"] * multilevel_df.loc[energy]["production"]
                ).sum()
                / multilevel_df.groupby(level=0)["production"].sum()[energy]
            )
            # total
            CO2_from_production_tot = (
                np.sum(
                    (multilevel_df.loc[energy]["CO2_from_production"] * multilevel_df.loc[energy]["production"]).sum()
                )
                + 1e-20
            ...
            CO2_per_use_tot = (
                np.sum((multilevel_df.loc[energy]["CO2_per_use"] * multilevel_df.loc[energy]["production"]).sum())
                + 1e-20
            )
            CO2_after_use_tot = (
                np.sum((multilevel_df.loc[energy]["CO2_after_use"] * multilevel_df.loc[energy]["production"]).sum())
                + 1e-20
            )
            CO2_from_other_consumption_tot = (
                np.sum(
                    (
                        multilevel_df.loc[energy]["CO2_from_other_consumption"]
                        * multilevel_df.loc[energy]["production"]
                    ).sum()
                )
                + 1e-20
            )
            # col1 to col2
            source += [
                i_label_dict["CO2 from production"],
            ]
            source_name += [
                "CO2 from production",
            ]
            ...
            target += [
                i_label_dict[GlossaryCore.TotalCO2Emissions],
            ]
            target_name += [
        ...
        customdata = list(
            np.array(
                [
                    ...
                    production_list,
                    flux_color,
                    np.where(np.array(flux) <= 1e-20, 0.0, np.array(flux)),
                ]
            ).T
        )
        hovertemplate = (
        ...
        rgba_list_over = cmap_over(0.1 + np.abs(flux_color) / np.max(np.abs(flux_color)))
        rgba_list_under = cmap_under(0.1 + np.abs(flux_color) / np.max(np.abs(flux_color)))
        color_over = ["rgb" + str(tuple(int((255 * (x * 0.8 + 0.2))) for x in rgba[0:3])) for rgba in rgba_list_over]
        color_under = ["rgb" + str(tuple(int((255 * (x * 0.8 + 0.2))) for x in rgba[0:3])) for rgba in rgba_list_under]
        color = np.where(np.array(flux_color) > 0.0, color_over, color_under)
        link = dict(
            source=source,
            target=target,
            value=list(np.where(np.abs(flux) > 0.0, 1.0 + 100.0 * np.abs(flux) / np.max(np.abs(flux)), 0.0)),
            color=list(color),
        ...
        for i_year in range(len(years)):
            ...
            for i, energy in enumerate(energy_list):
                # energies level
                production = multilevel_df.groupby(level=0)["production"].sum()[energy][i_year]
                # per kWh
                CO2_from_production = (
                    multilevel_df.loc[energy]["CO2_from_production"] * multilevel_df.loc[energy]["production"]
                ).sum()[i_year] / production
                CO2_per_use = (
                    multilevel_df.loc[energy]["CO2_per_use"] * multilevel_df.loc[energy]["production"]
                ).sum()[i_year] / production
                CO2_after_use = (
                    multilevel_df.loc[energy]["CO2_after_use"] * multilevel_df.loc[energy]["production"]
                ).sum()[i_year] / production
                CO2_from_other_consumption = (
                    multilevel_df.loc[energy]["CO2_from_other_consumption"] * multilevel_df.loc[energy]["production"]
                ).sum()[i_year] / production
            ...
            rgba_list_under = cmap_under(0.1 + np.abs(flux_color) / np.max(np.abs(flux_color)))
            color_over = [
                "rgb" + str(tuple(int((255 * (x * 0.8 + 0.2))) for x in rgba[0:3])) for rgba in rgba_list_over
            ]
            color_under = [
                "rgb" + str(tuple(int((255 * (x * 0.8 + 0.2))) for x in rgba[0:3])) for rgba in rgba_list_under
            ]
            color = np.where(np.array(flux_color) > 0.0, color_over, color_under)
            link = dict(
                source=source,
