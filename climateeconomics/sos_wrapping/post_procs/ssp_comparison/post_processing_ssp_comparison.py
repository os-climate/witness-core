...
_consumption = {
    FILE_NAME: "consumption_global_usd2020.csv",
...
_mean_temperature = {
    FILE_NAME: "mean_temperature_global.csv",
...
_co2_concentration = {
    FILE_NAME: "CO2_concentration_global.csv",
...
_ch4_concentration = {
    FILE_NAME: "CH4_concentration_global.csv",
    VAR_NAME: GlossaryCore.GHGCycleDfValue,
...
_n2o_concentration = {
    FILE_NAME: "N2O_concentration_global.csv",
    VAR_NAME: GlossaryCore.GHGCycleDfValue,
...
_co2_emissions = {
    FILE_NAME: "CO2_emissions_global.csv",
...
_ch4_emissions = {
    FILE_NAME: "CH4_emissions_global.csv",
    VAR_NAME: "GHGEmissions.GHG_emissions_detail_df",
...
_n2o_emissions = {
    FILE_NAME: "N2O_emissions_global.csv",
    VAR_NAME: "GHGEmissions.GHG_emissions_detail_df",
...
_forest_surface = {
    FILE_NAME: "land_cover_forest_global.csv",
...
_agriculture_surface = {
    FILE_NAME: "land_cover_pasture+cropland_global.csv",
    VAR_NAME: "Land_Use.land_surface_detail_df",
...
FINAL_ENERGY = "energy"
_final_energy = {
    FILE_NAME: "final_energy_global.csv",
...
COAL = "Coal"
_coal = {
    FILE_NAME: "coal_fraction.csv",
    UNIT_CONV_FACTOR: 1.0,
    WITNESS_VARS_COLS: [
        (f"EnergyMix.solid_fuel.{GlossaryCore.EnergyProductionDetailedValue}", "solid_fuel CoalExtraction (TWh)"),
        # ('EnergyMix.electricity.{GlossaryCore.EnergyProductionDetailedValue}', 'electricity CoalGen (TWh)'),
    ],
}
OIL_GAS = "Oil & Gas"
_oil_gas = {
    FILE_NAME: "oil_gas_fraction.csv",
    UNIT_CONV_FACTOR: 1.0,
    WITNESS_VARS_COLS: [
        (f"EnergyMix.methane.{GlossaryCore.EnergyProductionDetailedValue}", "methane FossilGas (TWh)"),
        ...
        (
            f"EnergyMix.fuel.{GlossaryCore.EnergyProductionDetailedValue}",
            "fuel.hydrotreated_oil_fuel HefaDecarboxylation (TWh)",
        ),
        (
            f"EnergyMix.fuel.{GlossaryCore.EnergyProductionDetailedValue}",
            "fuel.hydrotreated_oil_fuel HefaDeoxygenation (TWh)",
        ),
...
_non_fossil = {
    FILE_NAME: "non-fossil_fraction.csv",
    UNIT_CONV_FACTOR: 1.0,
}
...
PRIMARY_ENERGY_DATA = {COAL: _coal, OIL_GAS: _oil_gas, NON_FOSSIL: _non_fossil}
...
WITNESS_BRUT_ENERGY_TOTAL_MINUS = [
    ...
    (
        f"EnergyMix.electricity.{GlossaryCore.EnergyProductionDetailedValue}",
        "electricity CombinedCycleGasTurbine (TWh)",
    ),
    (f"EnergyMix.electricity.{GlossaryCore.EnergyProductionDetailedValue}", "electricity GasTurbine (TWh)"),
    (f"EnergyMix.electricity.{GlossaryCore.EnergyProductionDetailedValue}", "electricity OilGen (TWh)"),
    (f"EnergyMix.methane.{GlossaryCore.EnergyProductionDetailedValue}", "methane Methanation (TWh)"),
...
def get_ssp_data(data_name, data_dict, region="World"):
    ...
    var_df[SCENARIO] = [
        f"{_sc.split('-Baseline')[0]} ({_model})" for _sc, _model in var_df[[SCENARIO, MODEL]].values.tolist()
    ]
    var_df = (
        var_df[[SCENARIO] + CSV_YRS]
        .set_index(SCENARIO, drop=True)
        .transpose()
        .reset_index()
    ...
    var_df = var_df.reindex(
        columns=[YEARS] + sorted(set(var_df.columns) - {YEARS})
    )  # sort the scenarios by name for clarity
...
def get_comp_chart_from_df(comp_df, y_axis_name, chart_name):
    ...
    new_chart = TwoAxesInstanciatedChart(
        YEARS, y_axis_name, [min_x - 5, max_x + 5], [min_y - max_y * 0.05, max_y * 1.05], chart_name
    )
...
def get_witness_primary_energy_chart(execution_engine, namespace):
    ...
    for energy_type in energy_df.columns[1:]:
        series = InstanciatedSeries(
            list(WITNESS_YEARS), energy_df[energy_type].values.tolist(), energy_type, InstanciatedSeries.BAR_DISPLAY
        )
...
def get_ssp_primary_energy_charts():
    ...
    for scenario, scenario_df in scenario_dfs.items():
        ...
        for energy_type in var_dfs:
            series = InstanciatedSeries(
                list(WITNESS_YEARS),
                scenario_df[energy_type].values.tolist(),
                energy_type,
...
def post_processings(execution_engine, namespace, filters):
    ...
    def get_comparison_chart(data_name):
        ...
        witness_data = (
            execution_engine.dm.get_value(var_f_name)[[column]].copy().rename(columns={column: WITNESS_SERIES_NAME})
        )
