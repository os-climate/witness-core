...
def post_processings(execution_engine, scenario_name, chart_filters=None):
    ...
    if "Output per sector" in chart_list or True:
        ...
        new_chart = TwoAxesInstanciatedChart(
            GlossaryCore.Years, GlossaryCore.SectorGdpDf["unit"], chart_name=chart_name, stacked_bar=True
        )
        for sector in GlossaryCore.SectorsPossibleValues:
            new_series = InstanciatedSeries(
                years, list(gdp_all_sectors[sector].values), sector, display_type=InstanciatedSeries.BAR_DISPLAY
            )
