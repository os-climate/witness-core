...
def post_processing_filters(execution_engine, namespace):
    ...
    chart_list = ["GDP breakdown on sub-sectors"]
...
def post_processings(execution_engine, scenario_name, sector: str, chart_filters=None):
    ...
    if "GDP breakdown on sub-sectors" in chart_list:
        section_gdp_df = get_scenario_value(
            execution_engine, f"{sector}.{GlossaryCore.SectionGdpDfValue}", scenario_name
        )
        ...
        new_series = InstanciatedSeries(
            years, list(sector_gdp_df[sector]), f"Total GDP for {sector} sector", "lines", True
        )
        new_chart.add_series(new_series)
