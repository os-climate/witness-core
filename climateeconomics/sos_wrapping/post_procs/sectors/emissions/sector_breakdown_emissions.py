...
def post_processings(execution_engine, scenario_name, sector, chart_filters=None):
    ...
    if GlossaryCore.TotalEmissions in chart_list:
        ...
        breakdown_emission_sector = get_scenario_value(
            execution_engine, f"{sector}.{GlossaryCore.EmissionsDfValue}", scenario_name
        )
        ...
        new_chart = TwoAxesInstanciatedChart(
            GlossaryCore.Years,
            f"Emissions [{GlossaryCore.EmissionDf['unit']}]",
        ...
        new_series = InstanciatedSeries(
            years,
            list(breakdown_emission_sector[GlossaryCore.NonEnergyEmissions].values),
    ...
    if GlossaryCore.SectionEmissionPart in chart_list:
        ...
        sections_emission = get_scenario_value(
            execution_engine, f"{sector}.{GlossaryCore.SectionEmissionDfValue}", scenario_name
        )
        chart_name = f"Breakdown of emission per section for {sector} sector [{GlossaryCore.SectionEmissionDf['unit']}]"
        new_chart = TwoAxesInstanciatedChart(
            GlossaryCore.Years, GlossaryCore.SectionEmissionPart, chart_name=chart_name, stacked_bar=True
        )
        ...
        for section in GlossaryCore.SectionDictSectors[sector]:
            new_series = InstanciatedSeries(
                years,
                list(sections_emission[section].values),
                f"{section}",
        ...
        sector_emissions = get_scenario_value(
            execution_engine, f"{sector}.{GlossaryCore.EmissionsDfValue}", scenario_name
        )

        chart_name = f"Breakdown of emission share per section for {sector} sector [%]"
        new_chart = TwoAxesInstanciatedChart(
            GlossaryCore.Years, "Share percentage per section", chart_name=chart_name, stacked_bar=True
        )
        for section in GlossaryCore.SectionDictSectors[sector]:
            share_of_section = list(
                sections_emission[section].values / sector_emissions[GlossaryCore.TotalEmissions].values * 100.0
            )
            new_series = InstanciatedSeries(
                years, share_of_section, f"{section}", display_type=InstanciatedSeries.BAR_DISPLAY
            )
            new_chart.add_series(new_series)
    ...
    if GlossaryCore.SectionEnergyEmissionPart in chart_list:
        ...
        sections_energy_emission = get_scenario_value(
            execution_engine, f"{sector}.{GlossaryCore.SectionEnergyEmissionDfValue}", scenario_name
        )

        chart_name = f"Breakdown of energy emission per section for {sector} sector [{GlossaryCore.SectionEnergyEmissionDf['unit']}]"

        new_chart = TwoAxesInstanciatedChart(
            GlossaryCore.Years, GlossaryCore.SectionEnergyEmissionPart, chart_name=chart_name, stacked_bar=True
        )
        new_series = InstanciatedSeries(
            years,
            list(breakdown_emission_sector[GlossaryCore.EnergyEmissions].values),
            f"Total energy emissions",
            display_type=InstanciatedSeries.LINES_DISPLAY,
        )
        new_chart.add_series(new_series)
        ...
        for section in GlossaryCore.SectionDictSectors[sector]:
            new_series = InstanciatedSeries(
                years,
                list(sections_energy_emission[section].values),
                f"{section}",
                display_type=InstanciatedSeries.BAR_DISPLAY,
            )
            new_chart.add_series(new_series)
    ...
    if GlossaryCore.SectionNonEnergyEmissionPart in chart_list:
        ...
        sections_non_energy_emission = get_scenario_value(
            execution_engine, f"{sector}.{GlossaryCore.SectionNonEnergyEmissionDfValue}", scenario_name
        )

        chart_name = f"Breakdown of non energy emission per section for {sector} sector [{GlossaryCore.SectionNonEnergyEmissionDf['unit']}]"

        new_chart = TwoAxesInstanciatedChart(
            GlossaryCore.Years, GlossaryCore.SectionNonEnergyEmissionPart, chart_name=chart_name, stacked_bar=True
        )
        new_series = InstanciatedSeries(
            years,
            list(breakdown_emission_sector[GlossaryCore.NonEnergyEmissions].values),
            f"Total Non energy emissions",
            display_type=InstanciatedSeries.LINES_DISPLAY,
        ...
        for section in GlossaryCore.SectionDictSectors[sector]:
            new_series = InstanciatedSeries(
                years,
                list(sections_non_energy_emission[section].values),
                f"{section}",
                display_type=InstanciatedSeries.BAR_DISPLAY,
            )
            new_chart.add_series(new_series)
    ...
    if GlossaryCore.SectionEnergyConsumptionPartTWh in chart_list:
        ...
        sections_energy_consumption = get_scenario_value(
            execution_engine, f"{sector}.{GlossaryCore.SectionEnergyConsumptionDfValue}", scenario_name
        )
        ...
        new_chart = TwoAxesInstanciatedChart(
            GlossaryCore.Years, GlossaryCore.SectionEnergyConsumptionPartTWh, chart_name=chart_name, stacked_bar=True
        )
        ...
        for section in GlossaryCore.SectionDictSectors[sector]:
            new_series = InstanciatedSeries(
                years,
                list(sections_energy_consumption[section].values * 1000),
                f"{section}",
                display_type=InstanciatedSeries.BAR_DISPLAY,
            )
            new_chart.add_series(new_series)
