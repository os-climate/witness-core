...
def post_processings(execution_engine, scenario_name, chart_filters=None):  # scenario_name, chart_filters=None):
    ...
    if "Objective Lagrangian" in chart_list:
        ...
        graph_years = TwoAxesInstanciatedChart(
            GlossaryEnergy.Years, f"Invest {GlossaryEnergy.invest_mix} [G$]", chart_name="Output profile invest"
        )
        ...
        outputs = [
            f"{scenario_name.split('.')[0]}.{OBJECTIVE_LAGR}"
        ]  # the objective lagrangian is one level above the scenario level
        ...
        grad_analytical = mdo_disc.linearize(
            force_no_exec=True
        )  # can add data of the converged point (see sos_mdo_discipline.py)
        ...
        if dm_data_dict_pkl is not None:
            compare_test_passed, error_msg_compare = test_compare_dm(
                dm_data_dict_before, dm_data_dict_pkl, scenario_name, "pkl vs case dm"
            )
        ...
        if recompute_gradients:
            ...
            approx = DisciplineJacApprox(
                mdo_disc, approx_method=DisciplineJacApprox.FINITE_DIFFERENCES, step=finite_differences_step
            )
        ...
        new_chart = TwoAxesInstanciatedChart(
            "Design variables (coeff_i)", "Gradient [-]", chart_name=chart_name, y_min_zero=False
        )

        visible_line = True
        new_series = InstanciatedSeries(coeff_i, grad_analytical_list, "Adjoint Gradient", "lines", visible_line)
        new_chart.add_series(new_series)
        new_series = InstanciatedSeries(coeff_i, grad_approx_list, "Finite Differences", "lines", visible_line)
        new_chart.add_series(new_series)
        ...
        new_chart = TwoAxesInstanciatedChart(
            "Design variables (coeff_i)", "Gradient relative error [%]", chart_name=chart_name, y_min_zero=False
        )

        new_chart.primary_ordinate_axis_range = [-100.0, 100]
        visible_line = True
        # unless the optimum is reached, a non-zero gradient is expected
        error = (np.array(grad_analytical_list) - np.array(grad_approx_list)) / np.array(grad_approx_list) * 100.0
        new_series = InstanciatedSeries(coeff_i, list(error), "", "lines", visible_line)
        new_chart.add_series(new_series)
        instanciated_charts.append(new_chart)
