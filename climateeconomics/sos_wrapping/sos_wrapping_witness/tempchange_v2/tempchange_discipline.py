...
class TempChangeDiscipline(ClimateEcoDiscipline):
    ...
    DESC_IN = {
        ...
        "init_temp_atmo": {
            "type": "float",
            "default": DatabaseWitnessCore.TemperatureAnomalyPreIndustrialYearStart.value,
            "user_level": 2,
            "unit": "°C",
        },
        ...
        "temperature_model": {
            "type": "string",
            "default": "FUND",
            "possible_values": ["DICE", "FUND", "FAIR"],
            "structuring": True,
        },
        ...
        "pre_indus_co2_concentration_ppm": {
            "type": "float",
        ...
        "temperature_obj_option": {
            "type": "string",
            "possible_values": [TempChange.LAST_TEMPERATURE_OBJECTIVE, TempChange.INTEGRAL_OBJECTIVE],
            "default": TempChange.INTEGRAL_OBJECTIVE,
            "visibility": "Shared",
            "namespace": GlossaryCore.NS_WITNESS,
        },
        "temperature_change_ref": {
            "type": "float",
            "default": 0.2,
            "unit": "°C",
            "visibility": ClimateEcoDiscipline.SHARED_VISIBILITY,
        ...
        "temperature_end_constraint_ref": {"type": "float", "default": 3.0, "unit": "°C", "user_level": 2},
    ...
    def compute_sos_jacobian(self):
        ...
        if forcing_model == "DICE":
            ...
            self.set_partial_derivative_for_other_types(
                ("forcing_detail_df", "CH4 and N2O forcing"),
        ...
        if temperature_model == "DICE":
            ...
            if forcing_model == "Myhre":
                ...
                d_temp_d_co2_ppm = np.matmul(
                    d_temp_d_forcing_fund,
                    identity
                    * (d_forcing_datmo_conc["CO2 forcing CO2 ppm"] + d_forcing_datmo_conc["N2O forcing CO2 ppm"]),
                )
                d_temp_d_ch4_ppm = np.matmul(
                    d_temp_d_forcing_fund,
                    identity
                    * (d_forcing_datmo_conc["CH4 forcing CH4 ppm"] + d_forcing_datmo_conc["N2O forcing CH4 ppm"]),
                )
                d_temp_d_n2o_ppm = np.matmul(
                    d_temp_d_forcing_fund,
                    identity
                    * (
                        d_forcing_datmo_conc["CO2 forcing N2O ppm"]
                        + d_forcing_datmo_conc["CH4 forcing N2O ppm"]
    ...
    def get_post_processing_list(self, chart_filters=None):
        ...
        if "temperature evolution" in chart_list:
            ...
            temperature_df = deepcopy(self.get_sosdisc_outputs("temperature_detail_df"))
...
def temperature_evolution(model, temperature_df, instanciated_charts):
    if model == "DICE":
        ...
        legend = {GlossaryCore.TempAtmo: "Atmosphere", GlossaryCore.TempOcean: "Ocean"}
    ...
    min_values = {}
    for key in to_plot:
        min_values[key], max_values[key] = ppt.get_greataxisrange(temperature_df[to_plot])
    ...
    new_chart = TwoAxesInstanciatedChart(
        GlossaryCore.Years, "°C", [year_start - 5, year_end + 5], [min_value, max_value], chart_name
    )
    ...
    if model == "FUND":
        ...
        new_series = InstanciatedSeries(years, ordonate_data, "Seal level evolution [m]", "lines", visible_line)
        new_chart.series.append(new_series)
...
def radiative_forcing(forcing_df, instanciated_charts):
    ...
    new_chart = TwoAxesInstanciatedChart(GlossaryCore.Years, "Radiative forcing (W.m-2)", chart_name=chart_name)

    for forcing in forcing_df.columns:
        if forcing != GlossaryCore.Years:
            new_series = InstanciatedSeries(years, forcing_df[forcing].values.tolist(), forcing, "lines")
