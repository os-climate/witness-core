...
class GHGCycleDiscipline(ClimateEcoDiscipline):
    ...
    co2_init_conc_fund = (
        np.array([296.002949511, 5.52417779186, 6.65150094285, 2.39635475726, 0.17501699667]) * 412.4 / 296.002949511
    )
    ...
    DESC_IN = {
        ...
        "co2_emissions_fractions": {
            "type": "list",
            "subtype_descriptor": {"list": "float"},
            "unit": "-",
            "default": [0.13, 0.20, 0.32, 0.25, 0.10],
            "user_level": 2,
        },
        "co2_boxes_decays": {
            "type": "list",
            "subtype_descriptor": {"list": "float"},
            "unit": GlossaryCore.Years,
        ...
        "co2_pre_indus_conc": {
            "type": "float",
            "unit": "ppm",
            "default": DatabaseWitnessCore.CO2PreIndustrialConcentration.value,
            "user_level": 2,
        },
        ...
        "ch4_pre_indus_conc": {
            "type": "float",
            "unit": "ppb",
            "default": DatabaseWitnessCore.CH4PreIndustrialConcentration.value,
            "user_level": 2,
        },
        "ch4_init_conc": {
            "type": "float",
            "unit": "ppb",
            "default": DatabaseWitnessCore.CH4YearStartConcentration.value,
            "user_level": 2,
        },
        ...
        "n2o_pre_indus_conc": {
            "type": "float",
            "unit": "ppb",
            "default": DatabaseWitnessCore.N2OPreIndustrialConcentration.value,
            "user_level": 2,
        },
        "n2o_init_conc": {
            "type": "float",
            "unit": "ppb",
            "default": DatabaseWitnessCore.N2OYearStartConcentration.value,
            "user_level": 2,
        },
    ...
    def run(self):
        ...
        dict_values = {
            GlossaryCore.GHGCycleDfValue: self.ghg_cycle.ghg_cycle_df[
                [
                    GlossaryCore.Years,
    ...
    def compute_sos_jacobian(self):
        ...
        d_gwp20_objective_d_total_co2_emissions = self.ghg_cycle.d_gwp20_objective_d_ppm(
            d_ppm=d_ghg_ppm_d_emissions[GlossaryCore.CO2], specie=GlossaryCore.CO2
        )
        d_gwp20_objective_d_total_ch4_emissions = self.ghg_cycle.d_gwp20_objective_d_ppm(
            d_ppm=d_ghg_ppm_d_emissions[GlossaryCore.CH4], specie=GlossaryCore.CH4
        )
        d_gwp20_objective_d_total_n2o_emissions = self.ghg_cycle.d_gwp20_objective_d_ppm(
            d_ppm=d_ghg_ppm_d_emissions[GlossaryCore.N2O], specie=GlossaryCore.N2O
        )

        self.set_partial_derivative_for_other_types(
            ("gwp20_objective",),
            (GlossaryCore.GHGEmissionsDfValue, GlossaryCore.TotalCO2Emissions),
            d_gwp20_objective_d_total_co2_emissions,
        )
        self.set_partial_derivative_for_other_types(
            ("gwp20_objective",),
            (GlossaryCore.GHGEmissionsDfValue, GlossaryCore.TotalCH4Emissions),
            d_gwp20_objective_d_total_ch4_emissions,
        )
        self.set_partial_derivative_for_other_types(
            ("gwp20_objective",),
            (GlossaryCore.GHGEmissionsDfValue, GlossaryCore.TotalN2OEmissions),
            d_gwp20_objective_d_total_n2o_emissions,
        )
        ...
        self.set_partial_derivative_for_other_types(
            ...
            (GlossaryCore.GHGEmissionsDfValue, GlossaryCore.TotalCO2Emissions),
            self.ghg_cycle.d_total_co2_equivalent_d_conc(
                d_conc=d_ghg_ppm_d_emissions[GlossaryCore.CO2], specie=GlossaryCore.CO2, gwp=self.ghg_cycle.gwp_20
            ),
        )
        ...
        self.set_partial_derivative_for_other_types(
            ...
            (GlossaryCore.GHGEmissionsDfValue, GlossaryCore.TotalCH4Emissions),
            self.ghg_cycle.d_total_co2_equivalent_d_conc(
                d_conc=d_ghg_ppm_d_emissions[GlossaryCore.CH4], specie=GlossaryCore.CH4, gwp=self.ghg_cycle.gwp_20
            ),
        )
        ...
        self.set_partial_derivative_for_other_types(
            ...
            (GlossaryCore.GHGEmissionsDfValue, GlossaryCore.TotalN2OEmissions),
            self.ghg_cycle.d_total_co2_equivalent_d_conc(
                d_conc=d_ghg_ppm_d_emissions[GlossaryCore.N2O], specie=GlossaryCore.N2O, gwp=self.ghg_cycle.gwp_20
            ),
        )
    ...
    def get_post_processing_list(self, chart_filters=None):
        ...
        if "Atmospheric concentrations" in chart_list:
            ...
            new_series = InstanciatedSeries(years, ordonate_data, "Pre-industrial level", "dash_lines", True)
            new_chart.add_series(new_series)
            ...
            new_chart = TwoAxesInstanciatedChart(GlossaryCore.Years, "parts per billion", chart_name=chart_name)

            visible_line = True
            ordonate_data = list(ppm)
            new_series = InstanciatedSeries(years, ordonate_data, "ppb", "lines", visible_line)
            new_chart.add_series(new_series)
        ...
        if GlossaryCore.ExtraCO2EqSincePreIndustrialValue in chart_list:
            ...
            new_chart = TwoAxesInstanciatedChart(
                GlossaryCore.Years,
                GlossaryCore.ExtraCO2EqSincePreIndustrialDf["unit"],
            ...
            new_series = InstanciatedSeries(years, ordonate_data, "20-year basis (applied)", "lines", visible_line)
            new_chart.add_series(new_series)

            ordonate_data = list(extra_co2_eq_df[GlossaryCore.ExtraCO2EqSincePreIndustrial10OYbasisValue])
            new_series = InstanciatedSeries(years, ordonate_data, "100-year basis", "lines", visible_line)
            new_chart.add_series(new_series)
        ...
        if GlossaryCore.GlobalWarmingPotentialdDfValue in chart_list:
            ...
            new_chart = TwoAxesInstanciatedChart(
                GlossaryCore.Years,
                GlossaryCore.GlobalWarmingPotentialdDf["unit"],
                chart_name=chart_name,
                stacked_bar=True,
            )
            ...
            for ghg in GlossaryCore.GreenHouseGases:
                ...
                new_series = InstanciatedSeries(years, ordonate_data, ghg, "bar", visible_line)
                new_chart.add_series(new_series)

            ordonate_data = list(global_warming_potential_df[f"Total {GlossaryCore.YearBasis20}"])

            new_series = InstanciatedSeries(years, ordonate_data, "Total", "lines", visible_line)
            new_chart.add_series(new_series)
