...
class PolicyDiscipline(SoSWrapp):
    ...
    def run(self):
        ...
        dict_values = {GlossaryCore.CO2TaxesValue: self.policy_model.CO2_tax}
    ...
    def get_post_processing_list(self, chart_filters=None):
        ...
        if "CO2eq tax" in chart_list:
            ...
            new_chart = TwoAxesInstanciatedChart(GlossaryCore.Years, "$/tCO2eq", chart_name=chart_name)

            new_series = InstanciatedSeries(years, list(CCS_price["ccs_price_per_tCO2"].values), "CCS price", "lines")

            new_series2 = InstanciatedSeries(
                years, list(CO2_damage_price[GlossaryCore.CO2DamagePrice].values), "CO2eq damage", "lines"
            )

            new_series3 = InstanciatedSeries(years, list(CO2_tax[GlossaryCore.CO2Tax].values), "CO2eq tax", "lines")
