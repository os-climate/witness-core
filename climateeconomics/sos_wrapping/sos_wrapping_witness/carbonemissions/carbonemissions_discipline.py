...
class CarbonemissionsDiscipline(ClimateEcoDiscipline):
    ...
    DESC_IN = {
        ...
        "min_co2_objective": {"type": "float", "default": -1000.0, "unit": "GtCO2", "user_level": 2},
        "total_emissions_ref": {
            "type": "float",
            "default": 39.6,
            "unit": "GtCO2",
    ...
    def run(self):
        ...
        # Store output data
        dict_values = {
            GlossaryCore.CO2EmissionsDetailDfValue: CO2_emissions_df,
            GlossaryCore.CO2EmissionsDfValue: CO2_emissions_df[
                GlossaryCore.CO2EmissionsDf["dataframe_descriptor"].keys()
            ],
            "CO2_objective": CO2_objective,
            GlossaryCore.CO2EmissionsGtValue: self.emissions_model.co2_emissions[
    ...
    def compute_sos_jacobian(self):
        ...
        ) = self.emissions_model.compute_d_indus_emissions()
        ...
        columns_sources = self.get_sosdisc_inputs("CO2_emissions_by_use_sources").columns
        # fill jacobians
        ...
        sinks_dict = {
            "CO2_emissions_by_use_sinks": f"CO2 removed by energy mix (Gt)",
            "co2_emissions_needed_by_energy_mix": "carbon_capture needed by energy mix (Gt)",
            "co2_emissions_ccus_Gt": "carbon_storage Limited by capture (Gt)",
        }
    ...
    def get_chart_filter_list(self):
        ...
        chart_list = ["Carbon emissions", "Sources and sinks", "Cumulated CO2 emissions"]
    ...
    def get_chart_co2_emissions(self):
        ...
        CO2_emissions_df = deepcopy(self.get_sosdisc_outputs(GlossaryCore.CO2EmissionsDetailDfValue))
    ...
    def get_chart_cumulated_co2_emissions(self):
        ...
        new_chart = TwoAxesInstanciatedChart(GlossaryCore.Years, "CO2 emissions (GtCO2)", chart_name=chart_name)

        new_series = InstanciatedSeries(years, total_emission_cum.values.tolist(), "lines")
    ...
    def get_chart_sources_and_sinks(self, detailed=False):
        ...
        CO2_emissions_breakdown["category energy sources (Gt)"] = CO2_emissions_breakdown[cols_to_sum].sum(axis=1)

        cols_to_sum = []
        # Get all the sinks and put them as columns in df
        CO2_emissions_by_use_sinks = self.get_sosdisc_inputs("CO2_emissions_by_use_sinks")
        for col in CO2_emissions_by_use_sinks.columns:
            if col != GlossaryCore.Years:
                CO2_emissions_breakdown[col] = -CO2_emissions_by_use_sinks[col].values
                cols_to_sum += [
                    col,
                ]
        ...
        for col in co2_emissions_needed_by_energy_mix.columns:
            if col != GlossaryCore.Years:
                CO2_emissions_breakdown[col] = -co2_emissions_needed_by_energy_mix[col].values
                cols_to_sum += [
                    col,
                ]
        co2_emissions_ccus_Gt = self.get_sosdisc_inputs("co2_emissions_ccus_Gt")
        for col in co2_emissions_ccus_Gt.columns:
            if col != GlossaryCore.Years:
                CO2_emissions_breakdown[col] = -co2_emissions_ccus_Gt[col].values
                cols_to_sum += [
                    col,
                ]
        # Sum all the sources columns
        CO2_emissions_breakdown["category energy sinks (Gt)"] = CO2_emissions_breakdown[cols_to_sum].sum(axis=1)
        ...
        gross_output_ter = self.get_sosdisc_inputs(GlossaryCore.EconomicsDfValue)[GlossaryCore.GrossOutput].values
        ...
        # Sum all the sources columns
        CO2_emissions_breakdown["category land_use sources (Gt)"] = CO2_emissions_df["land_emissions"].values
        ...
        if detailed:
            for col in CO2_emissions_breakdown.columns:
                if "category" not in col and col != GlossaryCore.Years:
                    legend_title = f"{col}".replace(" (Gt)", "")
                    serie = InstanciatedSeries(
                        CO2_emissions_breakdown[GlossaryCore.Years].values.tolist(),
                        CO2_emissions_breakdown[col].values.tolist(),
                        legend_title,
                        "bar",
                    )
