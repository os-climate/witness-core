...
class PopulationDiscipline(ClimateEcoDiscipline):
    ...
    pop_init_df = pd.read_csv(join(global_data_dir, "population_by_age_2020.csv"))
    default_death_rate_params_df = pd.read_csv(join(global_data_dir, "death_rate_params_v2.csv"))
    ...
    default_climate_mortality_param_df = pd.read_csv(join(global_data_dir, "climate_additional_deaths_V2.csv"))
    ...
    DESC_IN = {
        ...
        "climate_mortality_param_df": {
            "type": "dataframe",
            "default": default_climate_mortality_param_df,
            "user_level": 3,
            "unit": "-",
            "dataframe_descriptor": {
                GlossaryCore.Years: ("float", None, False),
        ...
        "death_rate_param": {
            "type": "dataframe",
            "default": default_death_rate_params_df,
            "user_level": 3,
            "unit": "-",
            "dataframe_descriptor": {
                GlossaryCore.Years: ("float", None, False),
        ...
        # 4.03359157e+03
        "birth_rate_phi": {"type": "float", "default": 4.03360000e03, "user_level": 3, "unit": "-"},
        # 3.93860555e-01
    ...
    def setup_sos_disciplines(self):  # type: (...) -> None
        if GlossaryCore.YearStart in self.get_data_in():
            ...
            default_calories_pc_df = pd.DataFrame({GlossaryCore.Years: years, "kcal_pc": 2400.0})
    ...
    def run(self):
        ...
        population_df[GlossaryCore.PopulationValue] = population_df[GlossaryCore.PopulationValue] / self.model.million
        # disable pylint warning, known issue for pylint >2.4, pylint cannot get some variable type even if it has been set
        population_detail_df[GlossaryCore.Population1570] = working_age_population_df[
            GlossaryCore.Population1570
        ]  # pylint: disable=unsubscriptable-object
        working_age_population_df[GlossaryCore.Population1570] = (
            working_age_population_df[GlossaryCore.Population1570] / self.model.million
    ...
    def get_chart_filter_list(self):
        ...
        chart_list = [
            "World population",
            "Population detailed",
            "Population detailed year start",
            "Population detailed mid year",
            "15-49 age range birth rate",
            "knowledge",
            "death rate per age range",
            "Number of birth and death per year",
            "Cumulative climate deaths",
            "Number of climate death per year",
            "Cumulative malnutrition deaths",
            "Number of malnutrition death per year",
            "Malnutrition death rate per age range",
            "Life expectancy evolution",
            "working-age population over years",
        ]
        ...
        chart_filters.append(ChartFilter("Years for population", years, [year_start, year_end], GlossaryCore.Years))
    ...
    def get_post_processing_list(self, chart_filters=None):
        ...
        pop_df = deepcopy(self.get_sosdisc_outputs("population_detail_df"))
        birth_rate_df = deepcopy(self.get_sosdisc_outputs("birth_rate_df"))
        birth_df = deepcopy(self.get_sosdisc_outputs("birth_df"))
        death_rate_dict = deepcopy(self.get_sosdisc_outputs("death_rate_dict"))
        death_dict = deepcopy(self.get_sosdisc_outputs("death_dict"))
        life_expectancy_df = deepcopy(self.get_sosdisc_outputs("life_expectancy_df"))
        ...
        if "World population" in chart_list:
            ...
            min_value, max_value = self.get_greataxisrange(pop_df["total"])
            ...
            new_series = InstanciatedSeries(years, ordonate_data, GlossaryCore.PopulationValue, "lines", visible_line)
        ...
        if "working-age population over years" in chart_list:
            ...
            min_value, max_value = self.get_greataxisrange(pop_df[GlossaryCore.Population1570])

            chart_name = "working-age population over years"

            new_chart = TwoAxesInstanciatedChart(
                GlossaryCore.Years,
                "15-70 age range population",
                [year_start - 5, year_end + 5],
                [min_value, max_value],
                chart_name,
            )
        ...
        if "15-49 age range birth rate" in chart_list:
            ...
            min_value, max_value = self.get_greataxisrange(birth_rate_df["birth_rate"])

            chart_name = "15-49 age range birth rate"

            new_chart = TwoAxesInstanciatedChart(
                GlossaryCore.Years, " birth rate", [year_start - 5, year_end + 5], [min_value, max_value], chart_name
            )

            visible_line = True
            ordonate_data = list(birth_rate_df["birth_rate"])

            new_series = InstanciatedSeries(years, ordonate_data, "15-49 birth rate", "lines", visible_line)
        ...
        if "knowledge" in chart_list:
            ...
            min_value, max_value = self.get_greataxisrange(birth_rate_df["knowledge"])

            chart_name = "Knowledge yearly evolution"

            new_chart = TwoAxesInstanciatedChart(
                GlossaryCore.Years, "knowledge", [year_start - 5, year_end + 5], [min_value, max_value], chart_name
            )

            visible_line = True
            ordonate_data = list(birth_rate_df["knowledge"])

            new_series = InstanciatedSeries(years, ordonate_data, "knowledge", "lines", visible_line)
        ...
        if "death rate per age range" in chart_list:
            ...
            max_values = {}
            for key in to_plot:
                min_values[key], max_values[key] = self.get_greataxisrange(death_rate_dict["total"][key])
            ...
            new_chart = TwoAxesInstanciatedChart(
                GlossaryCore.Years, " death rate", [year_start - 5, year_end + 5], [min_value, max_value], chart_name
            )
            for key in to_plot:
                visible_line = True
        ...
        if "Malnutrition death rate per age range" in chart_list:
            ...
            max_values = {}
            for key in to_plot:
                min_values[key], max_values[key] = self.get_greataxisrange(death_rate_dict["diet"][key])
            ...
            for key in to_plot:
                ...
                new_series = InstanciatedSeries(
                    years,
                    ordonate_data,
                    f"death rate imputable to malnutrition for age range {key}",
                    "lines",
                    visible_line,
                )
        ...
        if "Number of birth and death per year" in chart_list:
            ...
            new_series = InstanciatedSeries(years, ordonate_data, "Number of birth per year", "lines", visible_line)

            new_chart.series.append(new_series)
            ordonate_data = list(death_dict["total"]["total"])

            new_series = InstanciatedSeries(years, ordonate_data, "Number of death per year", "lines", visible_line)
        ...
        if "Number of climate death per year" in chart_list:
            ...
            min_value, max_value = self.get_greataxisrange(death_dict["climate"]["total"])
        ...
        if "Number of malnutrition death per year" in chart_list:
            ...
            min_value, max_value = self.get_greataxisrange(death_dict["diet"]["total"])
        ...
        if "Cumulative malnutrition deaths" in chart_list:
            ...
            max_values = {}
            for key in to_plot:
                min_values[key], max_values[key] = self.get_greataxisrange(death_dict["diet"][key])
            ...
            new_series = InstanciatedSeries(years, ordonate_data, f"cumulative malnutrition deaths", "bar")
        ...
        if "Life expectancy evolution" in chart_list:
            ...
            min_value, max_value = self.get_greataxisrange(life_expectancy_df["life_expectancy"])
            ...
            new_series = InstanciatedSeries(years, ordonate_data, "Life expectancy", "lines", visible_line)
        ...
        if "Population detailed" in chart_list:
            ...
            for year in years_list:
                ...
                new_chart = TwoAxesInstanciatedChart("age", " number of people", chart_name=chart_name)

                ordonate_data = list(pop_df.iloc[year - year_start, 1:-2])

                new_series = InstanciatedSeries(pop_column, ordonate_data, "", "bar")
...
def graph_model_cumulative_climate_deaths(death_dict, instanciated_charts):
    ...
    max_values = {}
    for key in to_plot:
        min_values[key], max_values[key] = ppt.get_greataxisrange(death_dict["climate"][key])
    ...
    new_series = InstanciatedSeries(years, ordonate_data, f"cumulative climatic deaths", "bar")
...
def graph_model_world_population(pop_df, instanciated_charts):
    ...
    min_value, max_value = ppt.get_greataxisrange(pop_df["total"])
...
def graph_model_world_pop_and_cumulative_deaths(pop_df, death_dict, instanciated_charts):
    ...
    new_chart = TwoAxesInstanciatedChart(
        GlossaryCore.Years, " Number of people", [year_start - 5, year_end + 5], [min_value, max_value], chart_name
    )

    visible_line = True
    ordonate_data = list(death_dict["climate"]["cum_total"])
    new_series = InstanciatedSeries(years, ordonate_data, f"Cumulative climate deaths", "bar")
    new_chart.series.append(new_series)
