...
class PopulationDiscipline(ClimateEcoDiscipline):
    ...
    DESC_IN = {
        ...
        'death_rate_param': {'type':'dataframe', 'default': default_death_rate_params_df, 'user_level': 3}, 
        'birth_rate_upper': {'type': 'float', 'default': 1.12545946e-01, 'user_level': 3},
        'birth_rate_lower' : {'type': 'float', 'default': 2.02192894e-02, 'user_level': 3}, #2.2e-2
        'birth_rate_delta' : {'type': 'float', 'default': 6.19058508e-04, 'user_level': 3},#1.92403581e-04
        'birth_rate_phi' : {'type': 'float', 'default': 4.03360000e+03, 'user_level': 3}, #4.03359157e+03
        'birth_rate_nu' : {'type': 'float', 'default': 1.75808789e-01, 'user_level': 3}, #3.93860555e-01
        'lower_knowledge' : {'type': 'float', 'default': 10, 'user_level': 3},
        'upper_knowledge': {'type': 'float', 'default': 100, 'user_level': 3},
        'delta_knowledge': {'type': 'float', 'default': 0.0293357, 'user_level': 3},
        'phi_knowledge': {'type': 'float', 'default': 149.7919, 'user_level': 3},
        'nu_knowledge': {'type': 'float', 'default': 1.144062855, 'user_level': 3},
        'constant_birthrate_know' : {'type': 'float', 'default': 1.99999838e-02, 'user_level': 3},
        'alpha_birthrate_know' : {'type': 'float', 'default': 1.02007061e-01, 'user_level': 3},
        'beta_birthrate_know' : {'type': 'float', 'default': 8.01923418e-01, 'user_level': 3},
        'share_know_birthrate' : {'type': 'float', 'default': 7.89207064e-01, 'user_level': 3}
        }  

    DESC_OUT = {
        'population_df': {'type': 'dataframe', 'visibility': 'Shared', 'namespace': 'ns_witness'},
        'population_detail_df': {'type': 'dataframe'},
        'birth_rate_df': {'type': 'dataframe'},
        'death_rate_dict': {'type': 'dict'},
        'death_dict': {'type': 'dict'},
        'birth_df': {'type': 'dataframe'},
        'life_expectancy_df': {'type': 'dataframe'}
        }
    ...
    def run(self):
        ...
        # model execution
        population_detail_df, birth_rate_df, death_rate_dict, birth_df, death_dict, life_expectancy_df = self.model.compute(in_dict)
        
        population_df = population_detail_df[['years' , 'total']]
        population_df = population_df.rename(columns={"total": "population"})

        #Convert population in million of people
        population_df['population'] = population_df['population'] / self.model.million
    ...
    def get_post_processing_list(self, chart_filters=None):
        ...
        if '15-49 age range birth rate' in chart_list:
            ...
            chart_name = '15-49 age range birth_rate'
        ...
        if 'Number of birth and death per year' in chart_list:
            ...
            min_values['number_of_birth'], max_values['number_of_birth'] = self.get_greataxisrange(birth_df['number_of_birth'])
            min_values['number_of_death'], max_values['number_of_death'] = self.get_greataxisrange(death_dict['total']['total']) 
        ...
        if 'Cumulative climate deaths' in chart_list:
            ...
            max_values = {}
            for key in to_plot:          
                min_values[key], max_values[key] = self.get_greataxisrange(death_dict['climate'][key])
