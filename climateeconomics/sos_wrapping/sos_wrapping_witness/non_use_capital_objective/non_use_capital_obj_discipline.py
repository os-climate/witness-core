...
class NonUseCapitalObjectiveDiscipline(SoSWrapp):
    ...
    DESC_IN = {
        ...
        "forest_lost_capital_cons_limit": {
            "type": "float",
            "unit": "G$",
            "default": 40.0,
            "user_level": 2,
    ...
    def setup_sos_disciplines(self):
                ...
                for energy in energy_list:
                    if energy == BiomassDry.name:
                        ...
                        if f"{energy}.{GlossaryCore.techno_list}" in self.get_data_in():
                            techno_list = self.get_sosdisc_inputs(f"{energy}.{GlossaryCore.techno_list}")
                            if techno_list is not None:
                                energy_techno_dict[energy] = {"namespace": "ns_energy", "value": techno_list}
            ...
            if ccs_list is not None:
                for ccs in ccs_list:
                    ...
                    if f"{ccs}.{GlossaryCore.techno_list}" in self.get_data_in():
                        techno_list = self.get_sosdisc_inputs(f"{ccs}.{GlossaryCore.techno_list}")
                        if techno_list is not None:
                            energy_techno_dict[ccs] = {"namespace": GlossaryCore.NS_CCS, "value": techno_list}

        if "agri_capital_techno_list" in self.get_data_in():
            agriculture_techno_list = self.get_sosdisc_inputs("agri_capital_techno_list")
            if agriculture_techno_list is not None:
                energy_techno_dict[GlossaryCore.SectorAgriculture] = {
                    "namespace": "ns_forest",
        ...
        for non_use_capital_tuple in all_non_use_capital_list:
            dynamic_inputs[f"{non_use_capital_tuple[0]}non_use_capital"] = {
                "type": "dataframe",
                "visibility": SoSWrapp.SHARED_VISIBILITY,
            ...
            dynamic_inputs[f"{non_use_capital_tuple[0]}techno_capital"] = {
                "type": "dataframe",
                "visibility": SoSWrapp.SHARED_VISIBILITY,
                "namespace": non_use_capital_tuple[1],
                "unit": "G$",
    ...
    def run(self):
        ...
        dict_values = {
            "non_use_capital_df": self.model.non_use_capital_df,
    ...
    def get_post_processing_list(self, chart_filters=None):
        ...
        if "Non Use Capitals" in chart_list:
            ...
            new_chart = TwoAxesInstanciatedChart(
                GlossaryCore.Years, "non_use Capital [G$]", chart_name=chart_name, stacked_bar=True
            )
            for industry in non_use_capital_df.columns:
                if (
                    industry not in [GlossaryCore.Years, "Sum of non use capital"]
                    and not (non_use_capital_df[industry] == 0.0).all()
                ):
        ...
        if "Energy Mix Total Capital" in chart_list:
            ...
            new_chart = TwoAxesInstanciatedChart(GlossaryCore.Years, "Total Capital [G$]", chart_name=chart_name)
        ...
        if "Forest Management Lost Capital" in chart_list:
            forest_lost_capital = self.get_sosdisc_inputs("forest_lost_capital")
            ...
            new_chart = TwoAxesInstanciatedChart(
                GlossaryCore.Years, "Total Capital [G$]", chart_name=chart_name, stacked_bar=True
            )
...
def compute_full_techno_list(energy_techno_dict):
    ...
    for energy, techno_dict in energy_techno_dict.items():
        if energy == GlossaryCore.SectorAgriculture:
            full_techno_list.extend([(f"", techno_dict["namespace"]) for techno in techno_dict["value"]])
        else:
