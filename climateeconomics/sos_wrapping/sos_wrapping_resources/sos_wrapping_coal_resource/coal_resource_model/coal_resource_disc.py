...
class CoalDiscipline(SoSDiscipline):
    ...
    DESC_OUT = {
        ...
        ResourceModel.PAST_PRODUCTION: {'type': 'dataframe', 'unit': 'million_tonnes', 'visibility': SoSDiscipline.SHARED_VISIBILITY, 'namespace': 'coal_resource'}
    }
    ...
    def run(self):
        ...
        coal_demand ['coal_resource'] = inputs_dict['All_Demand']['coal_resource']
        self.coal_model.compute(coal_demand,'coal_resource',2001)
    ...
    def compute_sos_jacobian(self):
        ...
        coal_resource = 'coal_resource'
        coal_demand = pd.DataFrame(
            {'years': inputs_dict['All_Demand']['years'].values})
        coal_demand['coal_resource'] = inputs_dict['All_Demand'][coal_resource]
            
        grad_stock, grad_price, grad_use = self.coal_model.get_derivative_resource(coal_resource)
        # # ------------------------------------------------
        # # Stock resource gradient
        for coal_type in output_dict['resource_stock']:
            self.set_partial_derivative_for_other_types(
                (ResourceModel.RESOURCE_STOCK, coal_type), (f'{ResourceModel.DEMAND}', 'coal_resource'), grad_stock[coal_type])
        # # # ------------------------------------------------
        # # # Price resource gradient
        self.set_partial_derivative_for_other_types(
                (ResourceModel.RESOURCE_PRICE, 'price'), (f'{ResourceModel.DEMAND}', 'coal_resource'), grad_price)
        # # # ------------------------------------------------
        # # # Use resource gradient
        for coal_type in output_dict['resource_stock']:
            self.set_partial_derivative_for_other_types(
                (ResourceModel.USE_STOCK, coal_type), (f'{ResourceModel.DEMAND}', 'coal_resource'), grad_use[coal_type])                
        # # ------------------------------------------------
