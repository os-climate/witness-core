...
class GasDiscipline(SoSDiscipline):
    ...
    DESC_OUT = {
        ...
        ResourceModel.PAST_PRODUCTION: {'type': 'dataframe', 'unit': 'million_tonnes',
                                        'visibility': SoSDiscipline.SHARED_VISIBILITY, 'namespace': 'natural_gas_resource'}
    }
    ...
    def run(self):
        ...
        gas_demand[self.resource_name] = inp_dict['All_Demand'][self.resource_name]
        self.gas_model.compute(gas_demand, self.resource_name, 2010)
    ...
    def compute_sos_jacobian(self):
        ...
        gas_resource = self.resource_name
        gas_demand = pd.DataFrame(
            {'years': inputs_dict['All_Demand']['years'].values})
        gas_demand['gas_resource'] = inputs_dict['All_Demand'][gas_resource]

        grad_stock, grad_price, grad_use = self.gas_model.get_derivative_resource(
            gas_resource)
        # # ------------------------------------------------
