...
class CopperDisc(SoSWrapp):
    ...
    DESC_OUT = {
        CopperModel.COPPER_RESERVE: {"type": "dataframe", "unit": "million_tonnes"},
    ...
    def run(self):
        ...
        dict_values = {
            CopperModel.COPPER_RESERVE: self.copper_model.copper_reserve,
    ...
    def get_post_processing_list(self, filters=None):
        ...
        if "all" in charts_list:
            period_of_exploitation = np.arange(
                self.DESC_IN[GlossaryCore.YearStart]["default"], self.DESC_IN[GlossaryCore.YearEnd]["default"] + 1, 1
            ).tolist()
            ...
            chart_production = TwoAxesInstanciatedChart("Years [y]", "Production [Mt]", chart_name="Copper Production")

            chart_stock = TwoAxesInstanciatedChart("Years [y]", "Stock [Mt]", chart_name="Copper Use")

            chart_price_evolution = TwoAxesInstanciatedChart(
                "Years [y]", "Price/t [USD/t]", chart_name="Copper Price Evolution"
            )

            chart_copper_situation = TwoAxesInstanciatedChart(
                "Years [y]", "Copper [Mt]", chart_name="Copper Repartition", stacked_bar=True
            )

            chart_ratio = TwoAxesInstanciatedChart(
                "Years [y]", "Ratio [-]", chart_name="Copper Ratio extraction/demand"
            )
            ...
            bar_cumulated_production_serie = InstanciatedSeries(
                period_of_exploitation,
                cumulated_production_list.tolist(),
                "Cumulated Copper Consumption",
                InstanciatedSeries.BAR_DISPLAY,
            )
            bar_stock_series = InstanciatedSeries(
                period_of_exploitation, stock_list.tolist(), "Copper Stock", InstanciatedSeries.BAR_DISPLAY
            )
            bar_reserve_series = InstanciatedSeries(
                period_of_exploitation, reserve_list.tolist(), "Copper Reserve", InstanciatedSeries.BAR_DISPLAY
            )
