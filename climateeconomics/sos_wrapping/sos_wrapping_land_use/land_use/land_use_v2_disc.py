...
class LandUseV2Discipline(SoSWrapp):
    ...
    DESC_IN = {
        ...
        LandUseV2.TOTAL_FOOD_LAND_SURFACE: {
            "type": "dataframe",
            "unit": "Gha",
            "visibility": SoSWrapp.SHARED_VISIBILITY,
    ...
    def run(self):
        ...
        outputs_dict = {
            ...
            LandUseV2.LAND_SURFACE_FOR_FOOD_DF: self.land_use_model.land_surface_df[["Food Surface (Gha)"]].rename(
                columns={"Food Surface (Gha)": "Agriculture total (Gha)"}
            ),
        }
    ...
    def compute_sos_jacobian(self):
        ...
        self.set_partial_derivative_for_other_types(
            (LandUseV2.LAND_DEMAND_CONSTRAINT,),
            (LandUseV2.FOREST_SURFACE_DF, "global_forest_surface"),
        ...
        for techno in agri_techno + forest_techno:
            self.set_partial_derivative_for_other_types(
                (LandUseV2.LAND_DEMAND_CONSTRAINT,),
                (LandUseV2.LAND_DEMAND_DF, techno),
                -np.identity(len(years)) / inputs_dict[LandUseV2.LAND_DEMAND_CONSTRAINT_REF],
            )
    ...
    def get_post_processing_list(self, chart_filters=None):
        ...
        habitable_land_from_forest_agri_2020 = (
            land_surface_detailed["Available Agriculture Surface (Gha)"][0]
        ...
        shrub_surface = np.maximum(
            ...
            - (
                ...
                + land_surface_detailed["Total Agriculture Surface (Gha)"]
            ).values,
        )
                ...
                if land_surface_detailed is not None and land_demand_constraint is not None:
                    ...
                    for column in list(land_surface_detailed.columns):
                        if column in ["Total Forest Surface (Gha)", "Total Agriculture Surface (Gha)"]:
                            ...
                            color = {
                                "Total Forest Surface (Gha)": qualitative.Dark2[4],
                    ...
                    fig.update_yaxes(
                        title_text="(-1) * Land Demand Constraint",
                        secondary_y=True,
                        color=qualitative.Set1[0],
                        range=[0, 1.1 * max(-land_demand_constraint)],
                    )
        ...
        if "Detailed Land Usage [Gha]" in chart_list:
            if land_surface_detailed is not None:
                ...
                for column in list(land_surface_detailed.columns):
                    if column not in [
                        GlossaryCore.Years,
                        "Total Forest Surface (Gha)",
                        "Total Agriculture Surface (Gha)",
                        "Available Agriculture Surface (Gha)",
                        "Available Forest Surface (Gha)",
                        "Available Shrub Surface (Gha)",
                    ]:
                        legend = column.replace(" (Gha)", "")
                        new_series = InstanciatedSeries(
                            years,
                            (land_surface_detailed[column]).values.tolist(),
                            legend,
                ...
                new_chart = TwoAxesInstanciatedChart(
                    GlossaryCore.Years,
                    "Detailed Land Usage [Gha]",
                    chart_name="Detailed Land Usage [Gha]",
                    stacked_bar=True,
                )
        ...
        if "Surface Type in 2020 [Gha]" in chart_list:
            ...
            if land_surface_detailed is not None:
                ...
                for year in years:
                    # data from the model
                    agriculture_land = land_surface_detailed.loc[land_surface_detailed[GlossaryCore.Years] == year][
                        "Total Agriculture Surface (Gha)"
                    ].values[
                        0
                    ]  # 5.1 Gha in 2020
                    forest_land = land_surface_detailed.loc[land_surface_detailed[GlossaryCore.Years] == year][
                        "Total Forest Surface (Gha)"
                    ].values[
                        0
                    ]  # 3.9 Gha in 2020
                    if agriculture_land + forest_land > available_forest_agri_shrub:
                        shrub = 0.0
                        agriculture_land = (
                            available_forest_agri_shrub * agriculture_land / (agriculture_land + forest_land)
                        )
                ...
                for i, val in enumerate(years):
                    step = dict(
                        method="update",
                        args=[{"visible": [False] * len(fig.data)}, {"title": f"{chart_name} for year {val}"}],
                        label=str(val),
                    )
