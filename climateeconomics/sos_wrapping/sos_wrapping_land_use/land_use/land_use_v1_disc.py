...
class LandUseV1Discipline(SoSDiscipline):
    ...
    DESC_IN = {'year_start': {'type': 'int', 'default': default_year_start, 'unit': '[-]', 'visibility': SoSDiscipline.SHARED_VISIBILITY, 'namespace': 'ns_public'},
               ...
               LandUseV1.DEFORESTED_SURFACE_DF: {'type': 'dataframe', 'unit': 'Gha', 'visibility': SoSDiscipline.SHARED_VISIBILITY, 'namespace': 'ns_witness'}
               }
    ...
    def compute_sos_jacobian(self):
        ...
        model.compute(land_demand_df, total_food_land_surface, deforested_surface_df)
    ...
    def get_post_processing_list(self, chart_filters=None):
        ...
        if LandUseV1Discipline.FOREST_CHARTS in chart_list:
            ## ------------------------------------------------------------
            ## FOREST USAGE -> Technologies that uses forest (ManagedWood, UnmanagedWood..)
        ...
        if LandUseV1Discipline.AGRICULTURE_CHARTS in chart_list:
            ## ------------------------------------------------------------
            ## AGRICULTURE USAGE -> Technologies that uses agriculture land (CropEnergy, SolarPV..)
            if demand_df is not None:
        ...
        if LandUseV1Discipline.GLOBAL_CHARTS in chart_list:
            ...
            ## ------------------
            ## Sunburst figure for global land use. Source https://ourworldindata.org/land-use
            for year in years_list:
                # Create figure
                fig = go.Figure(go.Sunburst(
                        labels=["Land", "Ocean", "Habitable land", "Glaciers", "Barren land", "Agriculture", "Forest", "Shrub", "Urban", "Fresh water"],
                        parents=["Earth", "Earth", "Land", "Land", "Land", "Habitable land", "Habitable land", "Habitable land", "Habitable land", "Habitable land"],
                        values=[14.9, 36.1, 10.5, 1.5, 2.8, 5.1, 3.9, 1.2, 0.15, 0.15],
                        marker=dict(colors = ["#CD912A", "#1456C5", "#DBBF6A", "#D3D3D0", "#E7C841", "#7CC873", "#1EA02F", "#5C8C56", "#B1B4AF", "#18CDFA"]),
                        branchvalues="total",
                        rotation=90,
                    )
                )
            ...
            series_to_add = []
            ## ------------------
            ## Agriculture + forest surface are 92.03 M km^2 => 9.203 Gha. Source https://ourworldindata.org/land-use
