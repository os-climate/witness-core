...
class MeauxMorerePibCD(BaseCalibBis):
    """
    A custom version of production function based on 
    """
    def __init__(self):
        super().__init__('meauxmorere_CD')
        self.logger.info("")
    ...
    def eval_all(self, x):
        ...
        #Compute pib (iterate by year for projection reason)
    ...
    def compute_estimated_pib(self, x, year, productivity_df, working_pop, capital_u):
        ...
        output = productivity * (capital_u**alpha * (working_pop)**(1-alpha))     
        return output
