...
resource_name = (
    CopperResourceModel.resource_name
)  # CopperResourceModel.resource_name  PlatinumResourceModel.resource_name
...
past_production = pd.read_csv(
    join(dirname(__file__), f"../core_resources/models/resources_data/{resource_name}_production_data.csv")
)
...
def compute_Hubbert_parameters(past_production, production_years, regression_start, resource_type):
    ...
    for i_year, pp_year in enumerate(past_production_years):
        ...
        cumulative_production.loc[i_year, resource_type] = Q + P
    ...
    cumulative_sample = cumulative_production.loc[cumulative_production[GlossaryCore.Years] >= regression_start]

    ratio_sample = ratio_P_by_Q.loc[ratio_P_by_Q[GlossaryCore.Years] >= regression_start]

    fit = np.polyfit(cumulative_sample[resource_type], ratio_sample[resource_type], 1)
