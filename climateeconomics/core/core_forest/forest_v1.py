...
class Forest:
    ...
    def compute(self, in_dict):
        ...
        self.forest_surface_df["deforested_surface"] = -self.deforestation_surface["deforested_surface"].values / 1000
        ...
        self.forest_surface_df["forest_surface_evol"] = (
            self.forest_surface_df["forested_surface"] + self.forest_surface_df["deforested_surface"]
        )
        ...
        self.forest_surface_df["forested_surface_cumulative"] = np.cumsum(self.forest_surface_df["forested_surface"])

        # check limit of deforestation
        for element in range(0, len(years)):
            if (
                self.forest_surface_df.loc[element, "forest_surface_evol_cumulative"]
                < -self.limit_deforestation_surface / 1000
            ):
                self.forest_surface_df.loc[element, "forest_surface_evol"] = 0
                self.forest_surface_df.loc[element, "deforested_surface"] = -self.forest_surface_df.loc[
                    element, "forested_surface"
                ]
                self.forest_surface_df.loc[element, "forest_surface_evol_cumulative"] = (
                    -self.limit_deforestation_surface / 1000
                )
                self.forest_surface_df.loc[element, "deforested_surface_cumulative"] = (
                    -self.forest_surface_df.loc[element, "forested_surface_cumulative"]
                    - self.limit_deforestation_surface / 1000
                )

        self.CO2_emitted_df[GlossaryCore.Years] = self.years
        # in Gt of CO2
        self.CO2_emitted_df["emitted_CO2_evol"] = (
            -self.forest_surface_df["forest_surface_evol"] * self.CO2_per_ha / 1000
        )
        self.CO2_emitted_df["emitted_CO2"] = -self.forest_surface_df["deforested_surface"] * self.CO2_per_ha / 1000
        self.CO2_emitted_df["captured_CO2"] = -self.forest_surface_df["forested_surface"] * self.CO2_per_ha / 1000

        self.CO2_emitted_df["emitted_CO2_evol_cumulative"] = (
            -self.forest_surface_df["forest_surface_evol_cumulative"] * self.CO2_per_ha / 1000 + self.initial_emissions
        )
        self.CO2_emitted_df["emitted_CO2_cumulative"] = (
            -self.forest_surface_df["deforested_surface_cumulative"] * self.CO2_per_ha / 1000 + self.initial_emissions
        )
        self.CO2_emitted_df["captured_CO2_cumulative"] = (
            -self.forest_surface_df["forested_surface_cumulative"] * self.CO2_per_ha / 1000
        )
        # To make forest_disc work with forest_model v1 & v2
        self.forest_surface_df["global_forest_surface"] = np.zeros(len(years))

    # Gradients
    def d_deforestation_surface_d_deforestation_surface(
        self,
    ):
        """
        ...
        for i in range(0, number_of_values):
            if (
                self.forest_surface_df.loc[i, "forest_surface_evol_cumulative"]
                != -self.limit_deforestation_surface / 1000
            ):
    ...
    def d_forestation_surface_d_invest(
        self,
    ):
        """
        Compute gradient of deforestation surface by deforestation_surface (design variable)
