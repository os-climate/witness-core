...
class Forest:
    ...
    def set_data(self):
        ...
        self.managed_wood_invest_before_year_start = self.param[self.MW_INVEST_BEFORE_YEAR_START]
        ...
        self.mean_density = self.wood_density * (1.0 - self.residues_density_percentage) + self.residues_density * (
            1.0 - self.residues_density_percentage
        )
    ...
    def compute(self, in_dict):
        ...
        Computation methods
        """
        self.biomass_dry_calorific_value = BiomassDry.data_energy_dict["calorific_value"]  # kwh/kg
        # kwh/kg
        self.biomass_dry_high_calorific_value = BiomassDry.data_energy_dict["high_calorific_value"]
        ...
        self.forest_surface_df["forest_constraint_evolution"] = (
            self.forest_surface_df["reforestation_surface"] + self.forest_surface_df["deforestation_surface"]
        )
        ...
        ) * self.biomass_dry_calorific_value + self.managed_wood_df[
        ...
        if "CO2_taxes_factory" in self.biomass_dry_df:
            self.techno_prices["Forest_wotaxes"] = (
                self.biomass_dry_df["price_per_MWh"] - self.biomass_dry_df["CO2_taxes_factory"]
            )
        else:
        ...
        # CO2 consumed
        self.techno_consumption[f"{CO2.name} ({self.mass_unit})"] = (
            -self.techno_wood_info["CO2_from_production"]
            / self.biomass_dry_high_calorific_value
            * self.techno_production[f"{BiomassDry.name} ({BiomassDry.unit})"]
        )

        self.techno_consumption_woratio[f"{CO2.name} ({self.mass_unit})"] = (
            -self.techno_wood_info["CO2_from_production"]
            / self.biomass_dry_high_calorific_value
            * self.techno_production[f"{BiomassDry.name} ({BiomassDry.unit})"]
        )
    ...
    def compute_managed_wood_surface(self):
        ...
        mw_from_past_invest = self.managed_wood_invest_before_year_start[GlossaryCore.InvestmentsValue] / mw_cost
    ...
    def compute_managed_wood_production(self):
        ...
        cubic_meter_production = (
            self.managed_wood_initial_surface * self.actual_yield
            + (self.managed_wood_df["cumulative_surface"] - self.managed_wood_initial_surface) * self.managed_yield
        )
        self.managed_wood_df["delta_wood_production (Mt)"] = (
            self.managed_wood_df["delta_surface"]
            * self.managed_yield
            * (1 - self.residues_density_percentage)
            * self.wood_density
        )
        # Gm3* kg/m3 => Mt
        self.managed_wood_df["wood_production (Mt)"] = (
            cubic_meter_production * (1 - self.residues_density_percentage) * self.wood_density
        )

        self.managed_wood_df["wood_production_for_energy (Mt)"] = (
            self.managed_wood_df["wood_production (Mt)"] * self.wood_percentage_for_energy
        )
        ...
        self.managed_wood_df["delta_residues_production (Mt)"] = (
            self.managed_wood_df["delta_surface"]
            * self.managed_yield
            * self.residues_density_percentage
            * self.residues_density
        )
        self.managed_wood_df["residues_production (Mt)"] = (
            cubic_meter_production * self.residues_density_percentage * self.residues_density
        )
        self.managed_wood_df["residues_production_for_energy (Mt)"] = (
            self.managed_wood_df["residues_production (Mt)"] * self.wood_percentage_for_energy
        )
        self.managed_wood_df["residues_production_for_industry (Mt)"] = self.managed_wood_df[
            "residues_production (Mt)"
        ] * (1 - self.wood_percentage_for_energy)

        self.managed_wood_df["delta_biomass_production (Mt)"] = (
            self.managed_wood_df["delta_wood_production (Mt)"] + self.managed_wood_df["delta_residues_production (Mt)"]
        )
        self.managed_wood_df["biomass_production (Mt)"] = (
            self.managed_wood_df["wood_production (Mt)"] + self.managed_wood_df["residues_production (Mt)"]
        )

        # CO2 part
        self.managed_wood_df["delta_CO2_emitted"] = -self.managed_wood_df["delta_surface"] * self.CO2_per_ha / 1000
        # CO2 emitted is delta cumulate
        self.managed_wood_df["CO2_emitted"] = (
            -(self.managed_wood_df["cumulative_surface"] - self.managed_wood_initial_surface) * self.CO2_per_ha / 1000
        )
    ...
    def compute_reforestation_deforestation_surface(self):
        ...
        self.forest_surface_df["delta_deforestation_surface"] = (
            -self.deforest_invest[GlossaryCore.InvestmentsValue].values / self.deforest_cost_per_ha
        )

        # forested surface
        # invest in G$, coest_per_ha in $/ha --> Gha
        self.forest_surface_df["delta_reforestation_surface"] = (
            self.forest_investment["forest_investment"].values / self.cost_per_ha
        )
        ...
        for i in range(0, len(self.years)):
            ...
            if i == 0:
                self.forest_surface_df.loc[i, "unmanaged_forest"] = (
                    self.initial_unmanaged_forest_surface
                    + self.forest_surface_df.loc[i, "delta_reforestation_surface"]
                    + self.forest_surface_df.loc[i, "delta_deforestation_surface"]
                )
            else:
                self.forest_surface_df.loc[i, "unmanaged_forest"] = (
                    self.forest_surface_df.loc[i - 1, "unmanaged_forest"]
                    + self.forest_surface_df.loc[i, "delta_reforestation_surface"]
                    + self.forest_surface_df.loc[i, "delta_deforestation_surface"]
                )
            ...
            if self.forest_surface_df.loc[i, "unmanaged_forest"] <= 0:
                ...
                self.forest_lost_capital.loc[i, "reforestation"] = (
                    -self.forest_surface_df.loc[i, "delta_deforestation_surface"] * self.cost_per_ha
                )
            # recompute managed forest cumulated each year
            if i > 0:
                self.managed_wood_df.loc[i, "cumulative_surface"] = (
                    self.managed_wood_df.loc[i - 1, "cumulative_surface"] + self.managed_wood_df.loc[i, "delta_surface"]
                )
            ...
            if self.managed_wood_df.loc[i, "cumulative_surface"] <= 0:
                ...
                self.forest_lost_capital.loc[i, "deforestation"] = (
                    -self.managed_wood_df.loc[i, "cumulative_surface"] * self.deforest_cost_per_ha
                )
                ...
                self.forest_lost_capital.loc[i, "managed_wood"] = (
                    -deforested_managed_surface * self.techno_wood_info["managed_wood_price_per_ha"]
                )
    ...
    def compute_deforestation_biomass(self):
        ...
        self.biomass_dry_df["deforestation (Mt)"] = (
            -self.forest_surface_df["delta_deforestation_surface"] * self.unmanaged_yield * self.wood_density
        )
        self.biomass_dry_df["deforestation_for_energy"] = (
            self.biomass_dry_df["deforestation (Mt)"] * self.wood_percentage_for_energy
        )
        self.biomass_dry_df["deforestation_for_industry"] = (
            self.biomass_dry_df["deforestation (Mt)"] - self.biomass_dry_df["deforestation_for_energy"]
        )
        self.biomass_dry_df["deforestation_price_per_ton"] = (
            self.unmanaged_yield * self.wood_density / self.deforest_cost_per_ha
        )
        self.biomass_dry_df["deforestation_price_per_MWh"] = (
            self.biomass_dry_df["deforestation_price_per_ton"] / self.biomass_dry_calorific_value
        )
    ...
    def sumup_global_surface_data(self):
        ...
        """
        self.forest_surface_df["delta_global_forest_surface"] = (
            self.forest_surface_df["delta_reforestation_surface"]
            + self.forest_surface_df["delta_deforestation_surface"]
        )
        self.forest_surface_df["global_forest_surface"] = (
    ...
    def compute_global_CO2_production(self):
        ...
        self.CO2_emitted_df["delta_CO2_emitted"] = (
            -self.forest_surface_df["delta_global_forest_surface"] * self.CO2_per_ha / 1000
        )
        self.CO2_emitted_df["delta_CO2_deforestation"] = (
            -self.forest_surface_df["delta_deforestation_surface"] * self.CO2_per_ha / 1000
        )
        self.CO2_emitted_df["delta_CO2_reforestation"] = (
            -self.forest_surface_df["delta_reforestation_surface"] * self.CO2_per_ha / 1000
        )

        # remove CO2 managed surface from global emission because CO2_per_ha
        # from managed forest = 0
        self.CO2_emitted_df["CO2_deforestation"] = (
            -self.forest_surface_df["deforestation_surface"] * self.CO2_per_ha / 1000
        )
        self.CO2_emitted_df["CO2_reforestation"] = (
            -self.forest_surface_df["reforestation_surface"] * self.CO2_per_ha / 1000
        )
        self.CO2_emitted_df["initial_CO2_land_use_change"] = self.initial_emissions
        # global sum up
        self.CO2_emitted_df["global_CO2_emitted"] = (
            self.CO2_emitted_df["CO2_deforestation"] + self.CO2_emitted_df["initial_CO2_land_use_change"]
        )
        self.CO2_emitted_df["global_CO2_captured"] = self.CO2_emitted_df["CO2_reforestation"]
        self.CO2_emitted_df["emitted_CO2_evol_cumulative"] = (
            self.CO2_emitted_df["global_CO2_emitted"] + self.CO2_emitted_df["global_CO2_captured"]
        )
    ...
    def compute_biomass_dry_production(self):
        ...
        self.biomass_dry_df["biomass_dry_for_energy (Mt)"] = (
            self.managed_wood_df["wood_production_for_energy (Mt)"]
            + self.managed_wood_df["residues_production_for_energy (Mt)"]
            + self.biomass_dry_df["deforestation_for_energy"]
        )
        ...
        self.biomass_dry_df["price_per_ton"] = (
            self.biomass_dry_df["managed_wood_price_per_ton"] * self.managed_wood_part
            + self.biomass_dry_df["deforestation_price_per_ton"] * self.deforestation_part
        )

        self.biomass_dry_df["managed_wood_price_per_MWh"] = (
            self.biomass_dry_df["managed_wood_price_per_ton"] / self.biomass_dry_calorific_value
        )
        self.biomass_dry_df["price_per_MWh"] = self.biomass_dry_df["price_per_ton"] / self.biomass_dry_calorific_value
    ...
    def compute_price(self):
        ...
        self.biomass_dry_df[f"managed_wood_capex ($/t)"] = (
            self.techno_wood_info[f"managed_wood_price_per_ha"]
            * (self.crf + 0.045)
            / self.managed_yield
            / self.mean_density
            * 1000
        ...
        self.biomass_dry_df[f"managed_wood_price_per_ton"] = (
            (self.biomass_dry_df[f"managed_wood_capex ($/t)"] + self.biomass_dry_df[f"managed_wood_transport ($/t)"])
            * self.margin["margin"]
            / 100.0
        )
    ...
    def compute_crf(self):
        ...
        crf = (wacc * (1.0 + wacc) ** 100) / ((1.0 + wacc) ** 100 - 1.0)
    ...
    def compute_carbon_emissions(self):
        ...
        if "CO2_from_production" not in self.techno_wood_info:
            self.CO2_emissions["production"] = self.get_theoretical_co2_prod(unit="kg/kWh")
            ...
            if self.techno_wood_info["CO2_from_production_unit"] == "kg/kg":
                self.CO2_emissions["production"] = (
                    self.techno_wood_info["CO2_from_production"] / self.biomass_dry_high_calorific_value
                )
        ...
        self.CO2_emissions["Forest"] = self.CO2_emissions["production"] + co2_emissions_frominput_energies
    ...
    def compute_d_deforestation_surface_d_invest(self):
        ...
        d_deforestation_surface_d_forests = -np.identity(number_of_values) / self.deforest_cost_per_ha
    ...
    def compute_d_reforestation_surface_d_invest(self):
        ...
        """
        number_of_values = self.year_end - self.year_start + 1
        d_forestation_surface_d_invest = np.identity(number_of_values) / self.cost_per_ha
    ...
    def compute_d_mw_surface_d_invest(self):
        ...
        for i in range(construction_delay, number_of_values):
            result[i, i - construction_delay] = 1 / self.techno_wood_info["managed_wood_price_per_ha"]
        return result
    ...
    def compute_d_limit_surfaces_d_deforestation_invest(self, d_deforestation_surface_d_invest):
        ...
        d_delta_mw_surface_d_invest = np.zeros((number_of_values, number_of_values))
        d_delta_deforestation_surface_d_invest = d_deforestation_surface_d_invest
        d_cum_umw_surface_d_invest = np.zeros((number_of_values, number_of_values))
        d_lc_deforestation_d_invest = np.zeros((number_of_values, number_of_values))
        d_lc_reforestation_d_invest = np.zeros((number_of_values, number_of_values))
        d_lc_mw_d_invest = np.zeros((number_of_values, number_of_values))

        for i in range(0, len(self.years)):
            if i == 0:
                d_cum_umw_surface_d_invest[i] = d_delta_deforestation_surface_d_invest[i]
            else:
                d_cum_umw_surface_d_invest[i] = (
                    d_cum_umw_surface_d_invest[i - 1] + d_delta_deforestation_surface_d_invest[i]
                )
            # if unmanaged forest are empty, managed forest are removed
            if self.forest_surface_df.loc[i, "unmanaged_forest"] <= 1e-10:
                ...
                d_lc_mw_d_invest[i] = (
                    -d_cum_umw_surface_d_invest[i] * self.techno_wood_info["managed_wood_price_per_ha"]
                )
                # set unmanaged forest to 0
            ...
            if self.managed_wood_df.loc[i, "cumulative_surface"] <= 1e-10:
                ...
                d_lc_mw_d_invest[i] = (
                    -(d_deforestation_surface_d_invest[i] + d_lc_reforestation_d_invest[i] / self.cost_per_ha - sum[i])
                    * self.techno_wood_info["managed_wood_price_per_ha"]
                )
        ...
        return (
            d_cum_umw_surface_d_invest,
            d_delta_mw_surface_d_invest,
            d_delta_deforestation_surface_d_invest,
            d_lc_deforestation_d_invest,
            d_lc_reforestation_d_invest,
            d_lc_mw_d_invest,
        )
    ...
    def compute_d_limit_surfaces_d_reforestation_invest(self, d_reforestation_surface_d_invest):
        ...
        d_delta_mw_surface_d_invest = np.zeros((number_of_values, number_of_values))
        d_delta_deforestation_surface_d_invest = np.zeros((number_of_values, number_of_values))
        ...
        for i in range(0, len(self.years)):
            if i == 0:
                d_cum_umw_surface_d_invest[i] = (
                    d_delta_reforestation_surface_d_invest[i] + d_delta_deforestation_surface_d_invest[i]
                )
            else:
                d_cum_umw_surface_d_invest[i] = (
            ...
            if self.forest_surface_df.loc[i, "unmanaged_forest"] <= 1e-10:
                ...
                if i == 0:
                    ...
                    d_lc_reforestation_d_invest[i] = (
                        d_cum_umw_surface_d_invest[i - 1] + d_reforestation_surface_d_invest[i]
                    ) * self.cost_per_ha
                d_lc_mw_d_invest[i] = (
                    -d_cum_umw_surface_d_invest[i] * self.techno_wood_info["managed_wood_price_per_ha"]
                )
            ...
            if self.managed_wood_df.loc[i, "cumulative_surface"] <= 1e-10:
                ...
                d_lc_deforestation_d_invest[i] = -sum[i] * self.deforest_cost_per_ha
                d_lc_mw_d_invest[i] = (
                    -(d_lc_reforestation_d_invest[i] / self.cost_per_ha - sum[i])
                    * self.techno_wood_info["managed_wood_price_per_ha"]
                )

                d_delta_mw_surface_d_invest[i] = -sum[i - 1]
                d_delta_deforestation_surface_d_invest[i] = (
                    -d_reforestation_surface_d_invest[i] + d_delta_mw_surface_d_invest[i]
                )
    ...
    def compute_d_techno_prod_d_invest(self, d_delta_mw_d_invest, d_delta_deforestation_d_invest):
        ...
        d_techno_prod_d_invest = (
            d_mw_prod_wood_for_nrj + d_deforestation_prod_for_nrj
        ) * self.biomass_dry_calorific_value + d_mw_prod_residue_for_nrj * self.techno_wood_info[
            "residue_calorific_value"
        ]
    ...
    def compute_d_techno_price_d_invest(self, d_delta_mw_d_invest, d_delta_deforestation_d_invest):
        ...
        d_mw_prod_wood_for_nrj = self.d_cum(d_delta_mw_d_invest * self.wood_density * self.managed_yield) * (
            1 - self.residues_density_percentage
        )
        d_mw_prod_residue_for_nrj = (
            self.d_cum(d_delta_mw_d_invest * self.residues_density * self.managed_yield)
            * self.residues_density_percentage
        )
        ...
        v = self.managed_wood_df["biomass_production (Mt)"].values + self.biomass_dry_df["deforestation (Mt)"].values
        v_prime = (d_mw_prod + d_deforestation_prod).T
        v_square = v * v
        u = self.managed_wood_df["biomass_production (Mt)"].values
        u_prime = d_mw_prod.T
        d_mw_price_per_ton = (
            self.biomass_dry_df["managed_wood_price_per_ton"].values * (u_prime * v - v_prime * u) / v_square
        )
        ...
        d_deforestation_price_per_ton = (
            self.biomass_dry_df["deforestation_price_per_ton"].values * (u_prime * v - v_prime * u) / v_square
        )
