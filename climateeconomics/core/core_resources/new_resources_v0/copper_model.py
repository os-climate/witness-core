...
class CopperModel:
    ...
    def __init__(self, param):
        ...
        self.copper_stock = pd.DataFrame(
            columns=["Year", "Stock"]
        )  # pd.read_csv(join(data_dir, 'copper_previous_stock.csv'))
        self.copper_reserve = pd.DataFrame(
            columns=["Year", "Reserve"]
        )  # DataFrame with the wolrd's reserve updated each year
        self.copper_prod_price = pd.DataFrame(
            columns=["Year", "Price/t", "Total Price"]
        )  # DataFrame with prod price updated each year
        self.copper_prod = pd.DataFrame(
            columns=["Year", "Extraction", "World Production", "Cumulated World Production", "Ratio"]
        )
    ...
    def compute_copper_stock_and_production(self, year):
        ...
        if year == self.year_start:
            ...
            self.copper_prod.at[year, "Cumulated World Production"] = (
                self.copper_prod.at[year - 1, "Cumulated World Production"]
                + self.copper_prod.at[year, "World Production"]
            )
    ...
    def compute_copper_price(self, year):
        ...
        self.copper_prod_price.loc[year, "Total Price"] = (
            self.copper_prod.loc[year, "World Production"] * self.copper_prod_price.loc[year, "Price/t"] * 1000
        )  # conversion Mt
