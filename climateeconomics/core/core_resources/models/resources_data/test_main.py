...
copper = pd.read_csv(
    join(dirname(__file__), f"../resources_data/{copper_name}_consumed_data.csv")
)  # pd.read_csv(join(dirname(__file__),'copper_resource_consumed_data.csv')) ou : pd.DataFrame(columns= [GlossaryCore.Years , 'copper_consumption' ])
...
use_stock = pd.DataFrame({GlossaryCore.Years: np.insert(years, 0, np.arange(year_start - lifespan, year_start, 1))})
