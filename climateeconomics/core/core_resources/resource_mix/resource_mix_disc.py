...
class ResourceMixDiscipline(SoSWrapp):
    ...
    for resource in ResourceMixModel.RESOURCE_LIST:
        ratio_available_resource_default[resource] = np.linspace(1.0, 1.0, len(ratio_available_resource_default.index))
    default_conversion_dict = {
        ...
        OilResourceDiscipline.resource_name: {"price": 7.33, "production": 1.0, "stock": 1.0, "global_demand": 1.0},
        ...
        PlatinumResourceDiscipline.resource_name: {"price": 1.0, "production": 1.0, "stock": 1.0, "global_demand": 1.0},
    }
    ...
    DESC_IN = {
        ...
        ResourceMixModel.NON_MODELED_RESOURCE_PRICE: {
            "type": "dataframe",
            "unit": "$/t",
            "namespace": "ns_resource",
        ...
        "conversion_dict": {
            ...
            "unit": "[-]",
            "default": default_conversion_dict,
            "visibility": SoSWrapp.SHARED_VISIBILITY,
            "namespace": "ns_resource",
        },
    }
    ...
    DESC_OUT = {
        ResourceMixModel.ALL_RESOURCE_STOCK: {"type": "dataframe", "unit": "million_tonnes"},
        ...
        ResourceMixModel.RATIO_USABLE_DEMAND: {
            "type": "dataframe",
            "default": ratio_available_resource_default,
            "visibility": SoSWrapp.SHARED_VISIBILITY,
            "unit": "%",
            "namespace": "ns_resource",
        },
        ...
        if "resource_list" in self.get_data_in():
            ...
            for resource in resource_list:
                dynamic_inputs[f"{resource}.resource_price"] = {
                    "type": "dataframe",
                    "unit": ResourceMixModel.RESOURCE_PRICE_UNIT[resource],
                    "dataframe_descriptor": {GlossaryCore.Years: ("float", None, True), "price": ("float", None, True)},
                }
                dynamic_inputs[f"{resource}.resource_stock"] = {
                    "type": "dataframe",
                    "unit": ResourceMixModel.RESOURCE_STOCK_UNIT[resource],
                    "dataframe_descriptor": {
                        GlossaryCore.Years: ("float", None, True),
                ...
                dynamic_inputs[f"{resource}.predictable_production"] = {
                    "type": "dataframe",
                    "unit": ResourceMixModel.RESOURCE_PROD_UNIT[resource],
                    "dataframe_descriptor": {
                        GlossaryCore.Years: ("float", None, True),
                        "heavy": ("float", None, True),
                        "uranium_40": ("float", None, True),
    ...
    def compute_sos_jacobian(self):
        ...
        for resource_type in resource_list:
            grad_price, grad_use, grad_stock, grad_recycling, grad_demand = (
                self.all_resource_model.get_derivative_all_resource(inputs_dict, resource_type)
            )
            grad_ratio_on_stock, grad_ratio_on_demand, grad_ratio_on_recycling = (
                self.all_resource_model.get_derivative_ratio(inputs_dict, resource_type, output_dict)
            )

            for types in inputs_dict[f"{resource_type}.use_stock"]:
                if types != GlossaryCore.Years:
                    self.set_partial_derivative_for_other_types(
                        (ResourceMixModel.All_RESOURCE_USE, resource_type),
                        (f"{resource_type}.use_stock", types),
                        grad_use,
                    )

            self.set_partial_derivative_for_other_types(
                (ResourceMixModel.RATIO_USABLE_DEMAND, resource_type),
                ("resources_demand_woratio", resource_type),
                grad_ratio_on_demand,
            )
            ...
            for types in inputs_dict[f"{resource_type}.resource_stock"]:
                if types != GlossaryCore.Years:
                    self.set_partial_derivative_for_other_types(
                        ...
                        (f"{resource_type}.resource_stock", types),
                        grad_stock,
                    )
                    self.set_partial_derivative_for_other_types(
                        (ResourceMixModel.RATIO_USABLE_DEMAND, resource_type),
                        (f"{resource_type}.resource_stock", types),
                        grad_ratio_on_stock * grad_stock,
                    )
            ...
            for types in inputs_dict[f"{resource_type}.recycled_production"]:
                if types != GlossaryCore.Years:
                    self.set_partial_derivative_for_other_types(
                        ...
                        (f"{resource_type}.recycled_production", types),
                        grad_recycling,
                    )
                    self.set_partial_derivative_for_other_types(
                        (ResourceMixModel.RATIO_USABLE_DEMAND, resource_type),
                        (f"{resource_type}.recycled_production", types),
                        grad_ratio_on_recycling * grad_recycling,
                    )

            self.set_partial_derivative_for_other_types(
                (ResourceMixModel.ALL_RESOURCE_DEMAND, resource_type), ("resources_demand", resource_type), grad_demand
            )

            self.set_partial_derivative_for_other_types(
                (GlossaryCore.ResourcesPriceValue, resource_type),
                (f"{resource_type}.resource_price", "price"),
                grad_price,
            )
    ...
    def get_post_processing_list(self, chart_filters=None):
        ...
        if "all" in chart_list:
            ...
            stock_df = self.get_sosdisc_outputs(ResourceMixModel.ALL_RESOURCE_STOCK)
            years = stock_df[GlossaryCore.Years].values.tolist()
            price_df = self.get_sosdisc_outputs(GlossaryCore.ResourcesPriceValue)
            use_stock_df = self.get_sosdisc_outputs(ResourceMixModel.All_RESOURCE_USE)
            production_df = self.get_sosdisc_outputs(ResourceMixModel.ALL_RESOURCE_PRODUCTION)
            ratio_use_df = self.get_sosdisc_outputs(ResourceMixModel.RATIO_USABLE_DEMAND)
            demand_df = self.get_sosdisc_outputs(ResourceMixModel.ALL_RESOURCE_DEMAND)
            recycling_df = self.get_sosdisc_outputs(ResourceMixModel.ALL_RESOURCE_RECYCLED_PRODUCTION)

            # two charts for stock evolution and price evolution
            stock_chart = TwoAxesInstanciatedChart(
                GlossaryCore.Years, "stocks (Mt)", chart_name="Resources stocks through the years", stacked_bar=True
            )
            price_chart = TwoAxesInstanciatedChart(
                GlossaryCore.Years, "price ($/t)", chart_name="Resource price through the years", stacked_bar=True
            )
            use_stock_chart = TwoAxesInstanciatedChart(
                GlossaryCore.Years, "resource use (Mt) ", chart_name="Resource use through the years", stacked_bar=True
            )
            production_chart = TwoAxesInstanciatedChart(
                GlossaryCore.Years,
            ...
            ratio_use_demand_chart = TwoAxesInstanciatedChart(
                GlossaryCore.Years,
                "ratio usable stock / demand ",
                chart_name="ratio usable stock and prod on demand through the years",
                stacked_bar=True,
            )
            ...
            recycling_chart = TwoAxesInstanciatedChart(
                GlossaryCore.Years,
                "recycled production (Mt)",
                chart_name="recycled production through the years",
                stacked_bar=True,
            )
            for resource_kind in stock_df:
                if resource_kind != GlossaryCore.Years:
                    stock_serie = InstanciatedSeries(
                        years,
                        (stock_df[resource_kind]).values.tolist(),
                        resource_kind,
                    ...
                    production_serie = InstanciatedSeries(
                        years,
                        (production_df[resource_kind]).values.tolist(),
                        resource_kind,
                    ...
                    use_stock_serie = InstanciatedSeries(
                        years,
                        (use_stock_df[resource_kind]).values.tolist(),
                        resource_kind,
                        InstanciatedSeries.BAR_DISPLAY,
                    )
                    ...
                    ratio_use_serie = InstanciatedSeries(
                        years,
                        (ratio_use_df[resource_kind]).values.tolist(),
                        resource_kind,
                        InstanciatedSeries.LINES_DISPLAY,
                    )
                    ...
                    demand_serie = InstanciatedSeries(
                        years,
                        (demand_df[resource_kind]).values.tolist(),
                        resource_kind,
                        InstanciatedSeries.LINES_DISPLAY,
                    )
                    ...
                    recycled_production_serie = InstanciatedSeries(
                        years,
                        (recycling_df[resource_kind]).values.tolist(),
                        resource_kind,
                        InstanciatedSeries.BAR_DISPLAY,
                    )
            ...
            for resource_types in price_df:
                if resource_types != GlossaryCore.Years:
                    price_serie = InstanciatedSeries(
                        years,
                        (price_df[resource_types]).values.tolist(),
                        resource_types,
                        InstanciatedSeries.LINES_DISPLAY,
                    )
