...
class ResourceMixModel:
    ...
    RESOURCE_PROD_UNIT = {disc.resource_name: disc.prod_unit for disc in RESOURCE_DISC_LIST}

    RESOURCE_STOCK_UNIT = {disc.resource_name: disc.stock_unit for disc in RESOURCE_DISC_LIST}

    RESOURCE_PRICE_UNIT = {disc.resource_name: disc.price_unit for disc in RESOURCE_DISC_LIST}

    NON_MODELED_RESOURCE_LIST = list(
        set([resource["name"] for resource in ResourceGlossary.GlossaryDict.values()]).symmetric_difference(
            set(RESOURCE_LIST)
        )
    )
    ...
    def configure_parameters_update(self, inputs_dict):
        ...
        self.resources_demand = inputs_dict["resources_demand"]
        self.resources_demand = self.resources_demand.loc[self.resources_demand[GlossaryCore.Years] >= self.year_start]
        self.resources_demand = self.resources_demand.loc[self.resources_demand[GlossaryCore.Years] <= self.year_end]
        self.resources_demand_woratio = inputs_dict["resources_demand_woratio"]
        self.resources_demand_woratio = self.resources_demand_woratio.loc[
            self.resources_demand_woratio[GlossaryCore.Years] >= self.year_start
        ]
        self.resources_demand_woratio = self.resources_demand_woratio.loc[
            self.resources_demand_woratio[GlossaryCore.Years] <= self.year_end
        ]
        self.resource_list = inputs_dict["resource_list"]
        self.init_dataframes()
        ...
        for resource in self.resource_list:
            ...
            for types in data_frame_use:
                if types != GlossaryCore.Years:
                    self.all_resource_production[resource] = (
                        self.all_resource_production[resource] + data_frame_production[types]
                    )
                    ...
                    self.all_resource_recycled_production[resource] = (
                        self.all_resource_recycled_production[resource] + data_frame_recycled_production[types]
                    )
            # conversion in Mt of the different resource:
            self.all_resource_production[resource] = (
                self.all_resource_production[resource] * self.conversion_dict[resource]["production"]
            )
            self.all_resource_stock[resource] = (
                self.all_resource_stock[resource] * self.conversion_dict[resource]["stock"]
            )
            self.all_resource_use[resource] = self.all_resource_use[resource] * self.conversion_dict[resource]["stock"]
            self.all_resource_recycled_production[resource] = (
                self.all_resource_recycled_production[resource] * self.conversion_dict[resource]["stock"]
            )
            self.all_resource_price[resource] = (
                self.all_resource_price[resource] * self.conversion_dict[resource]["price"]
            )
            self.resource_demand[resource] = (
                self.resource_demand[resource] * self.conversion_dict[resource]["global_demand"]
            )
    ...
    def compute(self, inputs_dict):
        ...
        data_frame_other_resource_price = deepcopy(inputs_dict["non_modeled_resource_price"])
        ...
        self.all_resource_co2_emissions = self.get_co2_emissions(
            self.years, self.resource_list, list(ResourceMixModel.NON_MODELED_RESOURCE_LIST)
        )
    ...
    def compute_ratio(self):
        ...
        for resource in self.resource_list:
            # Available resources
            available_resource = (
                deepcopy(self.all_resource_stock[resource].values)
                + deepcopy(self.all_resource_production[resource].values)
                + deepcopy(self.all_resource_recycled_production[resource].values)
            )
            available_resource_limited = compute_func_with_exp_min(np.array(available_resource), 1.0e-10)
            ...
            demand_limited = compute_func_with_exp_min(np.array(demand_woratio), 1.0e-10)
            self.all_resource_ratio_usable_demand[resource] = (
                np.minimum(np.maximum(available_resource_limited / demand_limited, 1e-15), 1.0) * 100.0
            )
    ...
    def get_derivative_all_resource(self, inputs_dict, resource_type):
        ...
        grad_stock = self.conversion_dict[resource_type]["stock"] * np.identity(
            len(inputs_dict[f"{resource_type}.resource_stock"].index)
        )
        grad_use = self.conversion_dict[resource_type]["stock"] * np.identity(
            len(inputs_dict[f"{resource_type}.use_stock"].index)
        )
        grad_price = self.conversion_dict[resource_type]["price"] * np.identity(
            len(inputs_dict[f"{resource_type}.resource_price"].index)
        )
        grad_recycling = self.conversion_dict[resource_type]["stock"] * np.identity(
            len(inputs_dict[f"{resource_type}.recycled_production"].index)
        )
        grad_demand = self.conversion_dict[resource_type]["global_demand"] * np.identity(
    ...
    def get_derivative_ratio(self, inputs_dict, resource_type, output_dict):
        ...
        available_resource_limited = compute_func_with_exp_min(np.array(available_resource), 1.0e-10)
        d_available_resource_limited = compute_dfunc_with_exp_min(np.array(available_resource), 1.0e-10)
        demand = (
            inputs_dict["resources_demand_woratio"][resource_type]
            * self.conversion_dict[resource_type]["global_demand"]
        )
        demand_limited = compute_func_with_exp_min(demand.values, 1.0e-10)
        d_demand_limited = compute_dfunc_with_exp_min(demand.values, 1.0e-10)

        # If prod < cons, set the identity element for the given year to
        # the corresponding value
        d_ratio_d_stock = (
            np.identity(len(inputs_dict["resources_demand_woratio"].index))
            * 100.0
            * np.where(
                (available_resource_limited <= demand_limited) * (available_resource_limited / demand_limited > 1e-15),
    ...
    def get_co2_emissions(self, years, resource_list, non_modeled_resource_list):
        ...
        for resource in non_modeled_resource_list + resource_list:
            if resource in self.CO2_emissions_dict.keys():
                resources_CO2_emissions[resource] = np.ones(len(years)) * self.CO2_emissions_dict[resource]
            else:
                resources_CO2_emissions[resource] = np.zeros(len(years))
