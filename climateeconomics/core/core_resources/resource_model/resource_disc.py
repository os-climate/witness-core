...
class ResourceDiscipline(SoSWrapp):
    ...
    def compute_sos_jacobian(self):
        ...
        for sub_resource_type in sub_resource_list:
            self.set_partial_derivative_for_other_types(
                ("resource_stock", sub_resource_type),
                ("resources_demand", self.resource_name),
        ...
        for sub_resource_type in sub_resource_list:
            self.set_partial_derivative_for_other_types(
                ("use_stock", sub_resource_type), ("resources_demand", self.resource_name), grad_use[sub_resource_type]
            )
        # # ------------------------------------------------
        ...
        for sub_resource_type in sub_resource_list:
            self.set_partial_derivative_for_other_types(
                ...
                ("resources_demand", self.resource_name),
                grad_recycling[sub_resource_type],
            )
    ...
    def get_post_processing_list(self, chart_filters=None):
        ...
        if "Production" in chart_list:
            ...
            sub_resource_list = [col for col in list(production_df.columns) if col != GlossaryCore.Years]
        ...
        if "Recycling" in chart_list and number_of_subtypes < 2:
            ...
            recycling_charts = self.get_recycling_charts(recycling_df, use_stock_df)
    ...
    def get_stock_charts(self, stock_df, use_stock_df):
        ...
        stock_chart = TwoAxesInstanciatedChart(
            "Years",
            f"maximum stocks [{self.stock_unit}]",
        ...
        if len(sub_resource_list) > 1:
            use_stock_chart = TwoAxesInstanciatedChart(
                "Years",
                f"{self.resource_name} use [{self.stock_unit}]",
                chart_name=f"{self.resource_name} use per subtypes through the years",
                stacked_bar=True,
            )
        use_stock_cumulated_chart = TwoAxesInstanciatedChart(
            "Years",
        ...
        for sub_resource_type in sub_resource_list:
            stock_serie = InstanciatedSeries(
                list(stock_df[GlossaryCore.Years]),
                (stock_df[sub_resource_type]).values.tolist(),
                sub_resource_type,
            ...
            use_stock_serie = InstanciatedSeries(
                list(use_stock_df[GlossaryCore.Years]),
                (use_stock_df[sub_resource_type]).values.tolist(),
                sub_resource_type,
    ...
    def get_price_charts(self, price_df):
        price_chart = TwoAxesInstanciatedChart(
            "Years",
            f"price [{self.price_unit}]",
            chart_name=f"{self.resource_name} price through the years",
            stacked_bar=True,
        )
        price_serie = InstanciatedSeries(
            list(price_df[GlossaryCore.Years]),
            (price_df["price"]).values.tolist(),
            f"{self.resource_name} price",
            InstanciatedSeries.LINES_DISPLAY,
        )
    ...
    def get_production_charts(self, production_df, past_production_df, year_start, production_start):
        sub_resource_list = [col for col in production_df.columns if col != GlossaryCore.Years]

        past_production_cut = past_production_df.loc[past_production_df[GlossaryCore.Years] >= production_start]
        production_cut = production_df.loc[production_df[GlossaryCore.Years] <= year_start]
        if len(sub_resource_list) > 1:
            production_chart = TwoAxesInstanciatedChart(
                "Years",
                f"{self.resource_name} production per subtypes [{self.prod_unit}]",
                chart_name=f"{self.resource_name} production per subtypes through the years",
                stacked_bar=True,
            )
        production_cumulated_chart = TwoAxesInstanciatedChart(
            "Years",
            f"{self.resource_name} production [{self.prod_unit}]",
            chart_name=f"{self.resource_name} production through the years",
            stacked_bar=True,
        )

        model_production_cumulated_chart = TwoAxesInstanciatedChart(
            "Years",
            f"Comparison between pyworld3 and real {self.resource_name} production [{self.prod_unit}]",
            chart_name=f"{self.resource_name} production through the years",
            stacked_bar=True,
        )
        past_production_chart = TwoAxesInstanciatedChart(
            "Years",
        ...
        for sub_resource_type in sub_resource_list:
            production_serie = InstanciatedSeries(
                list(production_df[GlossaryCore.Years]),
                (production_df[sub_resource_type]).values.tolist(),
                sub_resource_type,
                InstanciatedSeries.BAR_DISPLAY,
            )
            if len(sub_resource_list) > 1:
            ...
            production_cut_series = InstanciatedSeries(
                list(production_df[GlossaryCore.Years]),
                (production_cut[sub_resource_type]).values.tolist(),
                sub_resource_type + " predicted production",
                InstanciatedSeries.BAR_DISPLAY,
            )
            past_production_series = InstanciatedSeries(
                list(past_production_df[GlossaryCore.Years]),
                (past_production_df[sub_resource_type]).values.tolist(),
                sub_resource_type,
                InstanciatedSeries.LINES_DISPLAY,
            )
            past_production_cut_series = InstanciatedSeries(
                list(production_df[GlossaryCore.Years]),
                (past_production_cut[sub_resource_type]).values.tolist(),
        ...
        list_of_charts = [past_production_chart, model_production_cumulated_chart, production_cumulated_chart]
        if len(sub_resource_list) > 1:
    ...
    def get_recycling_charts(self, recycling_df, use_stock_df):
        recycling_chart = TwoAxesInstanciatedChart(
            "Years",
            f"{self.resource_name} recycling and used stock [{self.stock_unit}]",
            chart_name=f"{self.resource_name} recycled quantity compared to used quantity through the years",
            stacked_bar=False,
        )

        sub_resource_list = [col for col in recycling_df.columns if col != GlossaryCore.Years]
        for sub_resource_type in sub_resource_list:
            recycling_serie = InstanciatedSeries(
                list(recycling_df[GlossaryCore.Years]),
                (recycling_df[sub_resource_type]).values.tolist(),
                f"{self.resource_name} recycled quantity",
                InstanciatedSeries.LINES_DISPLAY,
            )
            used_stock_serie = InstanciatedSeries(
                list(use_stock_df[GlossaryCore.Years]),
                use_stock_df[sub_resource_type].values.tolist(),
                f"{self.resource_name} extracted quantity",
                InstanciatedSeries.LINES_DISPLAY,
            )
