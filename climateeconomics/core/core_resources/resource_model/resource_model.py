...
class ResourceModel:
    ...
    def configure_parameters(self, inputs_dict):
        ...
        self.sub_resource_list = [
            col for col in list(self.resource_production_data.columns) if col != GlossaryCore.Years
        ]
    ...
    def init_dataframes(self):
        ...
        self.use_stock = pd.DataFrame(
            {
                GlossaryCore.Years: np.insert(
                    self.years, 0, np.arange(self.year_start - self.lifespan, self.year_start, 1)
                )
            }
        )
    ...
    def compute_stock(self):
        ...
        for resource_type in self.sub_resource_list:
            self.resource_stock[resource_type] = np.insert(np.zeros(len(self.years) - 1), 0, self.stock_start)
            # Concat what was consumed in the past years and what will be
            # consumed (needed for recycled materials calculations)
            self.use_stock[resource_type] = np.insert(
                np.zeros(len(self.years) - 1), 0, self.resource_consumed_data[f"{resource_type}_consumption"]
            )
            self.recycled_production[resource_type] = np.insert(
                np.zeros(len(self.years) - 1),
                0,
        ...
        for year_demand in self.years[1:]:
            ...
            for resource_type in ascending_price_resource_list:
                total_stock = total_stock + predictable_production_dict[resource_type][year_demand]

            # chek if the stock is not empty this year:
            if total_stock > 0:
                for resource_type in ascending_price_resource_list:
                    # compute recycled quantity of the different resources
                    self.compute_recycling(year_demand, resource_type, recycled_production_dict, use_stock_dict)
                    ...
                    if demand.real > 0:
                        ...
                        # resource we have and we don't answer all the demand
                            ...
                            demand = demand - use_stock_dict[resource_type][year_demand]
        ...
        self.recycled_production = pd.DataFrame.from_dict(recycled_production_dict)
    ...
    def compute_price(self):
        ...
        for resource_type in self.sub_resource_list:
            self.total_consumption["production"] = self.use_stock[resource_type] + self.total_consumption["production"]
        # Turn dataframes into dict of np array for faster computation time
    ...
    def get_derivative_resource(self):
        ...
        for year_demand in range(year_start + 1, year_end + 1):
            total_stock = 0
            grad_recycling = self.compute_derivative_recycling(
                year_demand, resource_type, grad_use, grad_stock, grad_recycling
            )
            # # ------------------------------------------------
            # # dealing with units
            demand = resource_demand_dict[self.resource_name][year_demand]
                ...
                for resource_type in ascending_price_resource_list:
                    if demand > 0:
                        if (
                            ...
                            - demand
                            >= 0
                            and resource_stock_dict[resource_type][year_demand] > 0
                        ):
                            ...
                            for year in range(year_start + 1, year_demand + 1):
                                ...
                                if (
                                    resource_stock_dict[resource_type][year] > 0
                                    and resource_demand_dict[self.resource_name][year] != 0
                                    and year > no_stock_year[resource_type]
                                ):
                                ...
                                if self.lifespan != 0:
                                    if (
                                        year_demand - self.lifespan > self.year_start
                                        and year < year_demand - self.lifespan
                                    ):
                                ...
                                # and if we stored resource type without demand
                                    ...
                                    resource_demand_dict[self.resource_name][year] != 0
                                    and year in year_stock[resource_type]
                                ):
                                    grad_use[resource_type][year_demand - year_start, year - year_start] = grad_demand

                            grad_stock[resource_type][year_demand - year_start] += grad_recycling[resource_type][
                                year_demand - year_start
                            ]
                            demand = 0
                            grad_demand = 0
                            year_stock[resource_type] = []
                            ...
                            grad_use[resource_type][
                                year_demand - year_start, year_demand - self.lifespan - year_start
                            ] += grad_recycling[resource_type][
                                year_demand - year_start, year_demand - self.lifespan - year_start
                            ]
                            ...
                            if resource_stock_dict[resource_type][year_demand - 1] > 0:
