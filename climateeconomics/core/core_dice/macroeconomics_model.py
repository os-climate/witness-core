...
class MacroEconomics:
    ...
    def set_data(self):
        ...
        self.saving_rate = pd.Series([self.param["saving_rate"]] * self.nb_per, index=self.years_range)
    ...
    def compute_population(self, year):
        ...
        """
        p_population = self.economics_df.loc[year - self.time_step, GlossaryCore.PopulationValue]
        population = p_population * (self.popasym / p_population) ** self.population_growth
    ...
    def compute_productivity_growthrate(self, year):
        ...
        productivity_gr = self.productivity_gr_start * np.exp(-self.decline_rate_tfp * 5 * (t - 1))
    ...
    def compute_productivity(self, year):
        ...
        p_productivity = self.economics_df.loc[year - self.time_step, GlossaryCore.Productivity]
    ...
    def compute_capital(self, year):
        ...
        if year == self.year_end:
            ...
            capital_a = capital * (1 - self.depreciation_capital) ** self.time_step + self.time_step * investment
            # Lower bound for capital
            self.economics_df.loc[year + self.time_step, GlossaryCore.Capital] = max(capital_a, self.lo_capital)
            return capital_a
    ...
    def compute_gross_output(self, year):
        ...
        gross_output = (
            productivity * capital**self.output_elasticity * (population / 1000) ** (1 - self.output_elasticity)
        )
    ...
    def compute_consumption_pc(self, year):
        ...
        self.economics_df.loc[year, GlossaryCore.PerCapitaConsumption] = max(consumption_pc, self.lo_conso)
    ...
    def compute_interest_rate(self, year):
        ...
        if year == self.year_end:
            ...
            consumption = self.economics_df.loc[year, GlossaryCore.Consumption]
            consumption_a = self.economics_df.loc[year + self.time_step, GlossaryCore.Consumption]
            interest_rate = (1 + self.init_rate_time_pref) * (consumption_a / consumption) ** (
                self.conso_elasticity / self.time_step
            ) - 1
    ...
    def compute(self, inputs, damage_prod=False):
        ...
        self.inputs = inputs
        self.abatecost = self.inputs["abatecost"].reindex(self.years_range)
        self.damefrac = self.inputs[GlossaryCore.DamageFractionOutput].reindex(self.years_range)
