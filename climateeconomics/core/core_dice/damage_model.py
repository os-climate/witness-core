...
class DamageModel:
    ...
    def compute_base_carbon_price(self, year):
        ...
        base_carbon_price = self.init_base_carbonprice * (1 + self.gr_base_carbonprice) ** (self.time_step * (t - 1))
    ...
    def compute_backstop_price(self, year):
        ...
        """
        t = ((year - self.year_start) / self.time_step) + 1
        backstop_price = self.cost_backtsop * (1 - self.init_cost_bacsktop) ** (t - 1)
    ...
    def compute_abatecost(self, year):
        ...
        abatecost = gross_output * adj_backstop_cost * emission_control_rate**self.exp_cont_f
    ...
    def compute_marg_abatecost(self, year):
        ...
        marg_abatecost = backstop_price * emissions_control_rate ** (self.exp_cont_f - 1)
    ...
    def compute_carbon_price(self, year):
        ...
        backstop_price = self.damage_df.loc[year, "backstop_price"]
        emissions_control_rate = self.emissions_control_rate[year]
        carbon_price = backstop_price * emissions_control_rate ** (self.exp_cont_f - 1)
        self.damage_df.loc[year, "carbon_price"] = carbon_price
        return carbon_price

    def compute(self, economics_df, emissions_df, temperature_df, emissions_control_rate):
