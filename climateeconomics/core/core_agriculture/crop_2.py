...
class Crop:
    ...
    def init_dataframes(self):
        ...
        dataframe_to_init = [
            GlossaryCore.FoodTypeWasteAtProductionName,
    ...
    def compute(self, inputs: dict):
        ...
        for food_type in inputs[GlossaryCore.FoodTypesName]:
            output_food_type = self.compute_food_type(
                ...
                workforce_food=self.inputs[GlossaryCore.WorkforceDfValue][GlossaryCore.Population1570].values,
                ...
                food_type_capex=self.inputs[GlossaryCore.FoodTypeCapexName][food_type].values,
                food_type_energy_need=self.inputs[GlossaryCore.FoodTypeEnergyNeedName][food_type].values,
                food_type_workforce_need=self.inputs[GlossaryCore.FoodTypeWorkforceNeedName][food_type].values,
                ...
                co2_emissions_per_prod_unit=self.inputs[GlossaryCore.FoodTypeEmissionsByProdUnitName.format(GlossaryCore.CO2)][food_type].values,
                ch4_emissions_per_prod_unit=self.inputs[GlossaryCore.FoodTypeEmissionsByProdUnitName.format(GlossaryCore.CH4)][food_type].values,
                n2o_emissions_per_prod_unit=self.inputs[GlossaryCore.FoodTypeEmissionsByProdUnitName.format(GlossaryCore.N2O)][food_type].values,
                ...
                kcal_per_prod_unit=self.inputs[GlossaryCore.FoodTypeKcalByProdUnitName][food_type].values,
                land_use_by_prod_unit=self.inputs[GlossaryCore.FoodTypeLandUseByProdUnitName][food_type].values,
    ...
    def compute_food_type(
        ...
        invest_food_type = invest_food * share_invest_food_type / 100. # G$
        energy_allocated_to_food_type = energy_consumption_food * share_energy_consumption_food_type / 100.
        workforce_allocated_to_food_type = workforce_food * share_workforce_food_type / 100.

        production_wo_ratio = invest_food_type / food_type_capex / 1000  # G$ / ($/ton) * 1000 = 10^9 $ / ($/ton) / 1000 = 10^6 t = Mt
        ...
        return {
            ...
            GlossaryCore.FoodTypeWasteAtProductionName: food_waste_before_distribution,
    ...
    @staticmethod
    def compute_productivity_loss_temp_change(temp_change: np.ndarray) -> np.ndarray:
        """compute the loss of productivity due to climate change, in percent of loss"""
        return 0
