...
class Crop:
    ...
    def set_data(self):
        ...
        if GlossaryCore.ConstructionDelay in self.techno_infos_dict:
            ...
            print(f"The construction_delay data is not set for Crop : default = 3 years  ")
    ...
    def create_dataframe(self):
        ...
        self.column_dict = {
            ...
            "fish (Gha)": GlossaryCore.Fish,
            "other (Gha)": GlossaryCore.OtherFood,
    ...
    def configure_parameters_update(self, inputs_dict):
        ...
        if inputs_dict["margin"] is not None:
            self.margin = inputs_dict["margin"].loc[inputs_dict["margin"][GlossaryCore.Years] <= self.year_end]
        ...
        self.crop_investment[GlossaryCore.InvestmentsValue] = (
            self.crop_investment[GlossaryCore.InvestmentsValue] * self.scaling_factor_crop_investment
        )
        if self.initial_age_distrib["distrib"].sum() > 100.001 or self.initial_age_distrib["distrib"].sum() < 99.999:
            sum_distrib = self.initial_age_distrib["distrib"].sum()
            raise Exception(f"The distribution sum is not equal to 100 % : {sum_distrib}")
    ...
    def compute_quantity_of_food(self):
        ...
        for key in self.updated_diet_df.keys():
            if key == GlossaryCore.Years:
                ...
                result[key] = (
                    self.population_df[GlossaryCore.PopulationValue].values * self.updated_diet_df[key].values * 1e6
                )
    ...
    def compute_surface_usage_by_food_type_wo_climate_impact(self):
        ...
        for key in quantity_of_food_df.keys():
            if key != GlossaryCore.Years:
                result[key + " (Gha)"] = self.kg_to_m2_dict[key] * quantity_of_food_df[key]
        ...
        for key in list_variables:
            ...
            if key == "fruits and vegetables" or key == "cereals" or key == "rice and maize":
                ...
                proportion = diet_init_kcal[key].values[0] / (
                    diet_init_kcal["fruits and vegetables"].values[0]
                    + diet_init_kcal["cereals"].values[0]
    ...
    def compute_surface_usage_by_food_type(self):
        ...
        surface_usage_by_food_type_with_climate_impact = self.food_surface_df_without_climate_change[
            non_years_columns
        ].multiply(other=(1 - pdctivity_reduction), axis=0)
    ...
    def compute_price(self):
        ...
        invest_inputs = self.crop_investment.loc[
            self.crop_investment[GlossaryCore.Years] <= self.cost_details[GlossaryCore.Years].max()
        ...
        len_y = max(self.cost_details[GlossaryCore.Years]) + 1 - min(self.cost_details[GlossaryCore.Years])
        self.cost_details["Crop_factory_amort"] = (
            np.tril(np.triu(np.ones((len_y, len_y)), k=0), k=self.nb_years_amort_capex - 1).transpose()
            * np.array(self.cost_details["Factory ($/MWh)"].values / self.nb_years_amort_capex)
        ).T.sum(axis=0)
        # pylint: enable=no-member

        # Compute and add transport
        self.cost_details["Transport ($/MWh)"] = (
            self.transport_cost["transport"]
            * self.transport_margin["margin"]
            / 100.0
            / self.data_fuel_dict["calorific_value"]
        )

        # Crop amort
        self.cost_details["Crop amort ($/MWh)"] = (
            self.cost_details["Crop_factory_amort"]
            + self.cost_details["Transport ($/MWh)"]
        ...
        # Total cost (MWh)
        self.cost_details["Total ($/MWh)"] = (
            self.cost_details["Energy costs ($/MWh)"]
            + self.cost_details["Factory ($/MWh)"]
            + self.cost_details["Transport ($/MWh)"]
        )

        # Add margin in %
        self.cost_details["Total ($/MWh)"] *= (
            self.margin.loc[self.margin[GlossaryCore.Years] <= self.cost_details[GlossaryCore.Years].max()][
                "margin"
            ].values
            / 100.0
        )
        ...
        if "CO2_taxes_factory" in self.cost_details:
            self.techno_prices["Crop_wotaxes"] = (
                self.cost_details["Total ($/MWh)"] - self.cost_details["CO2_taxes_factory"]
            )
        else:
        ...
        price_residue = price_crop * self.techno_infos_dict["crop_residue_price_percent_dif"]
    ...
    def check_capex_unity(self, data_tocheck):
        ...
        if data_tocheck["Capex_init_unit"] == "euro/ha":
            ...
            raise Exception(f"The CAPEX unity {capex_unit} is not handled yet in techno_type")
    ...
    def compute_primary_energy_production(self):
        ...
        self.production = (
            pd.merge(self.production, age_distrib_prod_sum, how="left", on=GlossaryCore.Years)
            .rename(columns={"distrib_prod (TWh)": "biomass_dry (TWh)"})
            .fillna(0.0)
        )
    ...
    def compute_aging_distribution_production(self):
        ...
        aging_distrib_year_df = pd.DataFrame({"age": self.initial_age_distrib["age"].values})
        aging_distrib_year_df[f"distrib_prod (TWh)"] = (
            self.initial_age_distrib["distrib"] * self.initial_production / 100.0
        )
        # Calculating yearly production based on investment
        production_from_invest = self.compute_prod_from_invest(construction_delay=self.construction_delay)
        ...
        range_years = np.arange(self.year_start, self.year_end + len_years)

        year_array = np.concatenate(tuple(range_years[i : i + len_years] for i in range(len_years)))
        age_array = np.concatenate(tuple(np.ones(len_years) * (len_years - i) for i in range(len_years, 0, -1)))
        prod_array = production_from_invest["prod_from_invest"].values.tolist() * len_years
        ...
        year_array = np.array([[year] * len(aging_distrib_year_df) for year in self.years]).flatten()
        age_values = aging_distrib_year_df["age"].values
        age_array = np.concatenate(tuple(age_values + i for i in range(len_years)))
        prod_array = aging_distrib_year_df[f"distrib_prod (TWh)"].values.tolist() * len_years

        old_prod_aged = pd.DataFrame(
            {GlossaryCore.Years: year_array, "age": age_array, f"distrib_prod (TWh)": prod_array}
        )

        # Concatenating the two created DataFrames
        self.age_distrib_prod_df = pd.concat([new_prod_aged, old_prod_aged], ignore_index=True)
        ...
        self.age_distrib_prod_df = self.age_distrib_prod_df.loc[
            ...
            (self.age_distrib_prod_df["age"] < self.techno_infos_dict["lifetime"])
    ...
    def compute_prod_from_invest(self, construction_delay):
        ...
        prod_before_ystart = pd.DataFrame(
            {
                ...
                "Capex ($/MWh)": self.cost_details.loc[
                    self.cost_details[GlossaryCore.Years] == self.year_start, "Capex ($/MWh)"
                ].values[0],
            }
        )
        ...
        production_from_invest["prod_from_invest"] = (
            production_from_invest[GlossaryCore.InvestmentsValue].values
        ...
        production_from_invest = production_from_invest[production_from_invest[GlossaryCore.Years] <= self.year_end]
    ...
    def compute_residue_from_food(self):
        ...
        residue_production = (
            self.total_food_land_surface["total surface (Gha)"]
            * self.techno_infos_dict["residue_density_percentage"]
    ...
    def compute_crop_for_energy_land_use(self):
        ...
        """
        self.land_use_required["Crop (Gha)"] = (
            self.mix_detailed_production["Crop for Energy (TWh)"] / self.techno_infos_dict["density_per_ha"]
        )
    ...
    def compute_carbon_emissions(self):
        ...
        if "CO2_from_production" not in self.techno_infos_dict:
            ...
            if self.techno_infos_dict["CO2_from_production_unit"] == "kg/kg":
                self.CO2_emissions["production"] = (
                    self.techno_infos_dict["CO2_from_production"] / self.data_fuel_dict["high_calorific_value"]
                )
        ...
        self.CO2_emissions["Crop"] = self.CO2_emissions["production"] + co2_emissions_frominput_energies
    ...
    def compute_land_emissions(self):
        ...
        for food in self.co2_emissions_per_kg:
            ...
            if self.kg_to_m2_dict[food] > 0.0:
                # CO2
                self.CO2_land_emissions_detailed[f"{food} (Gt)"] = (
                    self.co2_emissions_per_kg[food] / self.kg_to_m2_dict[food] * surface * self.m2toha * 1e9 * 1e-12
                )  # to m^2 and then to GtCo2
                # CH4
                self.CH4_land_emissions_detailed[f"{food} (Gt)"] = (
                    self.ch4_emissions_per_kg[food] / self.kg_to_m2_dict[food] * surface * self.m2toha * 1e9 * 1e-12
                )  # to m^2 and then to GtCo2
                # N20
                self.N2O_land_emissions_detailed[f"{food} (Gt)"] = (
                    self.n2o_emissions_per_kg[food] / self.kg_to_m2_dict[food] * surface * self.m2toha * 1e9 * 1e-12
                )  # to m^2 and then to GtCo2
            elif food == GlossaryCore.Fish:
                # CO2
                self.CO2_land_emissions_detailed[f"{food} (Gt)"] = (
                    self.co2_emissions_per_kg[food]
                    * self.updated_diet_df[food]
                    * self.population_df[GlossaryCore.PopulationValue].values
                    * 1e6
                    / 1e12
                )  # pop in millions, emissions in Gt
                # CH4
                self.CH4_land_emissions_detailed[f"{food} (Gt)"] = (
                    self.ch4_emissions_per_kg[food]
                    * self.updated_diet_df[food]
                    * self.population_df[GlossaryCore.PopulationValue].values
                    * 1e6
                    / 1e12
                ...
                # N20
                self.N2O_land_emissions_detailed[f"{food} (Gt)"] = (
                    self.n2o_emissions_per_kg[food]
                    * self.updated_diet_df[food]
                    * self.population_df[GlossaryCore.PopulationValue].values
                    * 1e6
                    / 1e12
                ...
                self.CO2_land_emissions_detailed[f"{food} (Gt)"] = 0.0
                self.CH4_land_emissions_detailed[f"{food} (Gt)"] = 0.0
                self.N2O_land_emissions_detailed[f"{food} (Gt)"] = 0.0
    ...
    def d_ghg_fish_emission_d_population(self):
        ...
        """
        dghg_dpop_co2 = np.diag(
            self.co2_emissions_per_kg[GlossaryCore.Fish] * self.updated_diet_df[GlossaryCore.Fish] * 1e6 / 1e12
        )
        dghg_dpop_ch4 = np.diag(
            self.ch4_emissions_per_kg[GlossaryCore.Fish] * self.updated_diet_df[GlossaryCore.Fish] * 1e6 / 1e12
        )
        dghg_dpop_n2o = np.diag(
            self.n2o_emissions_per_kg[GlossaryCore.Fish] * self.updated_diet_df[GlossaryCore.Fish] * 1e6 / 1e12
        )

        return {
            GlossaryCore.CO2: dghg_dpop_co2,
    ...
    def d_ghg_fish_emission_d_fish_kcal(self):
        ...
        dco2_dkcal_fish = np.diag(
            self.co2_emissions_per_kg[GlossaryCore.Fish]
            * 365
            / self.kg_to_kcal_dict[GlossaryCore.Fish]
            * self.population_df[GlossaryCore.PopulationValue].values
            * 1e6
            / 1e12
        )
        dch4_dkcal_fish = np.diag(
            self.ch4_emissions_per_kg[GlossaryCore.Fish]
            * 365
            / self.kg_to_kcal_dict[GlossaryCore.Fish]
            * self.population_df[GlossaryCore.PopulationValue].values
            * 1e6
        ...
        dn2o_dkcal_fish = np.diag(
            self.n2o_emissions_per_kg[GlossaryCore.Fish]
            * 365
            / self.kg_to_kcal_dict[GlossaryCore.Fish]
            * self.population_df[GlossaryCore.PopulationValue].values
            * 1e6
        ...
        return {
            GlossaryCore.CO2: dco2_dkcal_fish,
    ...
    def d_surface_d_vegetables_carbs_calories_per_day(self, population_df):
        ...
        for veg in vegetables_column_names:
            proportion = diet_df[veg].values[0] / (
                diet_df["fruits and vegetables"].values[0]
            ...
            total_surface_grad = (
                grad_value * population_df[GlossaryCore.PopulationValue].values * 1e6 * self.ha_to_m2 / 1e9
            )
    ...
    def d_surface_d_eggs_milk_calories_per_day(self, population_df):
        ...
        for veg in eggs_milk_column_names:
            proportion = diet_df[veg].values[0] / (diet_df["eggs"].values[0] + diet_df["milk"].values[0])
    ...
    def d_surface_d_calories(self, population_df, key_name):
        ...
        total_surface_grad = (
            red_meat_diet_grad * population_df[GlossaryCore.PopulationValue].values * 1e6 * self.ha_to_m2 / 1e9
        )
        total_surface_climate_grad = total_surface_grad * (1 - self.productivity_evolution["productivity_evolution"])
    ...
    def d_surface_d_white_meat(self, population_df):
        ...
        total_surface_grad = (
            sub_total_surface_grad * population_df[GlossaryCore.PopulationValue].values * 1e6 * self.ha_to_m2 / 1e9
        )
        total_surface_climate_grad = total_surface_grad * (1 - self.productivity_evolution["productivity_evolution"])
    ...
    def d_surface_d_milk_eggs_calories_percentage(self, population_df, veg):
        ...
        proportion = diet_df[veg].values[0] / (diet_df["milk"].values[0] + diet_df["eggs"].values[0])
    ...
    def compute_dprod_from_dinvest(self):
        ...
        arr_type = "float64"
        dprod_list_dinvest_list = np.zeros((nb_years, nb_years), dtype=arr_type)
        ...
        for i in range(nb_years):
            ...
            # 0,0,0]
            dpprod_dpinvest = 1 / self.cost_details[f"Capex ($/MWh)"].values[i] / self.data_fuel_dict["calorific_value"]
            is_invest_negative = max(
                np.sign(self.cost_details[GlossaryCore.InvestmentsValue].values[i] + np.finfo(float).eps), 0.0
            )
            dprod_list_dinvest_list[:, i] = np.hstack(
                (
                    np.zeros(first_len_zeros),
                    np.ones(len_non_zeros) * dpprod_dpinvest * is_invest_negative,
    ...
    def compute_dland_emissions_dfood_land_surface_df(self, food):
        ...
        if self.kg_to_m2_dict[food] > 0.0:
            ...
            co2 = self.co2_emissions_per_kg[food] * 0.0
            ch4 = self.ch4_emissions_per_kg[food] * 0.0
            n2O = self.n2o_emissions_per_kg[food] * 0.0
        return {
            GlossaryCore.CO2: co2,
    ...
    def compute_calories_per_day_constraint(self):
        self.calories_per_day_constraint = (
            self.calories_pc_df["kcal_pc"].values - self.constaint_calories_limit
        ) / self.constraint_calories_ref

    def compute_calories_per_capita(self):
        non_wasted_share = 1 - self.food_waste_percentage_df[GlossaryCore.FoodWastePercentageValue].values / 100.0
        self.produced_kcal = (
            self.red_meat_calories_per_day
    ...
    def compute_organic_waste(self):
        waste_share = self.food_waste_percentage_df[GlossaryCore.FoodWastePercentageValue] / 100.0
        self.organic_waste_df = pd.DataFrame({GlossaryCore.Years: self.years, "kcal": self.produced_kcal * waste_share})
    ...
    def compute_total_food_land_surface(self):
        ...
        self.food_land_surface_percentage_df = self.convert_surface_to_percentage(self.food_land_surface_df)

    def compute_mix_detailed_production(self):
        self.mix_detailed_production["Crop residues (TWh)"] = (
            self.residue_prod_from_food_surface.values
            + self.techno_infos_dict["residue_density_percentage"] * self.production["biomass_dry (TWh)"]
        )
        ...
        self.mix_detailed_production["Total (TWh)"] = (
            self.mix_detailed_production["Crop for Energy (TWh)"] + self.mix_detailed_production["Crop residues (TWh)"]
        )
    ...
    def rescale_production_and_consumption(self):
        ...
        self.techno_production = self.techno_production.rename(columns={"Total (TWh)": "biomass_dry (TWh)"})
    ...
    def compute_consumed_calories_per_capita_breakdown(self):
        ...
        self.consumed_calories_pc_breakdown_per_day_df = pd.DataFrame(
            {
                key: self.updated_diet_df[key] / 365 * self.kg_to_kcal_dict[key] * non_wasted_share
                for key in non_years_columns
            }
        )

        self.consumed_calories_pc_breakdown_per_day_df.insert(
            0, GlossaryCore.Years, self.updated_diet_df[GlossaryCore.Years]
        )
