...
class TempChange(object):
    ...
    def compute_exog_forcing_myhre(self, ch4_ppm, n2o_ppm):
        ...
        exog_forcing = (
            0.036 * (np.sqrt(ch4_ppm) - np.sqrt(self.ch4_conc_init_ppm))
            - MN(ch4_ppm, self.n2o_conc_init_ppm)
            + 0.12 * (np.sqrt(n2o_ppm) - np.sqrt(self.n2o_conc_init_ppm))
            - MN(self.ch4_conc_init_ppm, n2o_ppm)
    ...
    def compute_forcing_etminan(self, co2_ppm, ch4_ppm, n2o_ppm):
        ...
        co2_forcing = (
            -2.4e-7 * (co2_ppm - self.c0_ppm) ** 2
            + 7.2e-4 * sign_values * (co2_ppm - self.c0_ppm)
            - 2.1e-4 * n2omean
            + self.forcing_eq_co2 / np.log(2)
        ) * np.log(co2_ppm / self.c0_ppm)
        ch4_forcing = (-1.3e-6 * ch4mean - 8.2e-6 * n2omean + 0.043) * (
            np.sqrt(ch4_ppm) - np.sqrt(self.ch4_conc_init_ppm)
        )
    ...
    def compute_forcing_meinshausen(self, co2_ppm, ch4_ppm, n2o_ppm):
        ...
        alphap = d1 + a1 * (co2_ppm - self.c0_ppm) ** 2 + b1 * (co2_ppm - self.c0_ppm)
        ...
        self.forcing_df["CH4 forcing"] = ch4_forcing
        # N2O
        n2o_forcing = (a2 * np.sqrt(co2_ppm) + b2 * np.sqrt(n2o_ppm) + c2 * np.sqrt(ch4_ppm) + d2) * (
            np.sqrt(n2o_ppm) - np.sqrt(self.n2o_conc_init_ppm)
        )
    ...
    def compute_log_co2_forcing(self, co2_ppm):
        co2_forcing = self.forcing_eq_co2 / np.log(2) * np.log(co2_ppm / (self.c0_ppm))
        return co2_forcing
    ...
    ######### FUND ########
    def compute_temp_fund(self, year, temperature):
        """
        ...
        e_folding_time = max(alpha + beta_l * cs + beta_q * cs * cs, 1)
        radiative_forcing = self.temperature_df.loc[
            self.temperature_df[GlossaryCore.Years] == year, GlossaryCore.Forcing
        ].values[0]
        temperature = (1 - 1 / e_folding_time) * temperature + cs / (
            5.35 * np.log(2) * e_folding_time
        ) * radiative_forcing

        self.temperature_df.loc[self.temperature_df[GlossaryCore.Years] == year, GlossaryCore.TempAtmo] = temperature
        return temperature
    ...
    def compute_temperature_year_end_constraint(self):
        ...
        """
        temp_atmo_year_end = self.temperature_df.at[self.year_end, GlossaryCore.TempAtmo]
        self.temperature_end_constraint = np.array(
            [(self.temperature_end_constraint_limit - temp_atmo_year_end) / self.temperature_end_constraint_ref]
        )
    ...
    def compute_dexog_forcing_myhre(self, ch4_ppm, n2o_ppm):
        ...
        dexog_forcing_ch4 = 0.036 / (2 * np.sqrt(ch4_ppm)) - dMN_dc1c2(ch4_ppm, self.n2o_conc_init_ppm, "c1")
        dexog_forcing_n2o = 0.12 / (2 * np.sqrt(n2o_ppm)) - dMN_dc1c2(self.ch4_conc_init_ppm, n2o_ppm, "c2")
    ...
    def compute_dforcing_etminan(self, co2_ppm, ch4_ppm, n2o_ppm):
        ...
        # CO2
        dco2_forcing_dco2_ppm = (-2.4e-7 * 2.0 * (co2_ppm - self.c0_ppm) + sign_values * 7.2e-4) * np.log(
            co2_ppm / self.c0_ppm
        ) + 1.0 / co2_ppm * (
            -2.4e-7 * (co2_ppm - self.c0_ppm) ** 2
            + sign_values * 7.2e-4 * (co2_ppm - self.c0_ppm)
            - 2.1e-4 * n2omean
            + self.forcing_eq_co2 / np.log(2)
        )
        ...
        self.d_forcing_datmo_conc_dict = {
            "CO2 forcing CO2 ppm": dco2_forcing_dco2_ppm,
    ...
    def compute_dforcing_meinshausen(self, co2_ppm, ch4_ppm, n2o_ppm):
        ...
        dco2_forcing_dco2_ppm = (dalphap_dco2_ppm) * np.log(co2_ppm / self.c0_ppm) + (alphap + alpha_n2o) / co2_ppm
        ...
        dch4_forcing_dch4_ppm = a3 / (2.0 * np.sqrt(ch4_ppm)) * (np.sqrt(ch4_ppm) - np.sqrt(self.ch4_conc_init_ppm)) + (
            a3 * np.sqrt(ch4_ppm) + b3 * np.sqrt(n2o_ppm) + d3
        ) / (2.0 * np.sqrt(ch4_ppm))
    ...
    def compute_d_temp_d_forcing_fund(self):
        ...
        beta_q = 0.001
        e_folding_time = max(
            alpha + beta_l * self.climate_sensitivity + beta_q * self.climate_sensitivity * self.climate_sensitivity, 1
        )
    ...
    def compute(self, in_dict) -> DataFrame:
        ...
        if self.temperature_model == "DICE":
            ...
            for year in self.years_range[1:]:
                temperature = self.compute_temp_fund(year, temperature)
                sea_level = self.compute_sea_level_fund(year, sea_level, temperature)
