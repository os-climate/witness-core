...
class Population:
    ...
    def create_dataframe(self):
        ...
        self.population_dict = {GlossaryCore.YearStartDefault: np.array(pop_by_age + [self.total_pop])}

        column_list = self.age_list.copy()
        column_list.insert(0, GlossaryCore.Years)
        self.column_list = self.age_list.copy()
        # WORKING POULATION
        self.working_age_population_df = DataFrame(
            index=years_range, columns=[GlossaryCore.Years, GlossaryCore.Population1570]
        )
        self.working_age_population_df[GlossaryCore.Years] = years_range

        # BIRTH RATE
        # BASE => calculated from GDB and knowledge level
        self.birth_rate = DataFrame(
            {GlossaryCore.Years: years_range, "knowledge": 0.0, "birth_rate": 0.0}, index=years_range
        )

        # BIRTH NUMBER
        # BASE => calculated from GDB and knowledge level
        self.birth_df = DataFrame(
            {GlossaryCore.Years: years_range, "knowledge": 0.0, "birth_rate": 0.0}, index=years_range
        )
        ...
        # LIFE EXPECTANCY
        self.life_expectancy_df = DataFrame({GlossaryCore.Years: years_range, "life_expectancy": 0}, index=years_range)
    ...
    def compute_birth_rate_v1(self, year):
        ...
        birth_rate = self.br_upper + (self.br_lower - self.br_upper) / (
            1 + np.exp(-self.br_delta * (gdp / pop - self.br_phi))
        ) ** (1 / self.br_nu)
    ...
    def compute_death_rate(self, year):
        ...
        death_rate = param["death_rate_upper"] + (param["death_rate_lower"] - param["death_rate_upper"]) / (
            1 + np.exp(-param["death_rate_delta"] * (gdp / pop - param["death_rate_phi"]))
        ) ** (1 / param["death_rate_nu"])
    ...
    def compute_death_rate_v2(self, year):
        ...
        # For all age range compute death rate
        death_rate = param["death_rate_upper"].values + (
            param["death_rate_lower"].values - param["death_rate_upper"].values
        ) / (1 + np.exp(-param["death_rate_delta"].values * (gdp / pop - param["death_rate_phi"].values))) ** (
            1 / param["death_rate_nu"].values
        )
        ...
        for i in range(len(death_rate)):
            if diet_death_rate[i] >= 1 - death_rate[i] * (1 + climate_death_rate[i]):
                diet_death_rate[i] = (1 - death_rate[i] * (1 + climate_death_rate[i])) / (
                    1 + np.exp(-diet_death_rate[i])
                )
    ...
    def compute_death_number(self, year):
        ...
        for effect in self.death_rate_dict:
            if effect != "total":
                ...
                full_dr_death.append(self.death_rate_dict[effect][year][-1])
    ...
    def compute_population_next_year(self, year, total_death, nb_birth):
        if year > self.year_end:
            ...
            self.population_dict[year] = np.array(population_list + [self.total_pop])
    ...
    def compute(
        self, in_dict
    ) -> tuple[DataFrame, DataFrame, dict[str, DataFrame], DataFrame, dict, DataFrame, DataFrame]:
        """
        ...
        self.population_df = DataFrame.from_dict(self.population_dict, orient="index", columns=self.pop_df_column)

        self.population_df.insert(loc=0, column=GlossaryCore.Years, value=self.years_range)
        self.population_df = self.population_df.replace([np.inf, -np.inf], np.nan)

        # Compute working age population between 15 and 70 years
        working_age_idx = [str(i) for i in np.arange(15, 71)]
        self.working_age_population_df[GlossaryCore.Population1570] = self.population_df[working_age_idx].sum(axis=1)
        ...
        self.death_rate_df = DataFrame.from_dict(self.death_rate_df_dict, orient="index", columns=self.column_list)

        # recontruction of the death rate_dict with dataframes instead of
        # ditionaries at the end of the year loop
        self.death_rate_dict = {
            "base": self.base_death_rate_df,
        ...
        for effect in self.death_rate_dict:
            self.death_dict[effect] = DataFrame(self.death_list_dict[effect], index=self.years_range)
            self.death_dict[effect]["total"] = self.death_dict[effect].sum(axis=1, skipna=True)
        ...
        return (
            self.population_df.fillna(0.0),
            self.birth_rate.fillna(0.0),
            self.death_rate_dict,
            self.birth_df.fillna(0.0),
            self.death_dict,
            self.life_expectancy_df.fillna(0.0),
            self.working_age_population_df.fillna(0.0),
        )
    ...
    def compute_d_pop_d_output(self):
        ...
        for year in self.years_range:
            if year == self.year_end:
                ...
                d_birthrate_d_output += self.d_birthrate_d_output(year, d_pop_tot_d_output)
                d_birth_d_output += self.d_birth_d_generic(year, d_birthrate_d_output, d_pop_1549_d_output)

                d_base_deathrate_d_output[year] = self.d_base_death_rate_d_output(year, d_pop_tot_d_output)
                d_death_rate_climate_d_output[year] = self.d_climate_death_rate_d_output(
                    year, d_base_deathrate_d_output
                )

                d_death_d_output[year] = self.d_death_d_generic(
                    year, d_base_deathrate_d_output, d_death_rate_climate_d_output, d_pop_d_output
                )
                d_pop_d_output, d_pop_1549_d_output, d_pop_tot_d_output, d_working_pop_d_output = (
                    self.d_poptotal_generic(
                        year,
                        d_pop_d_output,
                        d_death_d_output,
                        d_birth_d_output,
    ...
    def d_birthrate_d_output(self, year, d_pop_tot_d_output):
        ...
        g_prime_f = (1 / nu) * (1 + np.exp(-delta * (gdp / pop - phi))) ** (1 / nu - 1)
        f_prime = -delta * (pop * idty * self.trillion - d_pop * gdp) / (pop**2) * (np.exp(-delta * (gdp / pop - phi)))
    ...
    def d_birth_d_generic(self, year, d_birthrate_d_output, d_pop_1549_d_output):
        ...
        d_birth_d_output[iyear] = d_birthrate_d_output[iyear] * pop_1549 + d_pop_1549 * br
    ...
    def d_base_death_rate_d_output(self, year, d_pop_tot_d_output):
        ...
        for age_range in param["param"].values:
            ...
            g_prime_f = (1 / nu) * (1 + np.exp(-delta * (gdp / pop - phi))) ** (1 / nu - 1)
            f_prime = (
                -delta * (pop * idty * self.trillion - d_pop * gdp) / (pop**2) * np.exp(-delta * (gdp / pop - phi))
            )
            u_prime = g_prime_f * f_prime

            d_death_rate_d_output_age = -(br_lower - br_upper) * u_prime / u_squared
    ...
    def d_climate_death_rate_d_output(self, year, d_base_death_rate):
        ...
        for age_range in param["param"].values:
            ...
            climate_death_rate = add_death["beta"][age_range] * (temp / cal_temp_increase) ** theta
    ...
    def d_death_d_generic(
        self, year, dict_d_base_death_rate_d_temp, dict_d_climate_death_rate_d_temp, dict_d_population_d_output
    ):
        """
        ...
        list_d_base_dr_d_out = list(dict_d_base_death_rate_d_temp[year].values())
        list_d_climate_dr_d_out = list(dict_d_climate_death_rate_d_temp[year].values())
        list_d_pop_d_out = {}
        ...
        base_list_d_dr_d_out = list(chain(*zip(*[base_list_d_dr_d_out_t for _ in range(5)])))
        ...
        # Duplicate to get one per age
        climate_list_d_dr_d_out = list(chain(*zip(*[climate_list_d_dr_d_out_t for _ in range(5)])))
        # and add value of last year
        ...
        for i in range(0, len(ages)):
            ...
            d_death[ages[i]] = (
                list_d_pop_d_out[ages[i]] * full_dr_death[i]
                + (climate_list_d_dr_d_out_i + base_list_d_dr_d_out[i]) * pop_year[i]
            )

        return d_death

    def d_poptotal_generic(self, year, d_pop, d_death, d_birth, d_pop_1549, d_total_pop, d_working_pop):
        """
        ...
        if year + 1 <= self.year_end:
            ...
            for i in range(1, len(age_list)):
                ...
                d_pop_d_age_prev[age] = d_pop.get(year, {}).get(age, 0) - d_death[year][age]  # .get(age, 0)
    ...
    def compute_d_pop_d_temp(self):
        ...
        for year in self.years_range:
            if year == self.year_end:
                ...
                d_birthrate_d_temp += self.d_birthrate_d_temp(year, d_pop_tot_d_temp)
                d_birth_d_temp += self.d_birth_d_generic(year, d_birthrate_d_temp, d_pop_1549_d_temp)

                d_base_death_rate[year] = self.d_base_death_rate_d_temp(year, d_pop_tot_d_temp)
                d_climate_death_rate[year] = self.d_climate_death_rate_d_temp(year, d_base_death_rate)

                d_death_d_temp[year] = self.d_death_d_generic(
                    year, d_base_death_rate, d_climate_death_rate, d_pop_d_temp
                )
                d_pop_d_temp, d_pop_1549_d_temp, d_pop_tot_d_temp, d_working_pop_d_temp = self.d_poptotal_generic(
                    year,
                    d_pop_d_temp,
                    d_death_d_temp,
                    d_birth_d_temp,
    ...
    def d_birthrate_d_temp(self, year, d_pop_tot_d_temp):
        ...
        # exp(-delta(gdp/pop-phi))
        g_prime_f = (1 / nu) * (1 + np.exp(-delta * (gdp / pop - phi))) ** (1 / nu - 1)
        f_prime = -delta * (-gdp * d_pop) / (pop**2) * np.exp(-delta * (gdp / pop - phi))
        u_prime = g_prime_f * f_prime
    ...
    def d_base_death_rate_d_temp(self, year, d_pop_tot_d_temp):
        ...
        for age_range in param["param"].values:
            ...
            w_squared = ((1 + np.exp(-delta * (gdp / pop - phi))) ** (1 / nu)) ** 2
            # w = g(f(x)) with g = f**(1:nu) and f = (1+
            # exp(-delta(gdp/pop-phi))
            g_prime_f = (1 / nu) * (1 + np.exp(-delta * (gdp / pop - phi))) ** (1 / nu - 1)
            f_prime = -delta * (-d_pop * gdp) / (pop**2) * np.exp(-delta * (gdp / pop - phi))
    ...
    def d_climate_death_rate_d_temp(self, year, d_base_death_rate):
        ...
        for age_range in param["param"].values:
            ...
            d_climate_death_rate = beta * theta * idty / cal_temp_increase * (temp / cal_temp_increase) ** (theta - 1)
            # (uv)' = u_prime_v * v_prime_u with u = base_deathrate v = climate_deathrate
            u_prime_v = d_base_death_rate[year][age_range] * climate_death_rate[age_range] / base_death_rate[age_range]
    ...
    def compute_d_pop_d_kcal_pc(self):
        ...
        for year in self.years_range:
            if year == self.year_end:
                ...
                d_birthrate_d_kcal_pc += self.d_birthrate_d_kcal_pc(year, d_pop_tot_d_kcal_pc)
                d_birth_d_kcal_pc += self.d_birth_d_generic(year, d_birthrate_d_kcal_pc, d_pop_1549_d_kcal_pc)
                d_base_death_rate[year] = self.d_base_death_rate_d_kcal_pc(year, d_pop_tot_d_kcal_pc)
                ...
                d_pop_d_kcal_pc, d_pop_1549_d_kcal_pc, d_pop_tot_d_kcal_pc, d_working_pop_d_kcal_pc = (
                    self.d_poptotal_generic(
                        year,
                        d_pop_d_kcal_pc,
                        d_death_d_kcal_pc,
    ...
    def d_base_death_rate_d_kcal_pc(self, year, d_pop_tot_d_kcal_pc):
        ...
        for age_range in param["param"].values:
            ...
            d_base_deathrate_d_kcal_pc[age_range] = (
                d_base_death_rate + d_base_death_rate * beta * (temp / cal_temp_increase) ** theta
            )
