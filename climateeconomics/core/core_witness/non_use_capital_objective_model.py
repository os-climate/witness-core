...
class NonUseCapitalObjective:
    ...
    def compute(self, inputs_dict):
        ...
        self.non_use_capital_df = self.agreggate_and_compute_sum("non_use_capital", inputs_dict)

        self.techno_capital_df = self.agreggate_and_compute_sum(GlossaryEnergy.TechnoCapitalValue, inputs_dict)
    ...
    def agreggate_and_compute_sum(self, name, inputs_dict):
        ...
        """
        name_df_list = [
            value.drop([GlossaryCore.Years], axis=1) for key, value in inputs_dict.items() if key.endswith(name)
        ]
        ...
        if len(name_df_list) != 0:
            ...
            non_use_capital_df[name_sum] = non_use_capital_df_concat.sum(axis=1)

            non_use_capital_df = pd.concat([non_use_capital_df, non_use_capital_df_concat], axis=1)
    ...
    def compute_objective(self):
        ...
        if "Sum of non use capital" in self.non_use_capital_df:
            self.non_use_capital_objective_wo_ponderation = (
                np.asarray([self.non_use_capital_df["Sum of non use capital"].sum()]) / self.delta_years
            )
            self.non_use_capital_objective = (
                self.non_use_capital_objective_wo_ponderation / self.non_use_capital_obj_ref
            )
    ...
    def compute_constraint(self):
        ...
        if "Sum of non use capital" in self.non_use_capital_df:
            self.non_use_capital_cons = (
                self.non_use_capital_cons_limit - self.non_use_capital_objective_wo_ponderation
            ) / self.non_use_capital_cons_ref

        reforestation_lost_capital_wo_ponderation = (
            np.asarray([self.forest_lost_capital["reforestation"].sum()]) / self.delta_years
        )
        managed_wood_lost_capital_wo_ponderation = (
            np.asarray([self.forest_lost_capital["managed_wood"].sum()]) / self.delta_years
        )
        deforestation_lost_capital_wo_ponderation = (
            np.asarray([self.forest_lost_capital["deforestation"].sum()]) / self.delta_years
        )
        forest_lost_capital_wo_ponderation = (
            reforestation_lost_capital_wo_ponderation
            + managed_wood_lost_capital_wo_ponderation
            + deforestation_lost_capital_wo_ponderation
        )
        self.forest_lost_capital_cons = (
            self.forest_lost_capital_cons_limit - forest_lost_capital_wo_ponderation
        ) / self.forest_lost_capital_cons_ref
    ...
    def get_energy_capital_trillion_dollars(self):
        ...
        energy_capital_df = pd.DataFrame(
            {GlossaryCore.Years: self.years_range, GlossaryCore.Capital: sum_techno_capital}
        )
