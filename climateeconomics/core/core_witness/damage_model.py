...
class DamageModel:
    ...
    def __init__(self, param):
        ...
        self.damage_fraction_df = pd.DataFrame(
            ...
            data={
                ...
                GlossaryCore.DamageFractionOutput: 0.0,
                GlossaryCore.BaseCarbonPrice: 0.0,
            },
        )

    def compute_damage_fraction_of_gdp(
        self,
    ):
        """
    ...
    def compute_CO2_damage_price_dev(self):
        ...
        self.co2_damage_price_df = pd.DataFrame(
            {GlossaryCore.Years: self.damage_fraction_df.index, GlossaryCore.CO2DamagePrice: co2_damage_price}
        )
    ...
    def compute_gradient(self):
        ...
        for i in range(nb_years):
            for line in range(nb_years):
                if i == line:
                    temp_atmo = self.temperature_df.at[years[line], GlossaryCore.TempAtmo]
                    if self.tipping_point_model:
                        if temp_atmo < 0:
                            ddamage_frac_output_temp_atmo[line, i] = 0.0
                        else:
                            ddamage_frac_output_temp_atmo[line, i] = (
                                (self.tp_a4 * (temp_atmo / self.tp_a3) ** self.tp_a4)
                        ...
                        ddamage_frac_output_temp_atmo[line, i] = (
                            self.damag_int + self.damag_quad * self.damag_expo * temp_atmo ** (self.damag_expo - 1)
                        )
    ...
    def d_co2_damage_price_dev_d_user_input(self, d_co2_extra_ton_damage_price_d_user_input):
        ...
        """
        d_co2_damage_price_d_co2_extra_ton_damage_price = np.tril(
            np.ones(len(d_co2_extra_ton_damage_price_d_user_input))
        )
    ...
    def d_extra_co2_t_damage_price_d_extra_co2_ton(self):
        ...
        extra_co2t_since_pre_indus = self.extra_gigatons_co2eq_since_pre_indus_df[
    ...
    def compute_extra_ton_damage_price(self):
        ...
        """
        extra_CO2eq = self.extra_gigatons_co2eq_since_pre_indus_df[
            GlossaryCore.ExtraCO2EqSincePreIndustrialValue
        ].values  # Gt
        ...
        self.extra_co2_eq_damage_price_df = pd.DataFrame(
            {GlossaryCore.Years: self.years_range, GlossaryCore.ExtraCO2tDamagePrice: extra_CO2t_eq_cost}
        )
    ...
    def compute_CO2_damage_price(self):
        ...
        co2_damage_price = np.zeros(len(self.damage_fraction_df.index), dtype=arr_type)
        ...
        for i, year in enumerate(self.damage_fraction_df.index):
            ...
            if year == self.year_end:
                co2_damage_price[i] = 1e3 * 1.01**i * damages[i] / self.total_emissions_ref
            else:
                k = max(year + 25 - self.year_end, 0)
                co2_damage_price[i] = 1e3 * 1.01**i * np.mean(damages[i : i + 25 - k]) / self.total_emissions_ref

        self.co2_damage_price_df = pd.DataFrame(
            {GlossaryCore.Years: self.years_range, GlossaryCore.CO2DamagePrice: co2_damage_price}
        )
