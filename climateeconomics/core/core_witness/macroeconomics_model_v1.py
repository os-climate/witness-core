...
#         economics_df['saving_rate'] = self.saving_rate
    ...
    def compute_capital(self, year: int):
        ...
        if year > self.year_end:
            ...
            ne_investment = self.economics_df.at[year - self.time_step, GlossaryCore.NonEnergyInvestmentsValue]
    ...
    def compute_investment(self, year: int):
        ...
        self.economics_df.loc[year, GlossaryCore.InvestmentsValue] = self.economics_df.at[year, GlossaryCore.EnergyInvestmentsValue] + \
                                                    self.economics_df.at[year, GlossaryCore.NonEnergyInvestmentsValue]
    ...
    def compute_energy_investment(self, year: int):
        ...
        em_wo_ren.loc[year, GlossaryCore.EnergyInvestmentsWoRenewableValue] = energy_investment_wo_tax * 1e3
        ...
        self.economics_df.loc[year,
                              [GlossaryCore.EnergyInvestmentsValue,
                               ...
                               GlossaryCore.EnergyInvestmentsFromTaxValue]] = [energy_investment,
                                                                               energy_investment_wo_tax,
                                                                               ren_investments]
        self.energy_investment.loc[year, GlossaryCore.EnergyInvestmentsValue] = energy_investment * 1e3 / self.scaling_factor_energy_investment  # Invest from T$ to G$ coupling variable
    ...
    def prepare_outputs(self):
        ...
        self.economics_df = self.economics_df[GlossaryCore.Economics_df['dataframe_descriptor'].keys()]
        self.economics_detail_df = self.economics_detail_df[GlossaryCore.EconomicsDetail_df['dataframe_descriptor'].keys()]
    ...
    def d_productivity_d_damage_frac_output(self):
        ...
        if self.damage_to_productivity:   # todo : maybe it can be computed more efficiently ?
            p_productivity_gr = self.economics_detail_df['productivity_gr'].values
            p_productivity = self.economics_detail_df['productivity'].values
    ...
    def d_gross_output_d_working_pop(self):
        ...
        productivity = self.economics_detail_df['productivity'].values
    ...
    def d_consumption_d_user_input(self, d_net_output_d_user_input, d_investment_d_user_input):
        ...
        consumption = self.economics_detail_df['consumption'].values
    ...
    def d_consumption_pc_d_population(self):
        ...
        self.economics_df.loc[year, 'pc_consumption'] = max(
            consumption_pc, self.lo_per_capita_conso)
        """
        consumption_pc = self.economics_detail_df['pc_consumption'].values
        consumption = self.economics_detail_df['consumption'].values
    ...
    def d_net_output_d_damage_frac_output(self, d_gross_output_d_damage_frac_output):
        ...
        for i in range(nb_years):
            gross_output = self.economics_detail_df.at[self.years_range[i], 'gross_output']
    ...
    def d_investment_d_share_investment_non_energy(self):
        ...
        net_output = self.economics_detail_df['output_net_of_d'].values
