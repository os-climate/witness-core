...
class MacroEconomics:
    ...
    def __init__(self, param):
        ...
        self.decline_rate_tfp: float = 0.0
        ...
        self.energy_eff_k: float = 0.0
        self.energy_eff_cst: float = 0.0
        self.energy_eff_xzero: float = 0.0
        self.energy_eff_max: float = 0.0
        ...
        self.co2_invest_limit: float = 0.0
    ...
    def set_data(self):
        ...
        self.nb_per = round(
            (self.param[GlossaryCore.YearEnd] - self.param[GlossaryCore.YearStart]) / self.param[GlossaryCore.TimeStep]
            + 1
        )
    ...
    def create_dataframe(self):
        ...
        default_index = np.arange(self.year_start, self.year_end + 1, self.time_step)
        param = self.param
        economics_df = pd.DataFrame(
            index=default_index, columns=list(GlossaryCore.EconomicsDetailDf["dataframe_descriptor"].keys())
        )

        for key in economics_df.keys():
            economics_df[key] = 0
        economics_df[GlossaryCore.Years] = self.years_range
        economics_df.loc[param[GlossaryCore.YearStart], GlossaryCore.GrossOutput] = self.init_gross_output
        economics_df.loc[param[GlossaryCore.YearStart], GlossaryCore.Productivity] = self.productivity_start
        economics_df.loc[param[GlossaryCore.YearStart], GlossaryCore.ProductivityWithDamage] = self.productivity_start
        economics_df.loc[param[GlossaryCore.YearStart], GlossaryCore.ProductivityWithoutDamage] = (
            self.productivity_start
        )
        economics_df.loc[param[GlossaryCore.YearStart], GlossaryCore.ProductivityGrowthRate] = (
            self.productivity_gr_start
        )
        economics_df.loc[param[GlossaryCore.YearStart], GlossaryCore.OutputGrowth] = self.init_output_growth
        #         economics_df['saving_rate'] = self.saving_rate
        self.economics_df = economics_df
        self.economics_df = self.economics_df.replace([np.inf, -np.inf], np.nan)

        self.energy_investment_wo_renewable = pd.DataFrame(
            index=default_index, columns=GlossaryCore.EnergyInvestmentsWoRenewable["dataframe_descriptor"].keys()
        )
        self.energy_investment_wo_renewable[GlossaryCore.Years] = self.years_range

        energy_investment = pd.DataFrame(
            index=default_index, columns=GlossaryCore.EnergyInvestments["dataframe_descriptor"].keys()
        )
        ...
        self.energy_investment = self.energy_investment.replace([np.inf, -np.inf], np.nan)
        # workforce_df
        workforce_df = pd.DataFrame(
            index=default_index, columns=[GlossaryCore.Years, GlossaryCore.EmploymentRate, GlossaryCore.Workforce]
        )
        ...
        self.damage_df = pd.DataFrame(
            index=default_index, columns=GlossaryCore.DamageDetailedDf["dataframe_descriptor"].keys()
        )
        ...
        self.df_gdp_per_country = pd.DataFrame(
            columns=[GlossaryCore.CountryName, GlossaryCore.Years, GlossaryCore.GDPName, GlossaryCore.GroupName]
        )
        self.dict_energy_consumption_detailed = {}
        self.dict_sector_emissions_detailed = {}
        return (
            economics_df.fillna(0.0),
            energy_investment.fillna(0.0),
        )
    ...
    def set_coupling_inputs(self, inputs: dict):
        ...
        self.co2_emissions_Gt = pd.DataFrame(
            {
                GlossaryCore.Years: inputs[GlossaryCore.CO2EmissionsGtValue][GlossaryCore.Years].values,
                GlossaryCore.TotalCO2Emissions: inputs[GlossaryCore.CO2EmissionsGtValue][
        ...
        self.share_non_energy_investment = pd.Series(
            inputs[GlossaryCore.ShareNonEnergyInvestmentsValue][GlossaryCore.ShareNonEnergyInvestmentsValue].values
            / 100.0,
            index=self.years_range,
        )
        ...
        self.dict_dataframe_energy_consumption_sections = dict(
            zip(
                self.sector_list,
                [
                    inputs[f"{GlossaryCore.SectorEnergyConsumptionPercentageDfName}_{sector}"]
    ...
    def compute_workforce(self):
        ...
        """
        self.workforce_df[GlossaryCore.Workforce] = (
            self.workforce_df[GlossaryCore.EmploymentRate] * self.working_age_population_df[GlossaryCore.Population1570]
        )
    ...
    def compute_productivity(self, year: int):
        ...
        productivity_w_damage = (
            p_productivity_w_damage
    ...
    def compute_capital(self, year: int):
        ...
        if year > self.year_end:
            ...
            capital_a = ne_capital * (1 - self.depreciation_capital) ** self.time_step + self.time_step * ne_investment
    ...
    def compute_energy_usage(self):
        ...
        optimal_energy_production = (
            self.max_capital_utilisation_ratio * non_energy_capital / self.capital_utilisation_ratio / energy_efficiency
        )  # Pwh
        ...
        self.economics_df[GlossaryCore.UnusedEnergy] = (
            np.maximum(net_energy_production - optimal_energy_production, 0.0) * 1e3
        )
        ...
        self.economics_df.loc[self.economics_df[GlossaryCore.EnergyWasted] < 0.0, GlossaryCore.EnergyWasted] = 0.0
    ...
    def compute_energy_wasted_objective(self):
        ...
        energy_wasted_objective = (
            self.economics_df[GlossaryCore.EnergyWasted].values.sum()
            * 1e-3
    ...
    def compute_investment(self, year: int):
        ...
        self.economics_df.loc[year, GlossaryCore.NonEnergyInvestmentsValue] = (
            self.share_non_energy_investment[year] * net_output
        )
        self.economics_df.loc[year, GlossaryCore.InvestmentsValue] = (
            self.economics_df.loc[year, GlossaryCore.EnergyInvestmentsValue]
            + self.economics_df.loc[year, GlossaryCore.NonEnergyInvestmentsValue]
        )
    ...
    def compute_energy_investment(self, year: int):
        ...
        self.energy_investment_wo_renewable.loc[year, GlossaryCore.EnergyInvestmentsWoRenewableValue] = (
            energy_investment_wo_tax * 10.0
        )  # in 100G$
        ...
        self.energy_investment.loc[year, GlossaryCore.EnergyInvestmentsValue] = energy_investment * 10.0  # 100G$
    ...
    def compute_energy_renewable_investment(self, year: int, energy_investment_wo_tax: float):
        ...
        co2_tax_eff = self.co2_tax_efficiency.loc[year, GlossaryCore.CO2TaxEfficiencyValue] / 100.0  # %
        ...
        if ren_investments > co2_invest_limit * energy_investment_wo_tax and ren_investments != 0.0:
            ren_investments = (
                ...
                * energy_investment_wo_tax
                / 10.0
                * (9.0 + np.exp(-co2_invest_limit * energy_investment_wo_tax / ren_investments))
            )
    ...
    def set_gross_output(self):
        ...
        self.economics_df = self.economics_df.merge(
            self.gross_output_in[[GlossaryCore.Years, GlossaryCore.GrossOutput]], on=GlossaryCore.Years, how="left"
        ).set_index(self.economics_df.index)
    ...
    def get_gdp_percentage_per_section(self):
        ...
        if start_year_csv > self.year_start:
            ...
            self.gdp_percentage_per_section_df.iloc[0 : (start_year_csv - self.year_start)][GlossaryCore.Years] = (
                np.arange(self.year_start, start_year_csv)
            )
        ...
        if end_year_csv > self.year_end:
            ...
            list_df_to_concat.extend([self.gdp_percentage_per_section_df.iloc[-1:]] * (self.year_end - end_year_csv))
            self.gdp_percentage_per_section_df = pd.concat(list_df_to_concat).reset_index(drop=True)
    ...
    def compute_section_gdp(self):
        ...
        self.section_gdp_df[self.section_list] = (
            self.section_gdp_df[self.section_list].multiply(
                self.economics_df.reset_index(drop=True)[GlossaryCore.OutputNetOfDamage], axis="index"
            )
            / 100.0
        )
    ...
    def compute_sector_gdp(self):
        ...
        dict_sectors_sections = {
            GlossaryCore.SectorServices: pd.DataFrame(
                {
                    GlossaryCore.Years: self.years_range,
                    **{section: self.section_gdp_df[section].values for section in GlossaryCore.SectionsServices},
                }
            ),
            GlossaryCore.SectorIndustry: pd.DataFrame(
                {
                    GlossaryCore.Years: self.years_range,
                    **{section: self.section_gdp_df[section].values for section in GlossaryCore.SectionsIndustry},
                }
            ),
            GlossaryCore.SectorAgriculture: pd.DataFrame(
                {
                    GlossaryCore.Years: self.years_range,
                    **{section: self.section_gdp_df[section].values for section in GlossaryCore.SectionsAgriculture},
                }
            ),
        }
        ...
        dict_sum_by_sector.update(
            {
                sector: dict_sectors_sections[sector][GlossaryCore.SectionDictSectors[sector]].sum(axis=1)
                for sector in self.sector_list
            }
        )
    ...
    def compute_output_growth(self):
        ...
        self.economics_df[GlossaryCore.OutputGrowth] = (gross_output.diff() / gross_output.shift(1)).fillna(0.0) * 100
    ...
    def compute_usable_capital_lower_bound_constraint(self):
        ...
        self.delta_capital_cons = (
            (usable_capital - self.capital_utilisation_ratio * ne_capital) / self.usable_capital_ref
            if not self.compute_gdp
            else np.zeros(self.nb_per)
        )
    ...
    def compute_usable_capital_objective(self):
        ...
        self.usable_capital_objective = np.array(
            [np.sum(np.power(self.usable_cap_sqrt, 2)) / (self.nb_years * self.usable_capital_objective_ref)]
        )
    ...
    def d_usable_capital_obj_d_input(self, dkne_dinput):
        d_Kuobj_dKne = (
            ...
            * self.usable_cap_sqrt
            / self.nb_years
            / self.usable_capital_objective_ref
        )
    ...
    def prepare_outputs(self):
        ...
        self.economics_detail_df = self.economics_detail_df[
            GlossaryCore.EconomicsDetailDf["dataframe_descriptor"].keys()
        ]

        self.energy_investment = self.energy_investment.fillna(0.0)

        self.energy_investment_wo_renewable = self.energy_investment_wo_renewable.fillna(0.0)
    ...
    def compute_regionalised_gdp(self):
        ...
        result_total_gdp_per_group = np.array(dict_linear_parameters["a"]) * self.years_range + np.array(
            dict_linear_parameters["b"]
        ).reshape(-1, 1)
        ...
        total_gdp_per_group = (
            percentage_gdp_per_group * self.economics_df[GlossaryCore.OutputNetOfDamage].values.reshape(-1, 1) / 100
        )
        ...
        for _, row in mean_percentage_gdp_country.iterrows():
            ...
            # and convert T$ to G$
            df_temp[GlossaryCore.GDPName] = (
                1000
                * row[GlossaryCore.MeanPercentageName]
                * self.total_gdp_per_group_df[row[GlossaryCore.GroupName]]
                / 100
            )
    ...
    def compute(self, inputs: dict):
        ...
        self.capital_df[GlossaryCore.NonEnergyInvestmentsValue] = self.economics_detail_df[
        ...
        self.capital_df[GlossaryCore.PerCapitaConsumption] = self.economics_detail_df[
        ...
        return (
            self.economics_detail_df,
            self.economics_df,
            self.damage_df,
            self.energy_investment,
            self.energy_investment_wo_renewable,
            self.workforce_df,
            self.capital_df,
            self.sector_gdp_df,
            self.energy_wasted_objective,
            self.total_gdp_per_group_df,
            self.percentage_gdp_per_group_df,
            self.df_gdp_per_country,
        )
    ...
    def compute_energy_consumption_per_section(self):
        ...
        for sector_name, percentage_sections_of_sector in self.dict_dataframe_energy_consumption_sections.items():
            ...
            merged_df_energy_prod = pd.merge(
                self.energy_production,
                self.sector_energy_consumption_percentage_df[[GlossaryCore.Years, sector_name]],
                on=GlossaryCore.Years,
                how="inner",
            )
            # division by 100 for percentages
            merged_df_energy_prod["energy_consumption_sector"] = (
                merged_df_energy_prod[GlossaryCore.TotalProductionValue] * merged_df_energy_prod[sector_name] / 100.0
            )
            merged_df_energy_prod = pd.merge(
                merged_df_energy_prod, percentage_sections_of_sector, on=GlossaryCore.Years, how="inner"
            )
            ...
            merged_df_energy_prod[list_sections] = percentage_sections_of_sector[list_sections].apply(
                lambda col: col * merged_df_energy_prod["energy_consumption_sector"] / 100.0
            )
            ...
            self.dict_energy_consumption_detailed[sector_name]["detailed"] = merged_df_energy_prod[
                list_columns_to_store
            ]

            # Compute total energy consumption for sector
            total_energy_consumption_sector = pd.DataFrame(
                columns=[GlossaryCore.Years, GlossaryCore.TotalEnergyConsumptionSectorName]
            )
            total_energy_consumption_sector[GlossaryCore.TotalEnergyConsumptionSectorName] = merged_df_energy_prod[
                list_sections
            ].sum(axis=1)
        ...
        self.dict_energy_consumption_detailed["total"] = (
            pd.concat(
                [
                    detailed_data["total"].set_index(GlossaryCore.Years)
                    for detailed_data in self.dict_energy_consumption_detailed.values()
                ],
                axis=1,
            )
            .sum(axis=1)
            .to_frame(name=GlossaryCore.TotalEnergyConsumptionAllSectorsName)
            .reset_index()
        )

    def compute_consumption_objective(self):
        self.consommation_objective = (
            np.array([self.economics_df[GlossaryCore.Consumption].mean()]) / self.consommation_objective_ref
        )

    def compute_energy_consumption_households(self):
        energy_consumption_households = (
            self.share_residential_df[GlossaryCore.ShareSectorEnergy].values
            * self.energy_production[GlossaryCore.TotalProductionValue].values
        ) / 100.0

        self.energy_consumption_households_df = pd.DataFrame(
            {GlossaryCore.Years: self.years_range, GlossaryCore.TotalProductionValue: energy_consumption_households}
        )
    ...
    def d_productivity_w_damage_d_damage_frac_output(self):
        ...
        for i in range(1, nb_years):
            d_productivity_w_damage_d_damage_frac_output[i, i] = (
                ...
                * self.damage_fraction_output_df.loc[self.years_range[i], GlossaryCore.DamageFractionOutput]
    ...
    def d_Y_Ku_Ew_Constraint_d_energy(self):
        ...
        index_zeros = self.economics_detail_df[GlossaryCore.UnusedEnergy].values > 0.0
        ...
        for i in range(1, self.nb_per + 1):
            for j in range(1, i):
                ...
                if i < self.nb_per:
                    d_Kne_dE[i, j] = (1 - self.depreciation_capital) * d_Kne_dE[
                        i - 1, j
        ...
        d_lower_bound_constraint_dE = (
            (d_Ku_d_E - self.capital_utilisation_ratio * d_Kne_dE) / self.usable_capital_ref
            if not self.compute_gdp
            else self._null_derivative()
        )
        ...
        d_Ku_obj_d_E = np.sum(
            2
            * (dKunbouded_d_E - self.capital_utilisation_ratio * d_Kne_dE)
            * self.usable_cap_sqrt.reshape(-1, 1)
            / (self.usable_capital_objective_ref * self.nb_years),
            axis=0,
        )
        # TODO use d_Ku_d_E
        ...
        d_Ew_dE = self._identity_derivative() * 1.0e3 - np.matmul(k, d_Kne_dE)  # Enet converted from PWh to TWh
        ...
        matrix_of_years_E_is_wasted = (self.economics_detail_df[GlossaryCore.EnergyWasted].values > 0.0).astype(int)
    ...
    def d_gross_output_d_working_pop(self):
        ...
        for i in range(1, self.nb_per + 1):
            for j in range(1, i):
                a = productivity[i - 1]
                b = alpha * usable_capital[i - 1] ** (gamma - 1) * d_Ku_d_wap[i - 1, j] + (1 - alpha) * working_pop[
                    i - 1
                ] * (gamma - 1) * employment_rate[i - 1] * (i - 1 == j)
                c = (alpha * usable_capital[i - 1] ** gamma + (1 - alpha) * working_pop[i - 1] ** gamma) ** (
                    1 / gamma - 1
                )
                ...
                if i < self.nb_per:
                    d_Kne_d_wap[i, j] = (1 - self.depreciation_capital) * d_Kne_d_wap[
                        i - 1, j
        ...
        d_lower_bound_constraint_d_wap = (
            (d_Ku_d_wap - self.capital_utilisation_ratio * d_Kne_d_wap) / self.usable_capital_ref
            if not self.compute_gdp
            else self._null_derivative()
        )
        d_Ku_unbouded_d_wap = self._null_derivative()
        d_Ku_obj_d_wap = np.sum(
            2
            * (d_Ku_unbouded_d_wap - self.capital_utilisation_ratio * d_Kne_d_wap)
            * self.usable_cap_sqrt.reshape(-1, 1)
            / (self.usable_capital_objective_ref * self.nb_years),
            axis=0,
        )
    ...
    def d_lotofthings_d_share_investment_non_energy(self):
        ...
        index_non_zeros = self.economics_detail_df[GlossaryCore.UnusedEnergy].values > 0.0
        dQ_dY = (
            1 - damefrac if not self.damage_to_productivity else (1 - damefrac) / (1 - self.frac_damage_prod * damefrac)
        )
        ...
        for i in range(1, self.nb_years):
            for j in range(i):
                d_KU_d_snei[i - 1, j] = (
                    index_non_zeros[i - 1] * self.max_capital_utilisation_ratio * d_Kne_d_snei[i - 1, j]
                )
                dY_d_snei[i - 1, j] = dY_dKU[i - 1, i - 1] * d_KU_d_snei[i - 1, j]
                d_Q_d_snei[i - 1, j] = dQ_dY[i - 1] * dY_d_snei[i - 1, j]
                d_Kne_d_snei[i, j] = (
                    (1 - self.depreciation_capital) * d_Kne_d_snei[i - 1, j]
                    + net_output[i - 1] * (i - 1 == j) / 100.0
        ...
        d_Ine_d_snei = np.diag(net_output) / 100.0 + np.diag(snei) @ d_Q_d_snei

        d_C_d_snei = d_Q_d_snei - d_Ine_d_snei

        d_lower_bound_constraint_d_snei = (
            (d_KU_d_snei - self.capital_utilisation_ratio * d_Kne_d_snei) / self.usable_capital_ref
            if not self.compute_gdp
            else self._null_derivative()
        )
        return (
    ...
    def _d_energy_investment_d_energy_investement_wo_tax(self):
        ...
        d_energy_investment_wo_renewable_d_energy_investment_wo_tax = (
            d_energy_investment_wo_tax_d_energy_investment_wo_tax * 1e3
        )  # TODO ? Sure of 1e3 ?
        ...
        d_energy_investment_d_energy_investment_wo_tax = (
            d_energy_investment_wo_tax_d_energy_investment_wo_tax + d_ren_investments_d_energy_investment_wo_tax
        )
    ...
    def d_investment_d_energy_investment_wo_tax(self):
        ...
        return (
            d_investment_d_energy_investment_wo_tax,
            d_energy_investment_d_energy_investment_wo_tax,
            d_non_energy_investment_d_energy_investment_wo_tax,
            d_energy_investment_wo_renewable_d_energy_investment_wo_tax,
        )
    ...
    def _d_ren_investments_d_energy_investment_wo_tax(self, denergy_investment_wo_tax):
        ...
        co2_tax_eff = self.co2_tax_efficiency[GlossaryCore.CO2TaxEfficiencyValue].values / 100.0  # %
    ...
    def d_energy_investment_d_co2_tax(self):
        ...
        dren_investments = self.d_renewable_investments_d_co2_tax(energy_investment_wo_tax)
        ...
        if self.invest_co2_tax_in_renawables:
            for i in range(nb_years):
                ...
                if ren_investments[i] > co2_invest_limit * energy_investment_wo_tax[i] and ren_investments[i] != 0.0:
                    ...
                    v_prime = (
                        d_ren_investments_dco2_taxes[i]
                        * co2_invest_limit
                        * energy_investment_wo_tax[i]
                        / (ren_investments[i] ** 2)
                    )
    ...
    def d_investment_d_user_input(self, d_net_output_d_user_input):
        ...
        d_non_energy_investment_d_user_input = (
            np.diag(self.share_non_energy_investment.values) @ d_net_output_d_user_input
        )
    ...
    def d_consumption_per_capita_d_user_input(self, d_consumption_d_user_input):
        ...
        """
        d_consumption_per_capita_d_consumption = np.diag(
            1 / self.population_df[GlossaryCore.PopulationValue].values * 1000
        )
    ...
    def d_gross_output_d_damage_frac_output(self):
        ...
        d_Y_d_dfo = (
            d_gross_output_d_productivity @ d_productivity_d_damage_frac_output
            if self.compute_gdp
            else self._null_derivative()
        )
        productivity = self.economics_detail_df[GlossaryCore.Productivity].values
        ...
        d_dQ_dY_d_dfo = (
            damefrac * 0 - 1.0
            if not self.damage_to_productivity
            else ((self.frac_damage_prod - 1) / ((1 - self.frac_damage_prod * damefrac) ** 2))
        )
        ...
        for i in range(1, self.nb_per):
            for j in range(i):
                ...
                d_Kne_d_dfo[i, j] = (1 - self.depreciation_capital) * d_Kne_d_dfo[
                    i - 1, j
                ...
                d_Y_d_dfo[i, j] += (
                    ...
                    * (
                        capital_u[i] ** (gamma - 1)
                        * (alpha * capital_u[i] ** gamma + (1 - alpha) * working_pop[i] ** gamma) ** (1 / gamma - 1)
                    )
                    if self.compute_gdp
                    else 0.0
                )
        ...
        d_lower_bound_constraint_d_dfo = (
            (d_Ku_d_dfo - self.capital_utilisation_ratio * d_Kne_d_dfo) / self.usable_capital_ref
            if not self.compute_gdp
            else self._null_derivative()
        )
        dKunbouded_d_E = self._null_derivative()
        d_Ku_obj_d_dfo = np.sum(
            2
            * (dKunbouded_d_E - self.capital_utilisation_ratio * d_Kne_d_dfo)
            * self.usable_cap_sqrt.reshape(-1, 1)
            / (self.usable_capital_objective_ref * self.nb_years),
            axis=0,
        )
    ...
    def d_net_output_d_damage_frac_output(self, d_gross_output_d_damage_frac_output):
        ...
        for i in range(nb_years):
            ...
            damage_frac_output = self.damage_fraction_output_df.loc[
                self.years_range[i], GlossaryCore.DamageFractionOutput
            ]
            for j in range(0, i + 1):
                if i == j:
                    if self.damage_to_productivity:
                        d_net_output_d_damage_frac_output[i, j] = (self.frac_damage_prod - 1) / (
                            (self.frac_damage_prod * damage_frac_output - 1) ** 2
                        ) * gross_output + (1 - damage_frac_output) / (
                            1 - self.frac_damage_prod * damage_frac_output
                        ) * d_gross_output_d_damage_frac_output[
                            i, j
                        ]
                    else:
                        d_net_output_d_damage_frac_output[i, j] = (
                            -gross_output + (1 - damage_frac_output) * d_gross_output_d_damage_frac_output[i, j]
                        )
                else:
                    if self.damage_to_productivity:
                        d_net_output_d_damage_frac_output[i, j] = (
                            (1 - damage_frac_output)
                            / (1 - self.frac_damage_prod * damage_frac_output)
                            * d_gross_output_d_damage_frac_output[i, j]
                        )
                    else:
                        d_net_output_d_damage_frac_output[i, j] = (
                            1 - damage_frac_output
            ...
            for i in range(nb_years):
                if ren_investments[i] > co2_invest_limit * energy_investment_wo_tax[i] and ren_investments[i] != 0.0:
                    ...
                    f_prime = (
                        ...
                        * energy_investment_wo_tax[i]
                        / 10.0
                        * (co2_invest_limit * energy_investment_wo_tax[i] / ren_investments[i] ** 2)
    ...
    def grad_energy_wasted_objective(self, grad_sum_energy_wasted, grad_sum_energy_total):
        ...
        grad_energy_wasted_obj = (
            sum_etotal * grad_sum_energy_wasted - sum_ewasted * grad_sum_energy_total
        ) / sum_etotal**2
