...
class CarbonCycle:
    ...
    def compute_atmo_conc(self, year):
        ...
        """
        p_atmo_conc = self.carboncycle_df.at[year - self.time_step, "atmo_conc"]
        p_shallow_ocean_conc = self.carboncycle_df.at[year - self.time_step, "shallow_ocean_conc"]
        p_emissions = self.CO2_emissions_df.at[year - self.time_step, "total_emissions"]
        atmo_conc = (
            p_atmo_conc * self.b_eleven
            + p_shallow_ocean_conc * self.b_twentyone
            + p_emissions * self.time_step / self.gtco2_to_gtc
        )
        # Lower bound
    ...
    def compute_lower_ocean_conc(self, year):
        ...
        """
        p_lower_ocean_conc = self.carboncycle_df.at[year - self.time_step, "lower_ocean_conc"]
        p_shallow_ocean_conc = self.carboncycle_df.at[year - self.time_step, "shallow_ocean_conc"]
    ...
    def compute_atmo_share(self, year):
        ...
        init_cum_total_emissions = self.CO2_emissions_df.at[self.year_start, "cum_total_emissions"]
        cum_total_emissions = self.CO2_emissions_df.at[year, "cum_total_emissions"]
    ...
    def compute_d_total_emissions(self):
        ...
        atmo_conc = self.carboncycle_df["atmo_conc"] / self.scale_factor_carbon_cycle
        # ---- initialisation
        ...
        if self.carboncycle_df["shallow_ocean_conc"].values[2] > self.lo_mu:
            d_swallow_d_totalemissions[2, 0] = time_step / gtco2_to_gtc * self.b_twelve
            d_atmoconc_d_totalemissions[2, 0] = time_step / gtco2_to_gtc * b_eleven
        ...
        for i in range(3, len(years)):
            for j in range(0, i):
                ...
                if np.real(self.carboncycle_df["lower_ocean_conc"].values[i]) > self.lo_ml:
                    ...
                    d_lower_d_totalemissions[i, j] = (
                        d_lower_d_totalemissions[i - 1, j] * b_thirtythree
                        + d_swallow_d_totalemissions[i - 1, j] * self.b_twentythree
                    )
                ...
                if np.real(self.carboncycle_df["shallow_ocean_conc"].values[i]) > self.lo_mu:
                    d_swallow_d_totalemissions[i, j] = (
                        d_atmoconc_d_totalemissions[i - 1, j] * self.b_twelve
                ...
                if np.real(atmo_conc.values[i]) > lo_mat:
                    if j == i - 1:
                        d_atmoconc_d_totalemissions[i, j] = (
                            d_atmoconc_d_totalemissions[i - 1, j] * b_eleven
                            + d_swallow_d_totalemissions[i - 1, j] * b_twentyone
                            + time_step / gtco2_to_gtc
                        )
                    else:
                        d_atmoconc_d_totalemissions[i, j] = (
                            d_atmoconc_d_totalemissions[i - 1, j] * b_eleven
        ...
        for i in range(0, len(years)):
            for j in range(0, len(years)):
                ...
                Cte = cum_total_emissions[self.year_start + time_step * j] + 0.000001
                d_atmo1850_dtotalemission[j, i] = d_atmoconc_d_totalemissions[j, i] / Cte
        ...
        for i in range(0, len(years)):
            for j in range(1, len(years)):
                ...
                Cte = cum_total_emissions[self.year_start + time_step * j] - init_cum_total_emissions
                d_atmotoday_dtotalemission[j, i] = d_atmoconc_d_totalemissions[j, i] / Cte

                d_atmotoday_dtotalemission[0, j] = 0

        return (
            d_atmoconc_d_totalemissions * self.scale_factor_carbon_cycle,
    ...
    def compute_d_cum_total_emissions(self):
        ...
        for i in range(1, len(years)):
            d_atmo1850_dcumemission[i, i] = (
                -(atmo_conc[years[i]] - 588.0) / (cum_total_emissions[years[i]] + 0.000001) ** 2
            )
        ...
        for i in range(1, len(years)):
            d_atmotoday_dcumtotalemission[i, i] = (
                -(self.carboncycle_df["atmo_conc"][self.year_start + self.time_step * i] - init_atmo_conc)
                / (cum_total_emissions[self.year_start + self.time_step * i] - init_cum_total_emissions) ** 2
            )

            d_atmotoday_dcumtotalemission[i, 0] = -d_atmotoday_dcumtotalemission[i, i]
    ...
    def compute_d_ppm(self, d_atmoconc_d_totalemissions):
        ...
        d_ppm = d_atmoconc_d_totalemissions / self.gtc_to_ppm / self.scale_factor_carbon_cycle
        return d_ppm

    def compute_d_objective(self, d_ppm):
        delta_years = len(self.carboncycle_df)
        d_ppm_objective = (1 - self.alpha) * (1 - self.beta) * d_ppm.sum(axis=0) / (self.ppm_ref * delta_years)
    ...
    def compute_objective(self):
        ...
        """
        delta_years = len(self.carboncycle_df)
        self.ppm_obj = np.asarray(
            [(1 - self.alpha) * (1 - self.beta) * self.carboncycle_df["ppm"].sum() / (self.ppm_ref * delta_years)]
        )
    ...
    def compute_rockstrom_limit_constraint(self):
        ...
        """
        self.rockstrom_limit_constraint = (
            -(self.carboncycle_df["ppm"].values - 1.1 * self.rockstrom_limit) / self.rockstrom_constraint_ref
        )
    ...
    def compute_minimum_ppm_limit_constraint(self):
        ...
        self.minimum_ppm_constraint = (
            -(self.minimum_ppm_limit - self.carboncycle_df["ppm"].values) / self.minimum_ppm_constraint_ref
        )
    ...
    def compute(self, inputs_models):
        ...
        self.carboncycle_df["atmo_conc"] = self.carboncycle_df["atmo_conc"] * self.scale_factor_carbon_cycle
