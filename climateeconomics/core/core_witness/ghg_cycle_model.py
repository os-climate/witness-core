...
class GHGCycle:
    ...
    def set_data(self):
        ...
        self.pred_indus_gwp20 = self.total_co2_equivalent(
            co2_conc=self.pre_indus_conc_co2,
            ch4_conc=self.pre_indus_conc_ch4,
            n2o_conc=self.pre_indus_conc_n2o,
            gwp=self.gwp_20,
        )
        self.pred_indus_gwp100 = self.total_co2_equivalent(
            co2_conc=self.pre_indus_conc_co2,
            ch4_conc=self.pre_indus_conc_ch4,
            n2o_conc=self.pre_indus_conc_n2o,
            gwp=self.gwp_100,
    ...
    def compute_co2_atm_conc(self, year, boxes):
        ...
        """
        emissions = (
            self.ghg_emissions_df.loc[
                self.ghg_emissions_df[GlossaryCore.Years] == year, GlossaryCore.TotalCO2Emissions
            ].values[0]
            * 1e3
        )  # in MtCO2
        boxes_tmp = [
            decay * box_conc + 0.000471 * em_ratio * emissions
            for (decay, box_conc, em_ratio) in zip(self.decays, boxes, self.em_ratios)
        ]
        boxes = boxes_tmp
        for i in [1, 2, 3, 4, 5]:
    ...
    def d_gwp100_objective_d_ppm(self, d_ppm: pd.Series, specie: str) -> float:
        ...
        """
        return (
            self.d_total_co2_equivalent_d_conc(d_conc=d_ppm, specie=specie, gwp=self.gwp_100).mean(axis=0)
            / self.pred_indus_gwp100
        )
    ...
    def d_gwp20_objective_d_ppm(self, d_ppm: pd.Series, specie: str) -> float:
        ...
        """
        return (
            self.d_total_co2_equivalent_d_conc(d_conc=d_ppm, specie=specie, gwp=self.gwp_20).mean(axis=0)
            / self.pred_indus_gwp20
        )
    ...
    def compute_objectives(self):
        ...
        """
        gwp100_all_years: pd.Series = self.total_co2_equivalent(
            co2_conc=self.ghg_cycle_df[GlossaryCore.CO2Concentration],
        ...
        gwp20_all_years: pd.Series = self.total_co2_equivalent(
            co2_conc=self.ghg_cycle_df[GlossaryCore.CO2Concentration],
            ch4_conc=self.ghg_cycle_df[GlossaryCore.CH4Concentration],
            n2o_conc=self.ghg_cycle_df[GlossaryCore.N2OConcentration],
            gwp=self.gwp_20,
    ...
    def compute_rockstrom_limit_constraint(self):
        ...
        self.rockstrom_limit_constraint = (
            -(self.ghg_cycle_df[GlossaryCore.CO2Concentration].values - 1.1 * self.rockstrom_limit)
            / self.rockstrom_constraint_ref
        )
    ...
    def compute_minimum_ppm_limit_constraint(self):
        ...
        self.minimum_ppm_constraint = (
            -(self.minimum_ppm_limit - self.ghg_cycle_df[GlossaryCore.CO2Concentration].values)
            / self.minimum_ppm_constraint_ref
        )
    ...
    def total_co2_equivalent(
        ...
        total_mass_co2_eq = (
            ch4_total_mass * gwp[GlossaryCore.CH4]
            + n2o_total_mass * gwp[GlossaryCore.N2O]
            + co2_total_mass * gwp[GlossaryCore.CO2]
        )
    ...
    def compute_extra_CO2_eq_Gt(self):
        ...
        CO2_eq_20y = self.total_co2_equivalent(
            co2_conc=self.ghg_cycle_df[GlossaryCore.CO2Concentration],
            ch4_conc=self.ghg_cycle_df[GlossaryCore.CH4Concentration],
            n2o_conc=self.ghg_cycle_df[GlossaryCore.N2OConcentration],
            gwp=self.gwp_20,
        ...
        CO2_eq_100y = self.total_co2_equivalent(
            co2_conc=self.ghg_cycle_df[GlossaryCore.CO2Concentration],
            ch4_conc=self.ghg_cycle_df[GlossaryCore.CH4Concentration],
            n2o_conc=self.ghg_cycle_df[GlossaryCore.N2OConcentration],
            gwp=self.gwp_100,
    ...
    def compute_global_warming_potentials(self):
        global_warming_potential_df = pd.DataFrame({GlossaryCore.Years: self.years_range})
        for year_basis, gwp in zip([GlossaryCore.YearBasis20, GlossaryCore.YearBasis100], [self.gwp_20, self.gwp_100]):
            to_sum = []
            for ghg, ghg_conc_name in zip(
                GlossaryCore.GreenHouseGases,
    ...
    def compute_concentration_ch4(self):
        ch4_concentrations = self._forecast_concentration(
            conc_init=self.init_conc_ch4,
            decay_rate=self.decay_ch4,
            conc_pre_indus=self.pre_indus_conc_ch4,
            emissions_to_pp=self.gt_to_pp[GlossaryCore.CH4],
            emissions=self.ghg_emissions_df[GlossaryCore.TotalCH4Emissions].values,
        )

        self.ghg_cycle_df[GlossaryCore.CH4Concentration] = ch4_concentrations

    def compute_concentration_n2o(self):
        n2o_concentrations = self._forecast_concentration(
            conc_init=self.init_conc_n2o,
            decay_rate=self.decay_n2o,
            conc_pre_indus=self.pre_indus_conc_n2o,
            emissions_to_pp=self.gt_to_pp[GlossaryCore.N2O],
            emissions=self.ghg_emissions_df[GlossaryCore.TotalN2OEmissions].values,
        )

        self.ghg_cycle_df[GlossaryCore.N2OConcentration] = n2o_concentrations

    def _forecast_concentration(
        self, conc_init: float, decay_rate: float, conc_pre_indus: float, emissions_to_pp: float, emissions: np.ndarray
    ):
    ...
    def d_conc_ch4_d_emissions(self):
        return self.d_conc_d_emission(decay_rate=self.decay_ch4, emissions_to_pp=self.gt_to_pp[GlossaryCore.CH4])

    def d_conc_n2o_d_emissions(self):
        return self.d_conc_d_emission(decay_rate=self.decay_n2o, emissions_to_pp=self.gt_to_pp[GlossaryCore.N2O])
