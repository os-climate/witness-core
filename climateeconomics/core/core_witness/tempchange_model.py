...
class TempChange(object):
    ...
    def set_data(self, inputs):
        ...
        if inputs[GlossaryCore.CarbonCycleDfValue] is not None:
            self.carboncycle_df = pd.DataFrame(
                {
                    GlossaryCore.Years: inputs[GlossaryCore.CarbonCycleDfValue][GlossaryCore.Years].values,
    ...
    def compute_exog_forcing_dice(self):
        ...
        exog_forcing = np.linspace(self.init_forcing_nonco, self.hundred_forcing_nonco, len(self.years_range))
    ...
    def compute_exog_forcing_myhre(self, atmo_conc):
        ...
        def MN(c1, c2):
            return 0.47 * np.log(1 + 2.01e-5 * (c1 * c2) ** (0.75) + 5.31e-15 * c1 * (c1 * c2) ** (1.52))

        exog_forcing = (
            0.036 * (np.sqrt(ch4_conc) - np.sqrt(self.ch4_conc_init_ppm))
            - MN(ch4_conc, self.n2o_conc_init_ppm)
            + 0.12 * (np.sqrt(n2o_conc) - np.sqrt(self.n2o_conc_init_ppm))
            - MN(self.ch4_conc_init_ppm, n2o_conc)
    ...
    def compute_forcing_etminan(self, atmo_conc):
        ...
        co2_forcing = (
            -2.4e-7 * (atmo_conc_ppm - self.c0_ppm) ** 2
            + 7.2e-4 * sign_values * (atmo_conc_ppm - self.c0_ppm)
            - 2.1e-4 * n2omean
            + self.forcing_eq_co2 / np.log(2)
        ) * np.log(atmo_conc_ppm / self.c0_ppm)
        ch4_forcing = (-1.3e-6 * ch4mean - 8.2e-6 * n2omean + 0.043) * (
            np.sqrt(ch4_conc) - np.sqrt(self.ch4_conc_init_ppm)
        )
    ...
    def compute_forcing_meinshausen(self, atmo_conc):
        ...
        alphap = d1 + a1 * (atmo_conc_ppm - self.c0_ppm) ** 2 + b1 * (atmo_conc_ppm - self.c0_ppm)
        ...
        # CH4
        ch4_forcing = (a3 * np.sqrt(ch4_conc) + b3 * np.sqrt(n2o_conc) + d3) * (
            np.sqrt(ch4_conc) - np.sqrt(self.ch4_conc_init_ppm)
        )

        self.forcing_df["CH4 forcing"] = ch4_forcing
        # N2O
        n2o_forcing = (a2 * np.sqrt(atmo_conc_ppm) + b2 * np.sqrt(n2o_conc) + c2 * np.sqrt(ch4_conc) + d2) * (
            np.sqrt(n2o_conc) - np.sqrt(self.n2o_conc_init_ppm)
        )
    ...
    def compute_log_co2_forcing(self, atmo_conc):
        ...
        co2_forcing = self.forcing_eq_co2 / np.log(2) * np.log(atmo_conc / (self.c0_ppm * self.ppm_to_gtc))
    ...
    def compute_temp_atmo(self, year):
        ...
        """
        p_temp_atmo = self.temperature_df.at[year - self.time_step, GlossaryCore.TempAtmo]
        p_temp_ocean = self.temperature_df.at[year - self.time_step, GlossaryCore.TempOcean]
        ...
        temp_atmo = p_temp_atmo + (self.climate_upper / (5.0 / self.time_step)) * (
            ...
            - ((self.transfer_upper / (5.0 / self.time_step)) * (p_temp_atmo - p_temp_ocean))
        )
        # Lower bound
    ...
    def compute_temp_ocean(self, year):
        ...
        p_temp_ocean = self.temperature_df.at[year - self.time_step, GlossaryCore.TempOcean]
        p_temp_atmo = self.temperature_df.at[year - self.time_step, GlossaryCore.TempAtmo]
        temp_ocean = p_temp_ocean + (self.transfer_lower / (5.0 / self.time_step)) * (p_temp_atmo - p_temp_ocean)
        # Bounds
    ...
    def compute_temperature_year_end_constraint(self):
        ...
        self.temperature_end_constraint = np.array(
            [(self.temperature_end_constraint_limit - temp_ocean_year_end) / self.temperature_end_constraint_ref]
        )
    ...
    def compute_dlog_co2_forcing(self, atmo_conc):
        ...
        d_forcing = self.forcing_eq_co2 / (np.log(2) * atmo_conc)
    ...
    def compute_dexog_forcing_myhre(self, atmo_conc):
        ...
        def dMN_dc1c2(c1, c2, x):
            ...
            if x == "c1":
                f = 1 + 2.01e-5 * (c1 * c2) ** (0.75) + 5.31e-15 * c1 * (c1 * c2) ** (1.52)
                fprime = 2.01e-5 * c2 ** (0.75) * 0.75 * c1 ** (0.75 - 1) + 5.31e-15 * c2 ** (1.52) * 2.52 * c1 ** (
                    2.52 - 1
                )
            elif x == "c2":

                f = 1 + 2.01e-5 * (c1 * c2) ** (0.75) + 5.31e-15 * c1 * (c1 * c2) ** (1.52)
                fprime = 2.01e-5 * c1 ** (0.75) * 0.75 * c2 ** (0.75 - 1) + 5.31e-15 * c1 ** (2.52) * 1.52 * c2 ** (
                    1.52 - 1
                )
        ...
        dexog_forcing = (
            0.036 * dch4_conc / (2 * np.sqrt(ch4_conc))
    ...
    def compute_dforcing_etminan(self, atmo_conc):
        ...
        dco2_forcing = (
            -2.4e-7 * datmo_conc_ppm * 2.0 * (atmo_conc_ppm - self.c0_ppm)
            + sign_values * 7.2e-4 * datmo_conc_ppm
            - 2.1e-4 * dn2omean
        ) * np.log(atmo_conc_ppm / self.c0_ppm) + datmo_conc_ppm / atmo_conc_ppm * (
            -2.4e-7 * (atmo_conc_ppm - self.c0_ppm) ** 2
            + sign_values * 7.2e-4 * (atmo_conc_ppm - self.c0_ppm)
            - 2.1e-4 * n2omean
            + self.forcing_eq_co2 / np.log(2)
        )

        dch4_forcing = (-1.3e-6 * dch4mean - 8.2e-6 * dn2omean) * (
            np.sqrt(ch4_conc) - np.sqrt(self.ch4_conc_init_ppm)
        ) + (-1.3e-6 * ch4mean - 8.2e-6 * n2omean + 0.043) * dch4_conc / (2.0 * np.sqrt(ch4_conc))

        dn2o_forcing = (-8.0e-6 * dco2mean + 4.2e-6 * dn2omean - 4.9e-6 * dch4mean) * (
            np.sqrt(n2o_conc) - np.sqrt(self.n2o_conc_init_ppm)
        ) + (-8.0e-6 * co2mean + 4.2e-6 * n2omean - 4.9e-6 * ch4mean + 0.117) * dn2o_conc / (2.0 * np.sqrt(n2o_conc))
    ...
    def compute_dforcing_meinshausen(self, atmo_conc):
        ...
        dalphap = a1 * datmo_conc_ppm * 2.0 * (atmo_conc_ppm - self.c0_ppm) + b1 * datmo_conc_ppm
        ...
        dco2_forcing = (dalphap + dalphaN2O) * np.log(atmo_conc_ppm / self.c0_ppm) + (
            alphap + alphaN2O
        ) * datmo_conc_ppm / atmo_conc_ppm

        # CH4
        dch4_forcing = (a3 * dch4_conc / (2.0 * np.sqrt(ch4_conc)) + b3 * dn2o_conc / (2.0 * np.sqrt(n2o_conc))) * (
            np.sqrt(ch4_conc) - np.sqrt(self.ch4_conc_init_ppm)
        ) + dch4_conc * (a3 * np.sqrt(ch4_conc) + b3 * np.sqrt(n2o_conc) + d3) / (2.0 * np.sqrt(ch4_conc))

        # N2O
        dn2o_forcing = (
            a2 * datmo_conc_ppm / (2.0 * np.sqrt(atmo_conc_ppm))
            + b2 * dn2o_conc / (2.0 * np.sqrt(n2o_conc))
            + c2 * dch4_conc / (2.0 * np.sqrt(ch4_conc))
        ) * (np.sqrt(n2o_conc) - np.sqrt(self.n2o_conc_init_ppm)) + (
            a2 * np.sqrt(atmo_conc_ppm) + b2 * np.sqrt(n2o_conc) + c2 * np.sqrt(ch4_conc) + d2
        ) * dn2o_conc / (
            2.0 * np.sqrt(n2o_conc)
        )
    ...
    def compute_d_temp_atmo(self):
        ...
        d_tempatmo_d_atmoconc = np.identity(nb_years) * self.climate_upper * self.time_step / 5.0 * dforcing_datmo_conc
        ...
        for i in range(2, nb_years):
            ...
            while j < i:
                ...
                d_tempatmo_d_atmoconc[i, j] = (
                    ...
                    * self.time_step
                    / 5.0
                    * (d_tempatmo_d_atmoconc[i - 1, j] - d_tempocean_d_atmoconc[i - 1, j])
                )
                ...
                d_tempocean_d_atmoconc[i, j] = d_tempocean_d_atmoconc[
                    i - 1, j
        ...
        return (
            d_tempatmo_d_atmoconc / self.scale_factor_carbon_cycle,
            d_tempocean_d_atmoconc / self.scale_factor_carbon_cycle,
        )
    ...
    def compute_d_temp_atmo_objective(self):
        ...
        if self.temperature_obj_option == TempChange.LAST_TEMPERATURE_OBJECTIVE:
            ...
            result[0] = (1 - self.beta) * (1 - self.alpha) / self.temperature_change_ref
            result[-1] = (1 - self.beta) * (1 - self.alpha) / self.temperature_change_ref
            ...
            dnn = (1 - self.beta) * (1 - self.alpha) / (self.temperature_change_ref * delta_years)
    ...
    def compute(self, in_dict):
        ...
        self.carboncycle_df = pd.DataFrame(
            {
                GlossaryCore.Years: in_dict[GlossaryCore.CarbonCycleDfValue][GlossaryCore.Years].values,
        ...
        if self.temperature_obj_option == TempChange.LAST_TEMPERATURE_OBJECTIVE:
            self.temperature_objective = np.asarray(
                [(1 - self.beta) * (1 - self.alpha) * (temperature_df_values[-1]) / (self.temperature_change_ref)]
            )
            ...
            self.temperature_objective = np.asarray(
                [
                    (1 - self.beta)
                    * (1 - self.alpha)
                    * self.temperature_df[GlossaryCore.TempAtmo].sum()
