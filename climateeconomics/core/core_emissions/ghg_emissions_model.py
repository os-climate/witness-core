...
class GHGEmissions:
    ...
    def __init__(self, param):
        ...
        self.epsilon = 1.0e-5  # for the CO2 objective function
    ...
    def configure_parameters(self):
        ...
        self.economic_sectors_except_agriculture = [
            sector for sector in self.new_sector_list if sector != GlossaryCore.SectorAgriculture
        ]
    ...
    def create_dataframe(self):
        ...
        self.years_range = np.arange(year_start, year_end + 1, self.time_step)
    ...
    def compute_land_emissions(self):
        ...
        self.ghg_emissions_df[GlossaryCore.insertGHGAgriLandEmissions.format(GlossaryCore.CO2)] = (
            self.CO2_land_emissions.drop(GlossaryCore.Years, axis=1).sum(axis=1).values
        )
        self.ghg_emissions_df[GlossaryCore.insertGHGAgriLandEmissions.format(GlossaryCore.CH4)] = (
            self.CH4_land_emissions.drop(GlossaryCore.Years, axis=1).sum(axis=1).values
        )
        self.ghg_emissions_df[GlossaryCore.insertGHGAgriLandEmissions.format(GlossaryCore.N2O)] = (
            self.N2O_land_emissions.drop(GlossaryCore.Years, axis=1).sum(axis=1).values
        )
    ...
    def compute_total_emissions(self):
        ...
        for ghg in self.GHG_TYPE_LIST:
            self.ghg_emissions_df[GlossaryCore.insertGHGNonEnergyEmissions.format(ghg)] = 0.0
        self.ghg_emissions_df[GlossaryCore.insertGHGNonEnergyEmissions.format(GlossaryCore.CO2)] = (
            self.total_economics_emisssions[GlossaryCore.NonEnergyEmissions].values
        )
    ...
    def compute_total_gwp(self):
        ...
        for ghg in self.GHG_TYPE_LIST:
            ...
            self.gwp_emissions[f"{ghg}_20"] = (
                self.ghg_emissions_df[GlossaryCore.insertGHGTotalEmissions.format(ghg)] * self.gwp_20[ghg]
            )
            self.gwp_emissions[f"{ghg}_100"] = (
                self.ghg_emissions_df[GlossaryCore.insertGHGTotalEmissions.format(ghg)] * self.gwp_100[ghg]
            )

        self.gwp_emissions[f"Total GWP (20-year basis)"] = self.gwp_emissions[
            [f"{ghg}_20" for ghg in self.GHG_TYPE_LIST]
        ].sum(axis=1)
        self.gwp_emissions[f"Total GWP (100-year basis)"] = self.gwp_emissions[
            [f"{ghg}_100" for ghg in self.GHG_TYPE_LIST]
        ].sum(axis=1)
        ...
        if self.affine_co2_objective:
            self.co2_emissions_objective = np.array(
                [
                    ...
                    / (20.0 * self.CO2EmissionsRef)
                ]
            )
        else:
            self.co2_emissions_objective = np.array(
                [
                    (np.sqrt((annual_co2_emissions_ref + mean_annual_co2_emissions) ** 2 + epsilon**2) - epsilon)
                    / (2.0 * annual_co2_emissions_ref)
                ]
            )
    ...
    def d_CO2_emissions_objective_d_total_co2_emissions(self):
        ...
        if self.affine_co2_objective:
            d_CO2_emissions_objective_d_total_co2_emissions = (
                np.ones(len(self.years_range)) / len(self.years_range) / (20.0 * self.CO2EmissionsRef)
            )
        else:
            epsilon = self.epsilon
            d_CO2_emissions_objective_d_total_co2_emissions = np.array(
                [
                    dCO2
                    * (annual_co2_emissions_ref + mean_annual_co2_emissions)
                    / np.sqrt((annual_co2_emissions_ref + mean_annual_co2_emissions) ** 2 + epsilon**2)
                    / (2.0 * annual_co2_emissions_ref)
                ]
            )
        ...
        self.total_energy_co2eq_emissions = pd.DataFrame(
            {
                ...
                GlossaryCore.TotalEnergyEmissions: self.GHG_total_energy_emissions[columns_to_sum]
                .multiply(self.gwp_100.values())
                .sum(axis=1),
            }
        )
    ...
    def compute_energy_emission_per_section(self):
        ...
        for sector in self.new_sector_list:
            section_energy_emissions = {GlossaryCore.Years: self.years_range}
            energy_sections_consumptions_consumptions_of_sector = self.param[
                f"{sector}.{GlossaryCore.SectionEnergyConsumptionDfValue}"
            ]
            for section in GlossaryCore.SectionDictSectors[sector]:
                section_energy_emissions[section] = (
                    energy_sections_consumptions_consumptions_of_sector[section].values * carbon_intensity
                )
    ...
    def compute_non_energy_emission_per_section(self):
        ...
        for sector in self.new_sector_list:
            section_non_energy_emissions = {GlossaryCore.Years: self.years_range}
            energy_sections_non_energy_emissions_gdp_of_sector = self.param[
                f"{sector}.{GlossaryCore.SectionNonEnergyEmissionGdpDfValue}"
            ]
            sector_sections_gdp = self.param[f"{sector}.{GlossaryCore.SectionGdpDfValue}"]
            for section in GlossaryCore.SectionDictSectors[sector]:
                section_non_energy_emissions[section] = (
                    energy_sections_non_energy_emissions_gdp_of_sector[section].values
                    * sector_sections_gdp[section].values
                    / 1000.0
                )
    ...
    def compute_total_emission_per_section(self):
        ...
        """
        for sector in self.new_sector_list:
            sections_emissions = {GlossaryCore.Years: self.years_range}
            for section in GlossaryCore.SectionDictSectors[sector]:
                sections_emissions[section] = (
                    self.dict_sector_sections_energy_emissions[sector][section].values
    ...
    def compute_total_emission_sectors(self):
        ...
        for sector in self.new_sector_list:
            ...
            sector_emissions[GlossaryCore.EnergyEmissions] = self.dict_sector_sections_energy_emissions[sector][
                sector_sections
            ].values.sum(axis=1)
            sector_emissions[GlossaryCore.NonEnergyEmissions] = self.dict_sector_sections_non_energy_emissions[sector][
                sector_sections
            ].values.sum(axis=1)
            sector_emissions[GlossaryCore.TotalEmissions] = (
                sector_emissions[GlossaryCore.EnergyEmissions] + sector_emissions[GlossaryCore.NonEnergyEmissions]
            )
    ...
    def compute_total_economics_emission(self):
        ...
        total_economics_emisssions[GlossaryCore.EnergyEmissions] = np.sum(
            [
                self.dict_sector_emissions[sector][GlossaryCore.EnergyEmissions]
                for sector in self.economic_sectors_except_agriculture
            ],
            axis=0,
        )
        total_economics_emisssions[GlossaryCore.NonEnergyEmissions] = np.sum(
            [
                self.dict_sector_emissions[sector][GlossaryCore.NonEnergyEmissions]
                for sector in self.economic_sectors_except_agriculture
            ],
            axis=0,
        )
        total_economics_emisssions[GlossaryCore.TotalEmissions] = np.sum(
            [
                self.dict_sector_emissions[sector][GlossaryCore.TotalEmissions]
                for sector in self.economic_sectors_except_agriculture
            ],
            axis=0,
        )
    ...
    def compute_carbon_intensity_of_energy_mix(self):
        ...
        """
        total_energy_emissions = self.total_energy_co2eq_emissions[
            GlossaryCore.TotalEnergyEmissions
        ].values  # GtCO2Eq : 1e12 kgCO2Eq
        total_energy_production = self.total_energy_production[
        ...
        self.carbon_intensity_of_energy_mix = pd.DataFrame(
            {GlossaryCore.Years: self.years_range, GlossaryCore.EnergyCarbonIntensityDfValue: carbon_intensity}
        )
    ...
    def d_section_energy_emissions_d_user_input(
        self, d_carbon_intensity_d_user_input, sector_name: str, section_name: str
    ):
        return (
            np.diag(self.param[f"{sector_name}.{GlossaryCore.SectionEnergyConsumptionDfValue}"][section_name].values)
            * d_carbon_intensity_d_user_input
        )
    ...
    def d_section_non_energy_emissions_d_gdp_section(self, sector: str, section: str):
        ...
        return np.diag(self.param[f"{sector}.{GlossaryCore.SectionNonEnergyEmissionGdpDfValue}"][section] / 1000.0)
    ...
    def compute_energy_mix_total_emissions(self):
        for ghg in self.GHG_TYPE_LIST:
            self.ghg_emissions_df[GlossaryCore.insertGHGEnergyEmissions.format(ghg)] = self.GHG_total_energy_emissions[
    ...
    def compute_gwp_per_sector(self):
        ...
        emission_types = {
            GlossaryCore.AgricultureAndLandUse: GlossaryCore.insertGHGAgriLandEmissions,
    ...
    def compute_energy_emission_households(self):
        ...
        energy_emission_households = (
            self.residential_energy_consumption[GlossaryCore.TotalProductionValue].values
            * self.carbon_intensity_of_energy_mix[GlossaryCore.EnergyCarbonIntensityDfValue].values
        )

        self.energy_emission_households_df = pd.DataFrame(
            {GlossaryCore.Years: self.years_range, GlossaryCore.TotalEmissions: energy_emission_households}
        )
