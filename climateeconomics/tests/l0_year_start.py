...
class YearStartTest(unittest.TestCase):
    ...
    def _test_year_witness_core(self, year: int) -> list[str]:
        ...
        is_available = DatabaseWitnessCore.get_forest_invest_before_year_start(year_start=year, construction_delay=3, is_available_at_year=True)
        if not is_available:
    ...
    def _test_year_witness_energy(self, year: int, techno_dict: dict) -> list[str]:
        ...
        for techno_name in techno_list:
            is_available = DatabaseWitnessEnergy.get_techno_prod(techno_name=techno_name, year=year, is_available_at_year=True)
            if not is_available:
