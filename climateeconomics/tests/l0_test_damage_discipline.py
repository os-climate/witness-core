...
class DamageDiscTest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.temperature_df = pd.DataFrame(
            {
                ...
                "exog_forcing": np.linspace(0.5, 1.0, len(self.years)),
                "forcing": np.linspace(2.46, 3.02, len(self.years)),
                "temp_atmo": np.linspace(0.85, 2.25, len(self.years)),
                "temp_ocean": np.linspace(0.006, 0.61, len(self.years)),
            }
        )
        ...
        self.damage_constraint_factor = np.concatenate(
