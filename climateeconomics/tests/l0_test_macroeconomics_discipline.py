...
class MacroDiscTest(unittest.TestCase):
    ...
    def test_execute(self):
        ...
        energy_investment_wo_tax = DataFrame(
            {GlossaryCore.Years: years, GlossaryCore.EnergyInvestmentsWoTaxValue: np.asarray([3.5] * nb_per)}
        )  # in G$

        share_non_energy_investment = DataFrame(
            {GlossaryCore.Years: years, GlossaryCore.ShareNonEnergyInvestmentsValue: np.asarray([27.0 - 2.6] * nb_per)}
        )
        ...
        default_CO2_tax = pd.DataFrame({GlossaryCore.Years: years, GlossaryCore.CO2Tax: 50.0}, index=years)
        ...
        self.working_age_pop_df = pd.DataFrame(
            {GlossaryCore.Years: self.years, GlossaryCore.Population1570: np.linspace(5490, 6061, len(self.years))}
        )
