...
class AgricultureMixModelTestCase(unittest.TestCase):
        ...
        self.land_use_required_crop = pd.DataFrame(
            {
                ...
                "Crop (Gha)": land_use_required_energy,
                "Crop for Food (Gha)": land_use_required_food,
            }
        )
        # ----------------------------------------------------
    ...
    def test_agriculture_mix_discipline(self):
        ...
        inputs_dict = {
            f"{test_name}.{GlossaryCore.YearStart}": self.year_start,
        ...
        ee.execute()
        disc = ee.dm.get_disciplines_with_name(f"{test_name}.{disc_name}")[0]
