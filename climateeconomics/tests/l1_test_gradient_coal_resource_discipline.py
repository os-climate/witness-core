...
class CoalResourceJacobianDiscTest(AbstractJacobianUnittest):
    ...
    def analytic_grad_entry(self):
        return [self.test_coal_resource_analytic_grad, self.test_coal_resource_damand_variable_analytic_grad]
    ...
    def setUp(self):
        ...
        self.energy_coal_demand_df = self.energy_coal_demand_df.loc[
            self.energy_coal_demand_df[GlossaryCore.Years] >= self.year_start
        ]
        self.energy_coal_demand_df = self.energy_coal_demand_df.loc[
            self.energy_coal_demand_df[GlossaryCore.Years] <= self.year_end
        ]
        self.energy_coal_variable_demand_df = self.energy_coal_variable_demand_df.loc[
            self.energy_coal_variable_demand_df[GlossaryCore.Years] >= self.year_start
        ]
        self.energy_coal_variable_demand_df = self.energy_coal_variable_demand_df.loc[
            self.energy_coal_variable_demand_df[GlossaryCore.Years] <= self.year_end
        ]
    ...
    def test_coal_resource_analytic_grad(self):
        ...
        self.check_jacobian(
            ...
            filename=f"jacobian_coal_discipline.pkl",
    ...
    def test_coal_resource_damand_variable_analytic_grad(self):
        ...
        self.check_jacobian(
            location=dirname(__file__),
            filename=f"jacobian_coal_demand_variable_discipline.pkl",
            discipline=disc_techno,
            step=1e-15,
            derr_approx="complex_step",
            local_data=disc_techno.local_data,
