...
class ConsumptionJacobianDiscTest(AbstractJacobianUnittest):
    ...
    def setUp(self):
        ...
        self.residential_energy_df = pd.DataFrame(
            {GlossaryCore.Years: self.years, GlossaryCore.TotalProductionValue: np.linspace(200, 10, len(self.years))}
        )

        self.investment_df = pd.DataFrame(
            {GlossaryCore.Years: self.years, GlossaryCore.InvestmentsValue: np.full(len(self.years), 10.0)}
        )
    ...
    def test_01_consumption_analytic_grad_welfare(self):
        ...
        self.check_jacobian(
            ...
            filename=f"jacobian_consumption_discipline_welfare.pkl",
    ...
    def test_02_consumption_analytic_grad_last_utility(self):
        ...
        self.check_jacobian(
            location=dirname(__file__),
            filename=f"jacobian_consumption_discipline_last_utility.pkl",
            discipline=disc_techno,
            step=1e-15,
            local_data=disc_techno.local_data,
            inputs=[
    ...
    def test_03_consumption_with_low_economy(self):
        ...
        economics_df = self.economics_df[[GlossaryCore.Years, GlossaryCore.OutputNetOfDamage]]
        ...
        self.check_jacobian(
            location=dirname(__file__),
            filename=f"jacobian_consumption_low_economy.pkl",
            discipline=disc_techno,
            step=1e-15,
            local_data=disc_techno.local_data,
            inputs=[
