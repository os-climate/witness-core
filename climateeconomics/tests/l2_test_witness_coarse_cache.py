...
class WitnessCoarseCache(unittest.TestCase):
    ...
    def test_01_cache_on_witness_coarse_optim_with_unconverged_mda(self):
        ...
        for cache_type_key in self.ee.dm.get_all_namespaces_from_var_name(
            "cache_type"
        ) + self.ee.dm.get_all_namespaces_from_var_name("residuals_history"):
        ...
        optim_output_df_simple_cache = optim_output_df_simple_cache.iloc[-1].drop("iteration")
        optim_output_df_without_cache = dm_without_cache.pop(
            "Test.WITNESS_MDO.WITNESS_Eval.FunctionsManager.optim_output_df"
        )
        optim_output_df_without_cache = optim_output_df_without_cache.iloc[-1].drop("iteration")

        # compare values in dm_with_simple_cache and dm_without_cache
        dict_error = {}
        compare_dict(dm_with_simple_cache, dm_without_cache, "", dict_error)
        self.assertDictEqual(dict_error, {})
        self.assertTrue(optim_output_df_simple_cache.equals(optim_output_df_without_cache))
