...
class MacroEconomicsJacobianDiscTest(AbstractJacobianUnittest):
    ...
    def setUp(self):
        ...
        self.energy_supply_df = pd.DataFrame(
            {GlossaryCore.Years: self.years, GlossaryCore.TotalProductionValue: np.linspace(43, 76, len(self.years))}
        )

        self.share_residential_energy_consumption = pd.DataFrame(
            {GlossaryCore.Years: self.years, GlossaryCore.ShareSectorEnergy: 10.0}
        )

        self.default_co2_efficiency = pd.DataFrame({GlossaryCore.Years: self.years, "CO2_tax_efficiency": 40.0})
        ...
        self.energy_investment_wo_tax = pd.DataFrame(
            {GlossaryCore.Years: self.years, GlossaryCore.EnergyInvestmentsWoTaxValue: [3.5] * self.nb_per}
        )
        ...
        self.energy_capital = pd.DataFrame(
            {GlossaryCore.Years: self.years, GlossaryCore.Capital: 16.09 * (1.02 ** np.arange(len(self.years)))}
        )
        ...
        self.energy_supply_df_all = pd.DataFrame(
            {GlossaryCore.Years: self.years, GlossaryCore.TotalCO2Emissions: np.linspace(35, 0, len(self.years))}
        )

        self.co2_emissions_gt = pd.DataFrame(
            {GlossaryCore.Years: self.years, GlossaryCore.TotalCO2Emissions: np.linspace(34, 55, len(self.years))}
        )

        self.carbon_intensity_energy = pd.DataFrame(
            {GlossaryCore.Years: self.years, GlossaryCore.EnergyCarbonIntensityDfValue: 10.0}
        )
        ...
        for sector, default_value_consumption in default_value_energy_consumption_dict.items():
            section_gdp_percentage_dict = {
                ...
                **dict(zip(default_value_consumption.columns[1:], default_value_consumption.values[0, 1:])),
            }
        ...
        for sector, default_value_emissions in default_non_energy_emissions_dict.items():
            section_gdp_percentage_dict = {
                ...
                **dict(zip(default_value_emissions.columns[1:], default_value_emissions.values[0, 1:])),
            }
        ...
        self.inputs_dict = {
            ...
            f"{self.name}.{self.model_name}.{GlossaryCore.SectorEnergyConsumptionPercentageDfName}_Services": self.default_value_dict_consumption[
                "Services"
            ],
            f"{self.name}.{self.model_name}.{GlossaryCore.SectionNonEnergyEmissionGdpDfValue}_Services": self.default_value_dict_emissions[
                "Services"
            ],
            f"{self.name}.{self.model_name}.{GlossaryCore.SectorEnergyConsumptionPercentageDfName}_Agriculture": self.default_value_dict_consumption[
                "Agriculture"
            ],
            f"{self.name}.{self.model_name}.{GlossaryCore.SectionNonEnergyEmissionGdpDfValue}_Agriculture": self.default_value_dict_emissions[
                "Agriculture"
            ],
            f"{self.name}.{self.model_name}.{GlossaryCore.SectorEnergyConsumptionPercentageDfName}_Industry": self.default_value_dict_consumption[
                "Industry"
            ],
            f"{self.name}.{self.model_name}.{GlossaryCore.SectionNonEnergyEmissionGdpDfValue}_Industry": self.default_value_dict_emissions[
                "Industry"
            ],
        ...
        self.checked_inputs = [
            f"{self.name}.{GlossaryCore.ShareNonEnergyInvestmentsValue}",
    ...
    def test_macro_economics_analytic_grad(self):
        ...
        self.check_jacobian(
            ...
            filename=f"jacobian_macroeconomics_discipline.pkl",
    ...
    def test_macro_economics_analytic_grad_gigantic_invest(self):
        ...
        energy_investment_wo_tax = pd.DataFrame(
            {
                GlossaryCore.Years: self.years,
                GlossaryCore.EnergyInvestmentsWoTaxValue: [50.0] * self.nb_per,
            }
        )

        share_non_energy_investment = pd.DataFrame(
            {GlossaryCore.Years: self.years, GlossaryCore.ShareNonEnergyInvestmentsValue: [20.0] * self.nb_per}
        )
        inputs_dict = self.inputs_dict
        ...
        inputs_dict.update(
            {
                ...
                f"{self.name}.gross_output_in": pd.DataFrame(
                    {GlossaryCore.Years: self.years, GlossaryCore.GrossOutput: 0.02}
                ),
            }
        )
    ...
    def test_gigantic_energy_production_no_damage_productivity(self):
        ...
        energy_prod[20:] = energy_prod[20:] / 10.0
