...
class EnergyInvestDiscTest(unittest.TestCase):
    ...
    def test_execute(self):
        ...
        mod_path = (
            "climateeconomics.sos_wrapping.sos_wrapping_sectors.energy_invest.energy_invest_disc.EnergyInvestDiscipline"
        )
        ...
        default_CO2_tax = DataFrame({GlossaryCore.Years: years, GlossaryCore.CO2Tax: np.linspace(10, 80, n_years)})

        default_co2_efficiency = DataFrame(
            {GlossaryCore.Years: years, GlossaryCore.CO2TaxEfficiencyValue: np.linspace(30, 30, n_years)}
        )

        co2_emissions_gt = DataFrame(
            {GlossaryCore.Years: years, GlossaryCore.TotalCO2Emissions: np.linspace(34, 55, n_years)}
        )

        energy_investment_wo_tax = DataFrame(
            {GlossaryCore.Years: years, GlossaryCore.EnergyInvestmentsWoTaxValue: 1.02**time * 0.7}
        )  # in T$
