...
class WITNESSParallelTest(unittest.TestCase):
    ...
    def test_01_exec_parallel(self):
        ...
        builder = self.ee8.factory.get_builder_from_process(repo, "witness_coarse")
            ...
            if disc1.jac is not None:
                ...
                for keyout, subjac in disc1.jac.items():
                    for keyin in subjac.keys():
                        comparison = disc1.jac[keyout][keyin].toarray() == disc2.jac[keyout][keyin].toarray()
                        try:
        ...
        self.assertDictEqual(dict_error, {".<study_ph>.n_processes.value": "1 and 16 don't match"})
