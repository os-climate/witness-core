...
class AgricultureMixJacobianDiscTest(AbstractJacobianUnittest):
    ...
    def analytic_grad_entry(self):
        return [self.test_agriculture_discipline_mix_analytic_grad]
    ...
    def test_agriculture_discipline_mix_analytic_grad(self):
        ...
        ns_dict = {
            "ns_agriculture": f"{self.test_name}",
        ...
        self.temperature_df = pd.DataFrame({GlossaryCore.Years: self.years, GlossaryCore.TempAtmo: temperature})

        self.population_df = pd.DataFrame({GlossaryCore.Years: self.years, GlossaryCore.PopulationValue: population})
        ...
        self.red_meat_percentage = pd.DataFrame(
            {GlossaryCore.Years: self.years, "red_meat_percentage": red_meat_percentage}
        )
        self.white_meat_percentage = pd.DataFrame(
            {GlossaryCore.Years: self.years, "white_meat_percentage": white_meat_percentage}
        )
        ...
        self.CO2_emissions_crop = pd.DataFrame({GlossaryCore.Years: self.years, "Crop": CO2_emissions})
        ...
        self.forest_invest_df = pd.DataFrame({GlossaryCore.Years: self.years, "forest_investment": forest_invest})
        mw_invest = np.linspace(1, 4, year_range)
        uw_invest = np.linspace(0, 1, year_range)
        self.mw_invest_df = pd.DataFrame({GlossaryCore.Years: self.years, GlossaryCore.InvestmentsValue: mw_invest})
        self.uw_invest_df = pd.DataFrame({GlossaryCore.Years: self.years, GlossaryCore.InvestmentsValue: uw_invest})
        ...
        CO2_emissions = np.linspace(0, 0, year_range)
        self.CO2_emissions_forest = pd.DataFrame({GlossaryCore.Years: self.years, "Forest": CO2_emissions})
        ...
        self.margin = pd.DataFrame({GlossaryCore.Years: self.years, "margin": np.ones(len(self.years)) * 110.0})
        # From future of hydrogen
        self.transport = pd.DataFrame({GlossaryCore.Years: self.years, "transport": np.ones(len(self.years)) * 7.6})
        ...
        self.co2_taxes = pd.DataFrame({GlossaryCore.Years: self.years, GlossaryCore.CO2Tax: func(self.years)})
        ...
        inputs_dict = {
            f"{self.test_name}.{GlossaryCore.YearStart}": self.year_start,
        ...
        self.check_jacobian(
            ...
            filename=f"jacobian_agriculture_mix_discipline.pkl",
            discipline=disc_techno,
            step=1e-15,
            derr_approx="complex_step",
            local_data=disc_techno.local_data,
            inputs=[
                f"{self.test_name}.{disc_name}.Crop.techno_consumption",
            ...
            outputs=[
                f"{self.test_name}.{disc_name}.CO2_emissions",
