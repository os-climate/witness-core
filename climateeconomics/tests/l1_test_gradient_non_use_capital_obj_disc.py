...
class NonUseCapitalObjJacobianDiscTest(AbstractJacobianUnittest):
    ...
    def setUp(self):
        ...
        loss_ref = 3
        loss_reforest = 3
        non_use_capital_fg = pd.DataFrame(
            {GlossaryCore.Years: np.arange(year_start, year_end + 1), "FossilGas": loss_fg}
        )
        non_use_capital_ub = pd.DataFrame(
            {GlossaryCore.Years: np.arange(year_start, year_end + 1), "UpgradingBiogas": loss_ub}
        )
        non_use_capital_rf = pd.DataFrame(
            {GlossaryCore.Years: np.arange(year_start, year_end + 1), "Refinery": loss_rf}
        )
        non_use_capital_ft = pd.DataFrame(
            {GlossaryCore.Years: np.arange(year_start, year_end + 1), "FischerTropsch": loss_ft}
        )
        non_use_capital_ct = pd.DataFrame(
            {GlossaryCore.Years: np.arange(year_start, year_end + 1), "direct_air_capture.AmineScrubbing": loss_ct}
        )
        non_use_capital_ref = pd.DataFrame(
            {GlossaryCore.Years: np.arange(year_start, year_end + 1), "Forest": loss_ref}
        )
        forest_lost_capital = pd.DataFrame(
            {
    ...
    def test_01_grad_non_use_capital_objective(self):
        ...
        self.check_jacobian(
            ...
            filename=f"jacobian_non_use_capital_objective.pkl",
