...
class PostProcessEnergy(unittest.TestCase):
    ...
    def test_post_processing_Table_plots(self):
        ...
        if exists(dump_dir):
            BaseStudyManager.static_load_data(dump_dir, self.ee, DirectLoadDump())
        else:
            self.ee.execute()
            BaseStudyManager.static_dump_data(dump_dir, self.ee, DirectLoadDump())

        ppf = PostProcessingFactory()

        for itm in self.namespace_list:
            filters = ppf.get_post_processing_filters_by_namespace(self.ee, itm)
            graph_list = ppf.get_post_processing_by_namespace(self.ee, itm, filters, as_json=False)
