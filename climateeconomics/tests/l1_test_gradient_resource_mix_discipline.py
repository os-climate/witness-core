...
class ResourceJacobianDiscTest(AbstractJacobianUnittest):
    ...
    def setUp(self):
        ...
        self.all_demand = read_csv(join(data_dir, "all_demand_with_high_demand.csv"))
    ...
    def analytic_grad_entry(self):
        return [self._test_All_resource_discipline_analytic_grad]
    ...
    def test_All_resource_discipline_analytic_grad(self):
        ...
        input_recycled_production = [
            f"{self.name}.{self.model_name}.{resource}.recycled_production"
        ...
        input_other_price = [f"{self.name}.{self.model_name}.{ResourceMixModel.NON_MODELED_RESOURCE_PRICE}"]
        input_names.extend(input_other_price)
        input_demand = [f"{self.name}.{self.model_name}.resources_demand"]
        input_demand.extend([f"{self.name}.{self.model_name}.resources_demand_woratio"])
        ...
        self.check_jacobian(
            ...
            filename=f"jacobian_all_resource_discipline.pkl",
