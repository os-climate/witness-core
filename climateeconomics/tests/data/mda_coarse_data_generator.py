...
def launch_data_pickle_generation(directory=""):
    ...
    builder = ee.factory.get_builder_from_process(repo, "witness_coarse")
    ...
    Energy_Mix_disc = ee.dm.get_disciplines_with_name(f"{name}.{model_name}")[0]
    energy_list = Energy_Mix_disc.get_sosdisc_inputs(GlossaryCore.energy_list)
    ...
    for namespace in namespaces:
        coupled_inputs += [input[len(namespace) :] for input in full_inputs if ee.dm.get_data(input, "coupling")]
    ...
    for key in Energy_Mix_disc.get_sosdisc_inputs().keys():
        ...
        mda_coarse_data_energymix_input_dict[key] = {
            "value": Energy_Mix_disc.get_sosdisc_inputs(key),
    ...
    namespaces = [f"{name}.", f"{name}.{model_name}."]
    for namespace in namespaces:
        coupled_outputs += [output[len(namespace) :] for output in full_outputs if ee.dm.get_data(output, "coupling")]
    for key in Energy_Mix_disc.get_sosdisc_outputs().keys():
        is_coupling = False
        if key in coupled_outputs:
            is_coupling = True
        mda_coarse_data_energymix_output_dict[key] = {
            "value": Energy_Mix_disc.get_sosdisc_outputs(key),
            "is_coupling": is_coupling,
        }
    ...
    for energy in energy_list:
        # Loop on energies
        energy_disc = ee.dm.get_disciplines_with_name(f"{name}.{model_name}.{energy}")[0]
        # Inputs
        ...
        for key in energy_disc.get_sosdisc_inputs().keys():
            ...
            mda_coarse_data_streams_input_dict[energy][key] = {
                "value": energy_disc.get_sosdisc_inputs(key),
                "is_coupling": is_coupling,
            }
        # Output
        ...
        for namespace in namespaces:
            coupled_outputs += [
                output[len(namespace) :] for output in full_outputs if ee.dm.get_data(output, "coupling")
            ]
        for key in energy_disc.get_sosdisc_outputs().keys():
            ...
            mda_coarse_data_streams_output_dict[energy][key] = {
                "value": energy_disc.get_sosdisc_outputs(key),
                "is_coupling": is_coupling,
            }
        ################
        # Technologies #
        ...
        for techno in technologies_list:
            ...
            techno_disc = ee.dm.get_disciplines_with_name(f"{name}.{model_name}.{energy}.{techno}")[0]
            # Inputs
            ...
            for namespace in namespaces:
                coupled_inputs += [
                    input[len(namespace) :] for input in full_inputs if ee.dm.get_data(input, "coupling")
                ]
            for key in techno_disc.get_sosdisc_inputs().keys():
                ...
                mda_coarse_data_technologies_input_dict[techno][key] = {
                    "value": techno_disc.get_sosdisc_inputs(key),
                    "is_coupling": is_coupling,
                }
            # Output
            ...
            for key in techno_disc.get_sosdisc_outputs().keys():
                ...
                mda_coarse_data_technologies_output_dict[techno][key] = {
                    "value": techno_disc.get_sosdisc_outputs(key),
                    "is_coupling": is_coupling,
                }

    ccs_list = Energy_Mix_disc.get_sosdisc_inputs(GlossaryCore.ccs_list)
    ###############
    # CCS Streams #
    ###############
    for stream in ccs_list:
        stream_disc = ee.dm.get_disciplines_with_name(f"{name}.CCUS.{stream}")[0]
        # Inputs
        ...
        for key in stream_disc.get_sosdisc_inputs().keys():
            ...
            mda_coarse_data_streams_input_dict[stream][key] = {
                "value": stream_disc.get_sosdisc_inputs(key),
                "is_coupling": is_coupling,
            }
        # Output
        ...
        for key in stream_disc.get_sosdisc_outputs().keys():
            ...
            mda_coarse_data_streams_output_dict[stream][key] = {
                "value": stream_disc.get_sosdisc_outputs(key),
                "is_coupling": is_coupling,
            }
        ################
        # Technologies #
        ...
        for techno in technologies_list:
            # Loop on technologies
            techno_disc = ee.dm.get_disciplines_with_name(f"{name}.CCUS.{stream}.{techno}")[0]
            # Inputs
            mda_coarse_data_technologies_input_dict[techno] = {}
            full_inputs = techno_disc.get_input_data_names()
            # For the coupled inputs and outputs, test inputs/outputs on all
            # namespaces
            coupled_inputs = []
            namespaces = [f"{name}.", f"{name}.CCUS.", f"{name}.CCUS.{stream}.", f"{name}.CCUS.{stream}.{techno}."]
            for namespace in namespaces:
                coupled_inputs += [
                    input[len(namespace) :] for input in full_inputs if ee.dm.get_data(input, "coupling")
                ]
    ...
    output = open(f"{prefix}/mda_coarse_data_energymix_input_dict.pkl", "wb")
    pickle.dump(mda_coarse_data_energymix_input_dict, output)
    output.close()

    output = open(f"{prefix}/mda_coarse_data_energymix_output_dict.pkl", "wb")
