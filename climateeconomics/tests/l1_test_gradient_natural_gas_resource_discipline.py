...
class NaturalGasResourceJacobianDiscTest(AbstractJacobianUnittest):
    ...
    def test_natural_gas_resource_analytic_grad(self):
        ...
        self.check_jacobian(location=dirname(__file__), filename=f'jacobian_natural_gas_discipline.pkl',
    ...
    def test_natural_gas_resource_demand_variable_analytic_grad(self):
        ...
        disc_techno = self.ee.root_process.proxy_disciplines[0].mdo_discipline_wrapp.mdo_discipline
        self.check_jacobian(location=dirname(__file__), filename=f'jacobian_natural_gas_demand_variable_discipline.pkl',
                            discipline=disc_techno, step=1e-15, derr_approx='complex_step', local_data=disc_techno.local_data,
                            inputs=[
                                f'{self.name}.{self.model_name}.resources_demand'],
                            outputs=[f'{self.name}.{self.model_name}.resource_stock',
