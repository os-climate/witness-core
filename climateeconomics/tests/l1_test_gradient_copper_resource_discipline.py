...
class CopperResourceJacobianDiscTest(AbstractJacobianUnittest):
    ...
    def analytic_grad_entry(self):
        return [self.test_copper_resource_analytic_grad, self.test_copper_resource_damand_variable_analytic_grad]
    ...
    def setUp(self):
        ...
        self.consumed_copper_df = read_csv(join(data_dir, "copper_resource_consumed_data.csv"))

        # part to adapt lenght to the year range

        self.energy_copper_demand_df = self.energy_copper_demand_df.loc[
            self.energy_copper_demand_df[GlossaryCore.Years] >= self.year_start
        ]
        self.energy_copper_demand_df = self.energy_copper_demand_df.loc[
            self.energy_copper_demand_df[GlossaryCore.Years] <= self.year_end
        ]
        self.energy_copper_variable_demand_df = self.energy_copper_variable_demand_df.loc[
            self.energy_copper_variable_demand_df[GlossaryCore.Years] >= self.year_start
        ]
        self.energy_copper_variable_demand_df = self.energy_copper_variable_demand_df.loc[
            self.energy_copper_variable_demand_df[GlossaryCore.Years] <= self.year_end
        ]
    ...
    def test_copper_resource_analytic_grad(self):
        ...
        mod_path = (
            "climateeconomics.core.core_resources.models.copper_resource.copper_resource_disc.CopperResourceDiscipline"
        )
        ...
        self.check_jacobian(
            ...
            filename=f"jacobian_copper_discipline.pkl",
    ...
    def test_copper_resource_damand_variable_analytic_grad(self):
        ...
        self.check_jacobian(
            location=dirname(__file__),
            filename=f"jacobian_copper_demand_variable_discipline.pkl",
            discipline=disc_techno,
            step=1e-15,
            derr_approx="complex_step",
            local_data=disc_techno.local_data,
