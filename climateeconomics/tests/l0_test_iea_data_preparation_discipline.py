...
class IEADataPreparationTest(unittest.TestCase):
    ...
    def test_execute(self):
        ...
        CO2_emissions_df = pd.DataFrame({Glossary.Years: years, Glossary.TotalCO2Emissions: [60, 40, 20, 30]})
        GDP_values = [120, 140, 145, 160]
        GDP_df = pd.DataFrame({Glossary.Years: years, Glossary.OutputNetOfDamage: GDP_values})
        CO2_tax_df = pd.DataFrame({Glossary.Years: years, Glossary.CO2Tax: [370, 500, 700, 800]})

        energy_production_df = pd.DataFrame({Glossary.Years: years, Glossary.TotalProductionValue: [40, 70, 80, 10]})

        population_df = pd.DataFrame({Glossary.Years: years, Glossary.PopulationValue: [8, 8.2, 8.3, 8]})

        temperature_df = pd.DataFrame({Glossary.Years: years, Glossary.TempAtmo: [2.2, 2.7, 2.75, 2.78]})
        ...
        for techno in l_technos_to_add:
            values_dict.update(
                {
                    f"{self.name}.{self.model_name}.{techno}_techno_production": pd.DataFrame(
                        {
        ...
        assert (
            gdp_interpolated.loc[gdp_interpolated[Glossary.Years].isin(years), Glossary.OutputNetOfDamage].tolist()
            == GDP_values
        )
        ...
        assert (
            gdp_interpolated.loc[gdp_interpolated[Glossary.Years] == 2035, Glossary.OutputNetOfDamage].values[0]
            == expected_value_2035
        )
