...
class SectorsRedistributionEnergyDisciplineJacobianDiscTest(AbstractJacobianUnittest):
    ...
    def setUp(self):
        ...
        self.energy_production_df = pd.DataFrame(
            {GlossaryCore.Years: self.years, GlossaryCore.TotalProductionValue: 10000 * 1.02**time}
        )
        self.share_energy_agriculture = pd.DataFrame(
            {GlossaryCore.Years: self.years, GlossaryCore.ShareSectorEnergy: np.linspace(5, 2, n_years)}
        )
        self.share_energy_services = pd.DataFrame(
            {GlossaryCore.Years: self.years, GlossaryCore.ShareSectorEnergy: np.linspace(30, 35, n_years)}
        )
        self.share_energy_residential = pd.DataFrame(
            {GlossaryCore.Years: self.years, GlossaryCore.ShareSectorEnergy: np.linspace(20, 25, n_years)}
        )
        self.share_energy_other = pd.DataFrame(
            {GlossaryCore.Years: self.years, GlossaryCore.ShareSectorEnergy: np.linspace(10, 10, n_years)}
        )
    ...
    def test_analytic_grad(self):
        ...
        inputs_checked += [
            f"{name}.{sector}.{GlossaryCore.ShareSectorEnergyDfValue}" for sector in self.sector_list[:-1]
        ]
        ...
        self.check_jacobian(
            ...
            filename=f"jacobian_sectors_redistribution_energy_discipline.pkl",
