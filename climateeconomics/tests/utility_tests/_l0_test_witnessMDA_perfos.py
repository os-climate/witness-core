...
class TestScatter(unittest.TestCase):
    ...
    def test_01_witness_perfos_execute(self):
        ...
        linearize_time = float([line for line in lines if "linearize" in line][0].split(",")[3])
        execute_time = float([line for line in lines if "execute_all_disciplines" in line][0].split(",")[3])
        inversion_time = float([line for line in lines if "algo_lib.py" in line][0].split(",")[3])
        pre_run_mda_time = float([line for line in lines if "pre_run_mda" in line][0].split(",")[3])
        dres_dvar_time = float([line for line in lines if "dres_dvar" in line][0].split(",")[3])
        gauss_seidel_time = float([line for line in lines if "gauss_seidel.py" in line][0].split(",")[3])

        _convert_array_into_new_type = float(
            [line for line in lines if "_convert_array_into_new_type" in line][0].split(",")[3]
        )
        ...
        ax1.pie(sizes, labels=labels, autopct=make_autopct(sizes), shadow=True, startangle=90)
        # Equal aspect ratio ensures that pie is drawn as a circle.
        ax1.axis("equal")
        ax1.set_title(f"WITNESS {mda_class} cache with {n_processes} procs, Total time : {total_time} s")

        fig_name = f"WITNESS_{mda_class}_{n_processes}_proc.png"
        plt.savefig(join(dirname(__file__), fig_name))
        if platform.system() == "Windows":
            plt.show()
        else:

            #
            os.system(f"git add ./climateeconomics/tests/utility_tests/{fig_name}")
            os.system(f"git add ./climateeconomics/tests/utility_tests/witness_perfos.csv")
            os.system(f'git commit -m "Add {fig_name}"')
    ...
    def test_04_witness_coarseperfos_multiproc(self):
        ...
        for n_proc in n_proc_list:
            ...
            self.ee = ExecutionEngine(self.name)
            repo = "climateeconomics.sos_processes.iam.witness"
            builder = self.ee.factory.get_builder_from_process(repo, "witness_coarse")
    ...
    def _test_06_witness_perfos_parallel_comparison(self):
        ...
        def stack_bar_chart(data, x, title, figsize, color, xlabel, ylabel, tailles, trick=False, given_labels=False):
            ...
            ax = data.plot(x=x, kind="bar", stacked=True, title=title, mark_right=True, figsize=figsize, color=color)
            ...
            for c in ax.containers:
                ...
                labels = [
                    f"{(v.get_height()) / (tailles[k]) * 100:0.1f}%" if (v.get_height()) > 0 else ""
        ...
        def witness_parallel_status():
            ...
            coarse_analysis = pd.DataFrame(
                {
                    "exec_mode": execution_modes,
                    "Linearize": linearizes_coarse,
                    "Execute": execute_coarse,
    ...
    def test_08_witness_full_mda_perfos_parallels(self):
        ...
        def get_categorized_times(result, categories_dict):
            ...
            for category, str_to_find in categories_dict.items():
                if str_to_find is None:
                    continue
                try:
                    cat_times[category] = float([line for line in lines if str_to_find in line][0].split(",")[3])
                except:
                    cat_times[category] = 0.0
            cat_times["Total"] = total_time
            cat_times["Others"] = total_time - sum(
                [
                    time
                    for key, time in cat_times.items()
                    if key in ["Linearize", "Pre-run", "Gauss Seidel", "Execute", "Matrix Inversion", "Matrix Build"]
                ]
            )
            return cat_times

        def get_stacked_bar_chart(
            labels, values_list, title="Fig Title", save=False, filename="witness_full_MDA_parallel_perfos"
        ):
            ...
            for operation in values_list[0].keys():
                if operation == "Total":
                    ...
                    ax.bar(
                        x + 0.5, [values[operation] for values in values_list][0], width, bottom=bottom, label=operation
                    )
        ...
        def get_operation_bar_chart(
            labels, values, title="Fig Title", save=False, filename="witness_full_MDA_parallel_perfos"
        ):
            x = np.arange(len(labels))  # the label locations
            width = 0.35  # the width of the bars
            fig, ax = plt.subplots()
        ...
        case_dict = {
            "GSNR-sequential": [1, "GSNewtonMDA"],
            "GSNR-10core": [10, "GSNewtonMDA"],
        ...
        for operation in operations_dict.keys():
            operations_fig += [
                get_operation_bar_chart(
                    case_dict.keys(),
                    [cat_times[operation] for cat_times in cat_times_list],
                    title=f"{operation} time",
                    save=True,
        ...
        stacked_data = [
            {
                key: value
                for key, value in cat_times.items()
                if key
                in ["Total", "Linearize", "Pre-run", "Gauss Seidel", "Execute", "Matrix Inversion", "Matrix Build"]
            }
            for cat_times in cat_times_list
        ]
        stacked_fig = get_stacked_bar_chart(
            case_dict.keys(),
            stacked_data,
            title="Stacked operations time",
            save=True,
        ...
        if platform.system() == "Windows":
            ...
            os.system(f"git add ./perfo_dir/*.csv")
            os.system(f"git add ./perfo_dir/*.jpg")
            os.system(f'git commit -m "Add perfo MDA parallel figures"')
            os.system("git pull")
            os.system("git push")
        ...
        def get_stacked_bar_chart(
            ...
            for operation in values_list[0].keys():
                if operation == "Total":
                    ...
                    ax.bar(
                        x + 0.5, [values[operation] for values in values_list], width, bottom=bottom, label=operation
                    )
