...
class WitnessFullJacobianDiscTest(AbstractJacobianUnittest):
    ...
    def test_01_gradient_objective_wrt_state_var_on_witness_full(self):
        ...
        input_full_names = ["Test.EnergyMix.invest_energy_mix", f"Test.{GlossaryCore.CO2TaxesValue}"]
    ...
    def test_02_gradient_residus_wrt_state_var_on_witness_full(self):
        ...
        output_full_names = [
            f"Test.{GlossaryCore.TemperatureDfValue}",
            f"Test.{GlossaryCore.UtilityDfValue}",
            f"Test.{GlossaryCore.EconomicsDfValue}",
            f"Test.{GlossaryCore.CarbonCycleDfValue}",
            "Test.CO2_emissions_df",
            f"Test.{GlossaryCore.DamageFractionDfValue}",
            f"Test.EnergyMix.{GlossaryCore.EnergyProductionValue}",
            f"Test.EnergyMix.{GlossaryCore.EnergyInvestmentsValue}",
            f"Test.EnergyMix.{GlossaryCore.CO2EmissionsGtValue}",
            f"Test.EnergyMix.{GlossaryCore.EnergyMeanPriceValue}",
        ]

        input_full_names = ["Test.EnergyMix.invest_energy_mix", f"Test.{GlossaryCore.CO2TaxesValue}"]
        input_full_names.extend([f"Test.EnergyMix.{energy}.invest_techno_mix" for energy in usecase.energy_list])

        self.check_jacobian(
            location=dirname(__file__),
            filename=f"jacobian_residus_wrt_state_var_on_witness_full.pkl",
            discipline=disc,
            inputs=input_full_names,
            local_data={},
            outputs=output_full_names,
    ...
    def test_03_gradient_residus_wrt_design_var_on_witness_full(self):
        ...
        design_var_path = (
            "climateeconomics.core.design_variables_translation.witness.design_var_disc.Design_Var_Discipline"
        )
        ...
        output_full_names = [
            f"{namespace}.{GlossaryCore.TemperatureDfValue}",
            f"{namespace}.{GlossaryCore.UtilityDfValue}",
            f"{namespace}.{GlossaryCore.EconomicsDfValue}",
            f"{namespace}.{GlossaryCore.CarbonCycleDfValue}",
            f"{namespace}.{GlossaryCore.CO2EmissionsDfValue}",
            f"{namespace}.{GlossaryCore.DamageFractionDfValue}",
            f"{namespace}.EnergyMix.{GlossaryCore.EnergyProductionValue}",
            f"{namespace}.EnergyMix.{GlossaryCore.EnergyInvestmentsValue}",
            f"{namespace}.EnergyMix.{GlossaryCore.CO2EmissionsGtValue}",
            f"{namespace}.EnergyMix.{GlossaryCore.EnergyMeanPriceValue}",
            f"{namespace}.CO2_objective",
            f"{namespace}.ppm_objective",
        ...
        self.check_jacobian(
            location=dirname(__file__),
            filename=f"jacobian_residus_wrt_design_var_on_witness_full.pkl",
            local_data={},
            discipline=disc,
            inputs=[
                f"{namespace}.EnergyMix.electricity.CoalGen.electricity_CoalGen_array_mix",
    ...
    def test_04_gradient_objective_wrt_design_var_on_witness_full(self):
        ...
        output_full_names = [f"Test.WITNESS_Eval.{obj}" for obj in self.obj_const]
        input_full_names = ["Test.WITNESS_Eval.CO2_taxes_array"]
        for energy in full_values_dict[f"{self.name}.WITNESS_Eval.{GlossaryCore.energy_list}"]:
            energy_wo_dot = energy.replace(".", "_")
            input_full_names.append(f"{self.name}.WITNESS_Eval.DesignVariables.{energy}.{energy_wo_dot}_array_mix")
        ...
        self.check_jacobian(
            location=dirname(__file__),
            filename=f"jacobian_objective_wrt_design_var_on_witness_full.pkl",
            discipline=disc,
            step=1.0e-15,
            derr_approx="complex_step",
            threshold=1e-5,
    ...
    def test_05_adjoint_with_bsplines(self):
        ...
        self.check_jacobian(
            location=dirname(__file__),
            filename=f"jacobian_adjoint_with_bsplines_witness_full.pkl",
            discipline=disc,
            local_data={},
            inputs=input_full_names,
            outputs=output_full_names,
