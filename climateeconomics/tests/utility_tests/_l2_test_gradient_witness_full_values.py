    ...
    def test_01(self):
        ...
        for i in range(79):
            ...
            if exists(filepath):
                ...
                try:
                    max_dict_discipline = {
                        f"{key} vs {key2}": np.max(np.abs(value2))
                        for key, value in pkl_dict.items()
                        for key2, value2 in value.items()
                    }
                    print(max_dict_discipline)
                    max_discipline[max(max_dict_discipline, key=max_dict_discipline.get)] = max(
                        list(max_dict_discipline.values())
                    )
                    print(
                        "max of discipline :",
                        max(list(max_dict_discipline.values())),
                        max(max_dict_discipline, key=max_dict_discipline.get),
                    )
                except:
                    pass
        print("max max max", max(list(max_discipline.values())), max(max_discipline, key=max_discipline.get))
