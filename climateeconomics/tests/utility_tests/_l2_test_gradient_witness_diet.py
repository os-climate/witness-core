...
from climateeconomics.sos_processes.iam.diet.diet_optim_process.usecase_diet_optim import Study as diet_usecase
...
class WitnessDevJacobianDiscTest(AbstractJacobianUnittest):
    ...
    def test_01_gradient_dev_specific_disciplines(self):
        ...
        builder = self.ee.factory.get_builder_from_process(
            'climateeconomics.sos_processes.iam.diet', 'diet_optim_process')
        ...
        for i in range(1):
            ...
            self.ee.load_study_from_input_dict(full_values_dict)
            self.ee.execute()
            for j, disc in enumerate(self.ee.root_process.sos_disciplines[0].sos_disciplines[0].sos_disciplines):
                ...
                filepath = join(dirname(__file__), AbstractJacobianUnittest.PICKLE_DIRECTORY, 'l2_diet',
                                pkl_name)
                if (len(inputs) != 0) and (len(outputs) != 0):
                    self.ee.dm.delete_complex_in_df_and_arrays()
                    self.check_jacobian(location=dirname(__file__), filename='l2_diet/' + pkl_name, discipline=disc,
