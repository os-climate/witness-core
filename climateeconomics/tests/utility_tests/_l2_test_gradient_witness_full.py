...
class WitnessFullJacobianDiscTest(AbstractJacobianUnittest):
    ...
    def _test_01_gradient_objective_wrt_state_var_on_witness_full_mda(self):
        ...
        chain_builders = self.ee.factory.get_builder_from_process(repo, "witness")
        ...
        input_full_names = [
            f"{self.name}.EnergyMix.invest_energy_mix",
            f"{self.name}.{GlossaryCore.CO2TaxesValue}",
            f"{self.name}.livestock_usage_factor_df",
        ]
        for energy in usecase.energy_list:
            input_full_names.append(f"{self.name}.EnergyMix.{energy}.{GlossaryCore.InvestLevelValue}")

        disc = self.ee.root_process

        self.check_jacobian(
            location=dirname(__file__),
            filename=f"jacobian_objective_wrt_state_var_on_witness_full.pkl",
            local_data={},
            discipline=disc,
    ...
    def test_02_gradient_objective_constraint_wrt_design_var_on_witness_full_subprocess_wofuncmanager(self):
        ...
        for energy in full_values_dict[
            f"{self.name}.{usecase.coupling_name}.{usecase.extra_name}.{GlossaryCore.energy_list}"
        ]:
        ...
        for energy in full_values_dict[
            f"{self.name}.{usecase.coupling_name}.{usecase.extra_name}.{GlossaryCore.ccs_list}"
        ]:
            energy_wo_dot = energy.replace(".", "_")
            input_full_names.append(
    ...
    def test_03_gradient_lagrangian_objective_wrt_design_var_on_witness_full_subprocess(self):
        ...
        output_full_names = [f"{self.name}.objective_lagrangian"]
        input_full_names = [
            f"{self.name}.{usecase.coupling_name}.{usecase.extra_name}.CO2_taxes_array",
            f"{self.name}.{usecase.coupling_name}.{usecase.extra_name}.livestock_usage_factor_array",
        ]
    ...
    def test_04_gradient_lagrangian_objective_wrt_csv_design_var_on_witness_full_subprocess(self):
        ...
        df_xvect = pd.read_csv(join(dirname(__file__), "data", "design_space_last_ite_crash.csv"))
        for i, row in df_xvect.iterrows():
            ns_var = self.ee.dm.get_all_namespaces_from_var_name(row["variable"])[0]
            values_dict_design_var[ns_var] = np.asarray(row["value"][1:-1].split(", "), dtype=float)
        dspace_df = df_xvect
    ...
    def test_05_gradient_lagrangian_objective_wrt_csv_design_var_on_witness_full_subprocess_each_step(self):
        ...
        values_dict_design_var = {}
        df_xvect = pd.read_csv(join(dirname(__file__), "data", "design_space_last_ite_full_wo_effect.csv"))
        for i, row in df_xvect.iterrows():
            try:
                ns_var = self.ee.dm.get_all_namespaces_from_var_name(row["variable"])[0]
                values_dict_design_var[ns_var] = np.asarray(row["value"][1:-1].split(", "), dtype=float)
        ...
        inputs_names = [
            f"{self.name}.{usecase.coupling_name}.WITNESS.EnergyMix.{energy}.energy_prices"
            for energy in energy_list
            if energy not in ["carbon_capture", "carbon_storage"]
        ]
        inputs_names.extend(
            [
                f"{self.name}.{usecase.coupling_name}.WITNESS.EnergyMix.{energy}.{GlossaryCore.EnergyProductionValue}"
                for energy in energy_list
                if energy not in ["carbon_capture", "carbon_storage"]
            ]
        )
        inputs_names.extend(
            [
                f"{self.name}.{usecase.coupling_name}.WITNESS.EnergyMix.{energy}.energy_consumption"
                for energy in energy_list
                if energy not in ["carbon_capture", "carbon_storage"]
            ]
        )
        inputs_names.extend(
            [
                f"{self.name}.{usecase.coupling_name}.WITNESS.CCUS.{energy}.energy_consumption"
                for energy in ["carbon_capture", "carbon_storage"]
            ]
        )
        inputs_names.extend(
            [
                f"{self.name}.{usecase.coupling_name}.WITNESS.CCUS.{energy}.{GlossaryCore.EnergyProductionValue}"
                for energy in ["carbon_capture", "carbon_storage"]
            ]
        )
        inputs_names.extend(
            [
                f"{self.name}.{usecase.coupling_name}.WITNESS.CCUS.{energy}.energy_prices"
                for energy in ["carbon_capture", "carbon_storage"]
            ]
        )
        inputs_names.extend([f"{self.name}.{usecase.coupling_name}.WITNESS.EnergyMix.syngas.syngas_ratio"])
        i = 0
        ...
        for disc in self.ee.root_process.proxy_disciplines[0].proxy_disciplines:
            ...
            outputs = [
                ...
                if self.ee.dm.get_data(output, "coupling") and not output.endswith("all_streams_demand_ratio")
            ]
            ...
            inputs = [
                input
                for input in inputs
                if self.ee.dm.get_data(input, "coupling")
            ...
            if i not in [63, 64]:
                ...
                filepath = join(
                    dirname(__file__), AbstractJacobianUnittest.PICKLE_DIRECTORY, "l2_witness_full", pkl_name
                )
                if len(inputs) != 0:
    ...
    def test_06_gradient_each_discipline_on_dm_pkl(self):
        ...
        pkl_dict = pd.read_pickle(join(dirname(__file__), "data", "dm_3_ite.pkl"))
        inp_dict = {key.replace("<study_ph>.WITNESS_MDO", self.name): value for key, value in pkl_dict.items()}
    ...
    def test_06_gradient_lagrangian_objective_wrt_csv_design_var_on_crashed_x(self):
        ...
        values_dict_design_var = {}
        df_xvect = pd.read_csv(join(dirname(__file__), "data", "design_space_last_ite_coarse_fail.csv"))
        for i, row in df_xvect.iterrows():
            try:
                ns_var = self.ee.dm.get_all_namespaces_from_var_name(row["variable"])[0]
                values_dict_design_var[ns_var] = np.asarray(row["value"][1:-1].split(", "), dtype=float)
        ...
        output_full_names = [f"{self.name}.objective_lagrangian"]
        input_full_names = [f"{self.name}.{usecase.coupling_name}.{usecase.extra_name}.livestock_usage_factor_array"]
    ...
    def test_07_gradient_all_disciplines_on_crashed_x(self):
        ...
        for disc in self.ee.root_process.proxy_disciplines[0].proxy_disciplines:
            ...
            filepath = join(dirname(__file__), AbstractJacobianUnittest.PICKLE_DIRECTORY, "l2_witness_full", pkl_name)
