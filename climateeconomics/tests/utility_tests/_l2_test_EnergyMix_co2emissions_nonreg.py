...
class EnergyMix_co2emissions_nonreg_test:
    ...
    def setUp(self):
        ...
        self.compare_list = [
            ...
            GlossaryCore.CO2EmissionsGtValue,
            "CCS_price",
        ]
    ...
    def run_non_reg(self, usecase, dm_ref, compare_list, usecase_name):
        ...
        usecase.load_data()
        inputs_dict = {}
        inputs_dict[usecase.ee.dm.get_all_namespaces_from_var_name("sub_mda_class")[0]] = "MDAGaussSeidel"
            ...
            for key, metadata in dm_ref.items():
                if param_name in key.split(".") and metadata["io_type"] == "out":
                    ...
                    if run_metadata is not None:
                        value_run = self.convert_types(run_metadata.get("value", None))
                        dm_run_to_compare[key] = value_run

        dict_error = {}
        test_passed = True
        output_error = ""
        compare_dict(dm_ref_to_compare, dm_run_to_compare, "", dict_error)
        if dict_error != {}:
            test_passed = False
    ...
    def test_non_reg_EnergyMDA_CO2_emissions(self):
        ...
        usecase = EnergyMDA()
        dm_ref = pickle.load(open(join(dirname(__file__), "data", "dm_energymda_co2emissionsnonreg.pkl"), "rb"))
