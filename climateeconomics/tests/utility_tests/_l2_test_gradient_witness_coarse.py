...
class WitnessCoarseJacobianDiscTest(AbstractJacobianUnittest):
    ...
    def test_03_gradient_lagrangian_objective_wrt_design_var_on_witness_coarse_subprocess(self):
        ...
        disc_techno.check_jacobian(
            ...
            outputs=output_full_names,
            load_jac_path=join(
                dirname(__file__), "jacobian_pkls", "jacobian_lagrangian_objective_wrt_design_var_on_witness_coarse.pkl"
            ),
        )
    ...
    def test_05_gradient_witness_coarse_eachdiscipline(self):
        ...
        full_values_dict[f"{self.name}.CCUS.ccs_percentage"] = pd.DataFrame(
            {
                GlossaryCore.Years: np.arange(GlossaryCore.YearStartDefault, GlossaryCore.YearEndDefault + 1),
                "ccs_percentage": 25,
            }
        )
        full_values_dict[f"{self.name}.sub_mda_class"] = "GSNewtonMDA"
        ...
        df_xvect = pd.read_csv(join(dirname(__file__), "data", "design_space_last_ite_coarse.csv"))
        ...
        inputs_names = [
            f"{self.name}.WITNESS_Eval.WITNESS.EnergyMix.{energy}.energy_prices"
            for energy in energy_list
            if energy not in ["carbon_capture", "carbon_storage"]
        ]
        inputs_names.extend(
            [
                f"{self.name}.WITNESS_Eval.WITNESS.EnergyMix.{energy}.{GlossaryCore.EnergyProductionValue}"
                for energy in energy_list
                if energy not in ["carbon_capture", "carbon_storage"]
            ]
        )
        inputs_names.extend(
            [
                f"{self.name}.WITNESS_Eval.WITNESS.EnergyMix.{energy}.energy_consumption"
                for energy in energy_list
                if energy not in ["carbon_capture", "carbon_storage"]
            ]
        )
        inputs_names.extend(
            [
                f"{self.name}.WITNESS_Eval.WITNESS.CCUS.{energy}.energy_consumption"
                for energy in ["carbon_capture", "carbon_storage"]
            ]
        )
        inputs_names.extend(
            [
                f"{self.name}.WITNESS_Eval.WITNESS.CCUS.{energy}.{GlossaryCore.EnergyProductionValue}"
                for energy in ["carbon_capture", "carbon_storage"]
            ]
        )
        inputs_names.extend(
            [
                f"{self.name}.WITNESS_Eval.WITNESS.CCUS.{energy}.energy_prices"
                for energy in ["carbon_capture", "carbon_storage"]
            ]
        )
        inputs_names.extend([f"{self.name}.WITNESS_Eval.WITNESS.EnergyMix.syngas.syngas_ratio"])
        i = 0
    ...
    def test_06_gradient_witness_coarse_subprocess_each_discipline_bis(self):
        ...
        builder = self.ee.factory.get_builder_from_process(
            "climateeconomics.sos_processes.iam.witness",
            "witness_coarse_process_one_distrib",
            techno_dict=GlossaryEnergy.DEFAULT_COARSE_TECHNO_DICT,
            one_invest_discipline=True,
        )
        self.ee.factory.set_builders_to_coupling_builder(builder)
        ...
        pkl_dict = pd.read_pickle(join(dirname(__file__), "data", "dm_crash.pkl"))
        inp_dict = {
            key.replace("usecase_witness_optim_invest_distrib", self.name): value for key, value in pkl_dict.items()
        }
