...
#             graph.to_plotly().show()
    ...
    def test_forest_discipline_high_lost_capital(self):
        ...
        self.margin = pd.DataFrame(
            {GlossaryCore.Years: years, 'margin': np.ones(len(years)) * 110.0})
        self.forest_invest_df = pd.DataFrame(
            {GlossaryCore.Years: years, 'forest_investment': np.array([262.3080913163442, 469.0288051740345, 646.7923512783858, 797.3169880002974,
                                                            ...
                                                            3.8911613193912378, 2.1242782531265565, 1.2239958500378234, 0.8981096666978611,
...
#             graph.to_plotly().show()
    ...
    def test_forest_discipline_low_lost_capital(self):
        ...
        self.margin = pd.DataFrame(
            {GlossaryCore.Years: years, 'margin': np.ones(len(years)) * 110.0})
        self.forest_invest_df = pd.DataFrame(
            {GlossaryCore.Years: years, 'forest_investment': np.array([26.3080913163442, 46.0288051740345, 64.7923512783858, 79.3169880002974,
                                                            ...
                                                            0.8911613193912378, 0.1242782531265565, 0.2239958500378234, 0.8981096666978611,
                                                            0.854415259679471, 0.849695588213618, 0.836683222163885, 0.8170981340520096,
                                                            0.7926602963997317, 0.7650896817287889, 0.7361062625609198, 0.707430011417863,
                                                            0.680780900821357, 0.6578789032931407, 0.6404439913549522, 0.6301961375285303,
                                                            0.6288553143356134])})
