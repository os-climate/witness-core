...
class LandUseV2TestCase(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.energy_land_demand_df = read_csv(join(data_dir, "land_demandV2.csv"))
        # part to adapt lenght to the year range
        self.energy_land_demand_df = self.energy_land_demand_df.loc[
            self.energy_land_demand_df[GlossaryCore.Years] >= self.year_start
        ]
        self.energy_land_demand_df = self.energy_land_demand_df.loc[
            self.energy_land_demand_df[GlossaryCore.Years] <= self.year_end
        ]
        ...
        self.param = {
            "land_demand_df": self.energy_land_demand_df,
    ...
    def test_land_use_v2_model(self):
        ...
        land_use.compute(self.energy_land_demand_df, self.total_food_land_surface, self.forest_surface_df)
