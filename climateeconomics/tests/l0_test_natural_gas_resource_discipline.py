...
class NaturalGasModelTestCase(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.energy_gas_demand_df = read_csv(join(data_dir, "all_demand_from_energy_mix.csv"))
        # part to adapt lenght to the year range

        self.energy_gas_demand_df = self.energy_gas_demand_df.loc[
            self.energy_gas_demand_df[GlossaryCore.Years] >= self.year_start
        ]
        self.energy_gas_demand_df = self.energy_gas_demand_df.loc[
            self.energy_gas_demand_df[GlossaryCore.Years] <= self.year_end
        ]

        self.param = {
            "resources_demand": self.energy_gas_demand_df,
    ...
    def test_gas_discipline(self):
        ...
        """
        name = "Test"
        model_name = "all_resource.natural_gas_resource"
