...
class AgricultureGHGEmissionDiscTest(unittest.TestCase):
    ...
    def test_execute(self):
        ...
        CO2_land_emissions_crop = pd.DataFrame(
            {GlossaryCore.Years: years, "emitted_CO2_evol_cumulative": np.linspace(0.7, 0.7, len(years))}
        )
        CO2_land_emissions_forest = pd.DataFrame(
            {GlossaryCore.Years: years, "emitted_CO2_evol_cumulative": np.linspace(-7.6, -8, len(years))}
        )
