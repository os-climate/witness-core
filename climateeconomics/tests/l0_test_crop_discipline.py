...
class CropTestCase(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.transport_cost = pd.DataFrame({GlossaryCore.Years: years, "transport": np.ones(len(years)) * 7.6})
        ...
        self.red_meat_calories_per_day = pd.DataFrame(
            {GlossaryCore.Years: years, "red_meat_calories_per_day": red_meat_daily_cal}
        )
        self.white_meat_calories_per_day = pd.DataFrame(
            {GlossaryCore.Years: years, "white_meat_calories_per_day": white_meat_daily_cal}
        )
        self.veg_calories_per_day = pd.DataFrame(
            {GlossaryCore.Years: years, "vegetables_and_carbs_calories_per_day": vegetables_and_carbs_calories_per_day}
        )

        self.milk_eggs_calories_per_day = pd.DataFrame(
            {GlossaryCore.Years: years, "milk_and_eggs_calories_per_day": milk_and_eggs}
        )

        self.fish_calories_per_day = pd.DataFrame(
            {GlossaryCore.Years: years, GlossaryCore.FishDailyCal: fish_daily_cal}
        )
        self.other_food_calories_per_day = pd.DataFrame(
            {GlossaryCore.Years: years, GlossaryCore.OtherDailyCal: other_food_daily_cal}
        )
        ...
        self.param = {
            ...
            "constraint_calories_limit": 1700.0,
            "constraint_calories_ref": 3400.0,
