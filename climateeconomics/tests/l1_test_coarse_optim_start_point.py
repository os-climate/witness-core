...
class StartPointOptimTest(unittest.TestCase):
    ...
    def test_start_point_optim(self):
        ...
        builder = self.ee.factory.get_builder_from_process(repo, "witness_coarse_dev_optim_process")
        ...
        ds_values_from_dm = {
            var: design_space_df[design_space_df["variable"] == var]["value"].values[0][0]
            for var in design_variables_to_check
        }

        # get activation elem from dm to check first element is not activated
        ds_activation_elem_from_dm = {
            var: design_space_df[design_space_df["variable"] == var]["activated_elem"].values[0][0]
            for var in design_variables_to_check
        }
        ...
        self.assertDictEqual(
            expected_values_dict,
            ds_values_from_dm,
            msg="Dictionnary of values for first point is " "not matching the expected values",
        )
        self.assertDictEqual(
            ds_activation_elem_from_dm,
            expected_activated_elem,
            msg="Dictionnary of activated "
