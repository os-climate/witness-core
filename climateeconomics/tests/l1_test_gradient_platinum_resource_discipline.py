...
class PlatinumResourceJacobianDiscTest(AbstractJacobianUnittest):
    """
    Platinum resource jacobian test class
    """

    # np.set_printoptions(threshold=np.inf)

    def analytic_grad_entry(self):
        return [self.test_platinum_resource_analytic_grad, self.test_platinum_resource_damand_variable_analytic_grad]
    ...
    def setUp(self):
        ...
        self.energy_platinum_demand_df = self.energy_platinum_demand_df.loc[
            self.energy_platinum_demand_df[GlossaryCore.Years] >= self.year_start
        ]
        self.energy_platinum_demand_df = self.energy_platinum_demand_df.loc[
            self.energy_platinum_demand_df[GlossaryCore.Years] <= self.year_end
        ]
        self.energy_platinum_variable_demand_df = self.energy_platinum_variable_demand_df.loc[
            self.energy_platinum_variable_demand_df[GlossaryCore.Years] >= self.year_start
        ]
        self.energy_platinum_variable_demand_df = self.energy_platinum_variable_demand_df.loc[
            self.energy_platinum_variable_demand_df[GlossaryCore.Years] <= self.year_end
        ]
    ...
    def test_platinum_resource_analytic_grad(self):
        ...
        mod_path = "climateeconomics.core.core_resources.models.platinum_resource.platinum_resource_disc.PlatinumResourceDiscipline"
        ...
        inputs_dict = {
            ...
            f"{self.name}.{self.model_name}.resources_demand": self.energy_platinum_demand_df,
            #    f'{self.name}.{self.model_name}.lifespan': self.lifespan,
            #    f'{self.name}.{self.model_name}.resource_consumed_data'
            #    : self.consumed_platinum_df,
        }
        ...
        self.check_jacobian(
            ...
            filename=f"jacobian_platinum_discipline.pkl",
    ...
    def test_platinum_resource_damand_variable_analytic_grad(self):
        ...
        inputs_dict = {
            f"{self.name}.{GlossaryCore.YearStart}": self.year_start,
            f"{self.name}.{GlossaryCore.YearEnd}": self.year_end,
            f"{self.name}.{self.model_name}.resources_demand": self.energy_platinum_variable_demand_df,
            #    f'{self.name}.{self.model_name}.lifespan': self.lifespan,
            #    f'{self.name}.{self.model_name}.resource_consumed_data' : self.consumed_platinum_df,
        }
        self.ee.load_study_from_input_dict(inputs_dict)
        ...
        self.check_jacobian(
            location=dirname(__file__),
            filename=f"jacobian_platinum_demand_variable_discipline.pkl",
            discipline=disc_techno,
            step=1e-15,
            derr_approx="complex_step",
            local_data=disc_techno.local_data,
