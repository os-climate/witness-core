...
class ForestJacobianDiscTest(AbstractJacobianUnittest):
    ...
    def test_forest_analytic_grad(self):
        ...
        self.check_jacobian(
            ...
            filename=f"jacobian_forest_v1_discipline.pkl",
            discipline=disc_techno,
            local_data=disc_techno.local_data,
            step=1e-15,
            derr_approx="complex_step",
            inputs=[f"{self.name}.{Forest.DEFORESTATION_SURFACE}", f"{self.name}.{Forest.REFORESTATION_INVESTMENT}"],
            outputs=[f"{self.name}.{Forest.FOREST_SURFACE_DF}", f"{self.name}.{Forest.CO2_EMITTED_FOREST_DF}"],
        )
