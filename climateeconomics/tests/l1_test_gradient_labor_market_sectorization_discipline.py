...
class LaborMarketJacobianDiscTest(AbstractJacobianUnittest):
        ...
        workforce_share = pd.DataFrame(
            {
                ...
                GlossaryCore.SectorServices: 100.0 - indusshare - agri,
            }
        )
    ...
    def test_labor_analytic_grad(self):
        ...
        ns_dict = {"ns_public": f"{self.name}", GlossaryCore.NS_WITNESS: f"{self.name}"}
        ...
        self.check_jacobian(
            ...
            filename=f"jacobian_labormarket_sectorization_discipline.pkl",
