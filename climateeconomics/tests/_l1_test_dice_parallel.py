...
class DICEParallelTest(unittest.TestCase):
    ...
    def test_01_exec_parallel(self):
        ...
        CCS_price = pd.DataFrame({GlossaryCore.Years: years, "ccs_price_per_tCO2": np.linspace(311, 515, len(years))})

        self.energy_mean_price = pd.DataFrame(
            {GlossaryCore.Years: years, GlossaryCore.EnergyPriceValue: np.arange(200, 200 + len(years))}
        )

        self.co2_emissions_ccus_Gt = pd.DataFrame(
            {GlossaryCore.Years: self.years, "carbon_storage Limited by capture (Gt)": 0.02}
        )
        ...
        self.CO2_emissions_by_use_sinks = pd.DataFrame(
            {GlossaryCore.Years: self.years, "CO2 removed by energy mix (Gt)": 0.0}
        )

        self.co2_emissions_needed_by_energy_mix = pd.DataFrame(
            {GlossaryCore.Years: self.years, "carbon_capture needed by energy mix (Gt)": 0.0}
        )
        ...
        values_dict[f"{self.name}.EnergyMix.co2_emissions_needed_by_energy_mix"] = (
        ...
        builder = self.ee8.factory.get_builder_from_process(repo, "witness_wo_energy")
        ...
        compare_dict(dm_dict_1, dm_dict_8, "", dict_error)
        ...
        self.assertDictEqual(dict_error, {".<study_ph>.n_processes.value": "1 and 8 don't match"})
