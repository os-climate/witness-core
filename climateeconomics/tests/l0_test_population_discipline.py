...
class PopDiscTest(unittest.TestCase):
    ...
    def test_execute(self):
        ...
        birth_rate = self.ee.dm.get_value(f"{self.name}.{self.model_name}.birth_rate_df")
        life_expectancy_df = self.ee.dm.get_value(f"{self.name}.{self.model_name}.life_expectancy_df")
    ...
    def test_kcaldegrowth(self):
        ...
        calories_pc_df = pd.DataFrame({GlossaryCore.Years: years, "kcal_pc": np.linspace(2000, 2000, len(years))})
