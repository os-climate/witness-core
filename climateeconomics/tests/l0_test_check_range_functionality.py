...
class CarbonEmissionDiscTestCheckRange(unittest.TestCase):
    ...
    def test_check_range(self):
        ...
        values_dict = {
            ...
            f"{self.name}.{GlossaryCore.CheckRangeBeforeRunBoolName}": False,
        }  # activate check before run
    ...
    def test_failing_check_range_input(self):
        ...
        self.energy_supply_df_all[GlossaryCore.TotalCO2Emissions] = np.linspace(0, -100000, len(self.years))
    ...
    def test_failing_check_range_output(self):
        ...
        CO2_emissions_by_use_sources = pd.DataFrame(
            {
                GlossaryCore.Years: self.years,
                "CO2 from energy mix (Gt)": 1.0e9,
                "carbon_capture from energy mix (Gt)": 0.0,
                "Total CO2 by use (Gt)": 1.0e9,
                "Total CO2 from Flue Gas (Gt)": 3.2,
            }
        )
        ...
        CO2_emitted_forest = pd.DataFrame(
            {
                GlossaryCore.Years: self.years,
                "emitted_CO2_evol": np.linspace(1.0e6, 1.0e6, len(self.years)),
                "emitted_CO2_evol_cumulative": np.cumsum(np.linspace(1.0e6, 1.0e6, len(self.years))) + 1.0e9,
            }
        )
