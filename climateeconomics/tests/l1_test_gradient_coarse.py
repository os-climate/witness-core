...
class CoarseJacobianTestCase(AbstractJacobianUnittest):
    ...
    def _test_problematic_optim_point_3(self):
        self.energy_name = 'CCUS'
        self.ee = ExecutionEngine(self.name)
        ns_dict = {'ns_public': f'{self.name}',
                   'ns_hydrogen': f'{self.name}',
                   'ns_syngas': f'{self.name}',
        ...
        input_list = [GlossaryEnergy.ccs_list, 'co2_for_food', GlossaryCore.YearStart, GlossaryCore.YearEnd,
                      'co2_emissions_needed_by_energy_mix', 'carbon_capture_from_energy_mix']
        inputs_dict = {}
        ...
        self.check_jacobian(location=dirname(__file__),
                            filename='jacobian_at_opt_point_uc4_CCUS.pkl',
                            discipline=disc_techno, step=1e-15, derr_approx='complex_step',
                            local_data=disc_techno.local_data,
                            inputs=checked_input_jacobian,
