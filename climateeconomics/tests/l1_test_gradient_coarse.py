...
class CoarseJacobianTestCase(AbstractJacobianUnittest):
    ...
    def test_01_coarse_renewable_techno_discipline_jacobian(self):
        ...
        builder = self.ee.factory.get_builder_from_module(self.techno_name, mod_path)
        ...
        pkl_file = open(join(dirname(__file__), "data/mda_coarse_data_technologies_input_dict.pkl"), "rb")
        ...
        pkl_file = open(join(dirname(__file__), "data/mda_coarse_data_technologies_output_dict.pkl"), "rb")
        ...
        disc = self.ee.dm.get_disciplines_with_name(f"{self.name}.{self.techno_name}")[
            0
        ].mdo_discipline_wrapp.mdo_discipline

        self.check_jacobian(
            location=dirname(__file__),
            filename=f"jacobian_{self.techno_name}.pkl",
            discipline=disc,
            step=1.0e-18,
    ...
    def test_02_coarse_fossil_techno_discipline_jacobian(self):
        ...
        mod_path = (
            "energy_models.models.fossil.fossil_simple_techno.fossil_simple_techno_disc.FossilSimpleTechnoDiscipline"
        )
        builder = self.ee.factory.get_builder_from_module(self.techno_name, mod_path)
    ...
    def test_06_coarse_renewable_stream_discipline_jacobian(self):
        ...
        pkl_file = open(join(dirname(__file__), "data/mda_coarse_data_streams_input_dict.pkl"), "rb")
        mda_data_input_dict = pickle.load(pkl_file)
        pkl_file.close()
        ...
        pkl_file = open(join(dirname(__file__), "data/mda_coarse_data_streams_output_dict.pkl"), "rb")
        mda_data_output_dict = pickle.load(pkl_file)
        pkl_file.close()
        ...
        disc = self.ee.dm.get_disciplines_with_name(f"{self.name}.{self.energy_name}")[
            0
        ].mdo_discipline_wrapp.mdo_discipline

        self.check_jacobian(
            location=dirname(__file__),
            filename=f"jacobian_{self.energy_name}.pkl",
            discipline=disc,
            step=1.0e-18,
            derr_approx="complex_step",
            threshold=1e-5,
    ...
    def test_08_coarse_energymix_discipline_jacobian(self):
        ...
        pkl_file = open(join(dirname(__file__), "data/mda_coarse_data_energymix_input_dict.pkl"), "rb")
        mda_data_input_dict = pickle.load(pkl_file)
        pkl_file.close()
        namespace = f"{self.name}"
        inputs_dict = {}
        ...
        pkl_file = open(join(dirname(__file__), "data/mda_coarse_data_energymix_output_dict.pkl"), "rb")
        mda_data_output_dict = pickle.load(pkl_file)
        pkl_file.close()
        ...
        for energy in energy_types:
            inputs_dict[f"{self.name}.{self.energy_name}.{energy}.{GlossaryEnergy.EnergyTypeCapitalDfValue}"] = (
                techno_capital
            )
