...
class SectorDisciplineJacobianTest(AbstractJacobianUnittest):
    ...
    def setUp(self):
        ...
        self.energy_supply_df = pd.DataFrame(
            {GlossaryCore.Years: self.years, GlossaryCore.TotalProductionValue: np.linspace(23, 76, len(self.years))}
        )

        self.total_invest = pd.DataFrame(
            {GlossaryCore.Years: self.years, GlossaryCore.InvestmentsValue: 5 * 1.02 ** np.arange(len(self.years))}
        )

        self.damage_fraction_df = pd.DataFrame(
            {GlossaryCore.Years: self.years, GlossaryCore.DamageFractionOutput: 1e-2, GlossaryCore.BaseCarbonPrice: 0.0}
        )
        ...
        self.energy_carbon_intensity = pd.DataFrame(
            {GlossaryCore.Years: self.years, GlossaryCore.EnergyCarbonIntensityDfValue: 100.0}
        )

    def analytic_grad_entry(self):
        return [self.test_analytic_grad, self.test_gradient_withotudamagetoproductivity]
    ...
    def test_analytic_grad(self):
        ...
        mod_path = (
            "climateeconomics.sos_wrapping.sos_wrapping_sectors.industrial.industrial_discipline.IndustrialDiscipline"
        )
        ...
        disc = self.ee.dm.get_disciplines_with_name(f"{self.name}.{GlossaryCore.SectorIndustry}")[0]
        ...
        self.check_jacobian(
            location=os.path.abspath(dirname(__file__)),
            filename=f"jacobian_sector_discipline.pkl",
    ...
    def test_gradient_withotudamagetoproductivity(self):
        ...
        self.check_jacobian(
            ...
            filename=f"jacobian_sector_discipline_withoutdamage.pkl",
            discipline=disc_techno,
            step=1e-15,
            derr_approx="complex_step",
            local_data=disc_techno.local_data,
    ...
    def test_gradient_without_climate_impact_on_gdp(self):
        ...
        self.check_jacobian(
            location=dirname(__file__),
            filename=f"jacobian_sector_discipline_withoutdamage_on_gdp.pkl",
            discipline=disc_techno,
            step=1e-15,
            derr_approx="complex_step",
            local_data=disc_techno.local_data,
