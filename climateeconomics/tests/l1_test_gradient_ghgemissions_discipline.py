...
class GHGEmissionsJacobianDiscTest(AbstractJacobianUnittest):
    ...
    def setUp(self):
        ...
        self.GHG_total_energy_emissions = pd.DataFrame(
            {
        ...
        self.CO2_land_emissions = pd.DataFrame(
            {
                GlossaryCore.Years: years,
                "Crop": np.linspace(0.0, 0.0, len(years)),
                "Forest": np.linspace(3.0, 4.0, len(years)),
            }
        )
        ...
        self.energy_production = pd.DataFrame({GlossaryCore.Years: years, GlossaryCore.TotalProductionValue: 100.0})

        self.residential_energy_consumption = pd.DataFrame(
            {GlossaryCore.Years: years, GlossaryCore.TotalProductionValue: 1.2}
        )

        def generate_energy_consumption_df_sector(sector_name):
            out = {GlossaryCore.Years: years}
            out.update({section: 10.0 for section in GlossaryCore.SectionDictSectors[sector_name]})
            return pd.DataFrame(out)

        self.ghg_eenergy_consumptions_sectors = {
            f"{self.name}.{sector}.{GlossaryCore.SectionEnergyConsumptionDfValue}": generate_energy_consumption_df_sector(
                sector
            )
        ...
        self.ghg_sections_gdp = {
            f"{self.name}.{sector}.{GlossaryCore.SectionGdpDfValue}": generate_energy_consumption_df_sector(sector)
            for sector in GlossaryCore.SectorsPossibleValues
        }
        ...
        self.inputs_cheked += [
            f"{self.name}.{sector}.{GlossaryCore.SectionEnergyConsumptionDfValue}"
            for sector in GlossaryCore.DefaultSectorListGHGEmissions
        ]
        self.inputs_cheked += [
            f"{self.name}.{sector}.{GlossaryCore.SectionGdpDfValue}"
            for sector in GlossaryCore.DefaultSectorListGHGEmissions
        ]
    ...
    def analytic_grad_entry(self):
        return [self.test_carbon_emissions_analytic_grad_affine_co2_objective]
    ...
    def test_carbon_emissions_analytic_grad(self):
        ...
        self.check_jacobian(
            ...
            filename=f"jacobian_ghg_emission_discipline.pkl",
    ...
    def test_carbon_emissions_analytic_grad_affine_co2_objective(self):
        ...
        self.check_jacobian(
            location=dirname(__file__),
            filename=f"jacobian_ghg_emission_discipline_affine_co2_objective.pkl",
            discipline=disc_techno,
            step=1e-15,
            derr_approx="complex_step",
            local_data=disc_techno.local_data,
