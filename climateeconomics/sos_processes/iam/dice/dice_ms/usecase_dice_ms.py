...
class Study(StudyManager):
    ...
    def setup_usecase(self, study_folder_path=None):
        ...
        values_dict = {}
        scenario_list = [scenario_A, scenario_C, scenario_D, scenario_B, scenario_E]
        for scenario in scenario_list:
            values_dict[f"{self.study_name}.{self.scatter_scenario}.{scenario}.{GlossaryCore.EconomicsDfValue}"] = (
                economics_df
            )

        values_dict[f"{self.study_name}.{self.scatter_scenario}.samples_df"] = pd.DataFrame(
            {"selected_scenario": [True for _ in scenario_list], "scenario_name": scenario_list}
        )
