...
class Study(StudyManager):
    ...
    def setup_usecase(self, study_folder_path=None):
        ...
        energy_prod = pd.DataFrame({
            GlossaryCore.Years: years,
            GlossaryCore.TotalProductionValue: np.linspace(40, 60, len(years))
        })

        energy_capital = pd.DataFrame({
            GlossaryCore.Years: years,
            GlossaryCore.Capital: np.linspace(40, 60, len(years))
        })

        energy_mean_price = pd.DataFrame({
            GlossaryCore.Years: years,
            GlossaryCore.EnergyPriceValue: np.linspace(40, 60, len(years))
        })

        ccs_price = pd.DataFrame({
            GlossaryCore.Years: years,
            "ccs_price_per_tCO2": np.linspace(40, 60, len(years))
        })
        ...
        data_energy = {
            f"{self.study_name}.{GlossaryCore.EnergyProductionValue}": energy_prod,
        ...
        setup_data_list.append(data_energy)
