...
Modifications on 2023/10/24-2023/11/06 Copyright 2023 Capgemini
...
class ProcessBuilder(BaseProcessBuilder):
    ...
    def get_builders(self):
        ...
        ns_dict = {
            GlossaryCore.NS_WITNESS: self.ee.study_name,
        ...
        self.ee.ns_manager.add_ns(
            GlossaryCore.NS_REGIONALIZED_POST_PROC, f"{self.ee.study_name}.Macroeconomics.Regions"
        )
        ...
        self.ee.ns_manager.add_ns(
            GlossaryCore.NS_SECTORS_POST_PROC_GDP, f"{self.ee.study_name}.Macroeconomics.{GlossaryCore.EconomicSectors}"
        )
        ...
        self.ee.ns_manager.add_ns(
            GlossaryCore.NS_SECTORS_POST_PROC_EMISSIONS,
        ...
        post_proc_module = (
            f"climateeconomics.sos_wrapping.post_procs.sectors.emissions.{GlossaryCore.Households.lower()}"
        )
        self.ee.post_processing_manager.add_post_processing_module_to_namespace(ns, post_proc_module)

        for sector in GlossaryCore.DefaultSectorListGHGEmissions:
            ns = f"ns_{sector.lower()}_emissions"
            self.ee.ns_manager.add_ns(
                ns, f"{self.ee.study_name}.{GHGemissionsDiscipline.name}.{GlossaryCore.EconomicSectors}.{sector}"
            )
        ...
        for sector in GlossaryCore.SectorsPossibleValues:
            ...
            self.ee.ns_manager.add_ns(
                ns, f"{self.ee.study_name}.Macroeconomics.{GlossaryCore.EconomicSectors}.{sector}"
            )
