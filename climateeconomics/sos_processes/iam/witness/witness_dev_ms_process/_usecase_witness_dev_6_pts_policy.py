...
from climateeconomics.sos_processes.iam.witness.witness_dev_optim_process.usecase_witness_optim_invest_distrib import (
    Study as witness_dev_optim_usecase,
)
...
class Study(ClimateEconomicsStudyManager):
    ...
    def setup_usecase(self, study_folder_path=None):
        witness_ms_usecase = witness_dev_optim_usecase(execution_engine=self.execution_engine)
        ...
        for alpha_i in alpha_list:
            ...
            values_dict[
                f"{self.study_name}.{self.scatter_scenario}.{scenario_i}.{witness_ms_usecase.optim_name}.{witness_ms_usecase.coupling_name}.{witness_ms_usecase.extra_name}.co2_damage_price_percentage"
            ] = alpha_i
            values_dict[
                f"{self.study_name}.{self.scatter_scenario}.{scenario_i}.{witness_ms_usecase.optim_name}.{witness_ms_usecase.coupling_name}.{witness_ms_usecase.extra_name}.ccs_price_percentage"
            ] = alpha_i
        ...
        for scenario in scenario_list:
            scenarioUseCase = witness_dev_optim_usecase(bspline=self.bspline, execution_engine=self.execution_engine)
            ...
            values_dict[
                f"{self.study_name}.{self.scatter_scenario}.{scenario}.{witness_ms_usecase.optim_name}"
                f".{witness_ms_usecase.coupling_name}.{witness_ms_usecase.extra_name}.FunctionsManager.function_df"
            ] = default_func_df
            values_dict[
                f"{self.study_name}.{self.scatter_scenario}.{scenario}.{witness_ms_usecase.optim_name}.{witness_ms_usecase.coupling_name}.{witness_ms_usecase.extra_name}.alpha"
            ] = 1.0
