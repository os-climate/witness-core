    ...
    def setup_usecase(self, study_folder_path=None):
        ...
        for resource in ResourceGlossary.GlossaryDict.values():
            if resource["name"] not in modeled_resources:
                non_modeled_resource_price[resource["name"]] = resource["price"]
            resources_CO2_emissions[resource["name"]] = resource["CO2_emissions"]
        resources_CO2_emissions.index = resources_CO2_emissions[GlossaryCore.Years]
        resource_input[self.study_name + self.all_resource_name + ".non_modeled_resource_price"] = (
            non_modeled_resource_price
        )
        ...
        data_dir_resource = join(dirname(dirname(dirname(dirname(dirname(__file__))))), "tests", "data")
        resource_demand = pd.read_csv(join(data_dir_resource, "all_demand_from_energy_mix.csv"))

        resource_demand = resource_demand.loc[resource_demand[GlossaryCore.Years] >= self.year_start]
        resource_demand = resource_demand.loc[resource_demand[GlossaryCore.Years] <= self.year_end]
        if self.main_study:
            resource_input[self.study_name + self.all_resource_name + ".resources_CO2_emissions"] = (
