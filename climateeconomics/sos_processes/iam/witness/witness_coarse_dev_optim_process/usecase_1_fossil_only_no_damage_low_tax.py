...
class Study(StudyOptimInvestDistrib):
    ...
    def setup_usecase(self, study_folder_path=None):
        ...
        max_invest = 3000.0
        dspace_invests = {
            "fossil.FossilSimpleTechno.fossil_FossilSimpleTechno_array_mix": [300.0, 300.0, 3000.0, True],
        ...
        min_UR = 50.0
        dspace_UR = {
            "fossil_FossilSimpleTechno_utilization_ratio_array": [min_UR, min_UR, 100.0, True],
            "renewable_RenewableSimpleTechno_utilization_ratio_array": [min_UR, min_UR, 100.0, False],
            ...
            "carbon_capture.flue_gas_capture.FlueGasTechno_utilization_ratio_array": [min_UR, min_UR, 100.0, False],
            "carbon_storage.CarbonStorageTechno_utilization_ratio_array": [min_UR, min_UR, 100.0, False],
        }
        ...
        dvar_descriptor = data_witness[
            f"{self.study_name}.{self.optim_name}.{self.witness_uc.coupling_name}.DesignVariables.design_var_descriptor"
        ]
        design_var_descriptor_ine_variable = self.get_ine_dvar_descr()

        dvar_descriptor.update({"share_non_energy_invest_ctrl": design_var_descriptor_ine_variable})
        ...
        data_witness.update(
            {
                ...
                f"{self.study_name}.{self.optim_name}.{self.witness_uc.coupling_name}.{self.witness_uc.extra_name}.share_non_energy_invest_ctrl": np.array(
                    [27.0] * (GlossaryCore.NB_POLES_COARSE - 1)
                ),
            }
        )
