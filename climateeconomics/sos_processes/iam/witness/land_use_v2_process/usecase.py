...
class Study(StudyManager):
    ...
    def setup_usecase(self, study_folder_path=None):
        ...
        land_demand_df = pd.read_csv(join(data_dir, "land_demandV2.csv"))
        # cut land_demand_df to respect years of study case
        land_demand_df = land_demand_df.loc[land_demand_df[GlossaryCore.Years] >= self.year_start]
        land_demand_df = land_demand_df.loc[land_demand_df[GlossaryCore.Years] <= self.year_end]
        self.total_food_land_surface = pd.DataFrame(index=years, columns=[GlossaryCore.Years, "total surface (Gha)"])
        ...
        self.forest_surface_df = pd.DataFrame(
            index=years, columns=[GlossaryCore.Years, "forest_constraint_evolution", "global_forest_surface"]
        )
        ...
        landuse_input[self.study_name + self.extra_name + ".land_demand_df"] = land_demand_df
        landuse_input[self.study_name + ".total_food_land_surface"] = self.total_food_land_surface
        landuse_input[self.study_name + ".forest_surface_df"] = self.forest_surface_df
