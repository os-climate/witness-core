...
class Study(ClimateEconomicsStudyManager):
    """
    Usecase 7, mda run only, no optim, based on usecase_witness_coarse_new. Working assumptions:
    - Macro-model: compute GDP
    - Damage: activated on population, GDP
    - Tax: activated 100% on ccs price and co2 damage
    - invest: mix fossil renewable, ccs (IEA inspired NZE)
    """
    ...
    def setup_usecase(self, study_folder_path=None):
        ...
        updated_data = {
            f"{self.study_name}.assumptions_dict": {
                "compute_gdp": True,
        ...
        invest_percentage_gdp_df = pd.read_csv(join(dirname(__file__), "uc7_percentage_of_gdp_energy_invest.csv"))
        invest_percentage_per_techno_df = pd.read_csv(join(dirname(__file__), "uc7_techno_invest_percentage.csv"))
        ...
        invest_percentage_gdp_df = invest_percentage_gdp_df[
            ...
            & (invest_percentage_gdp_df[GlossaryCore.Years] <= self.year_end)
        ...
        invest_percentage_per_techno_df = invest_percentage_per_techno_df[
            ...
            & (invest_percentage_per_techno_df[GlossaryCore.Years] <= self.year_end)
        ].reset_index(drop=True)
