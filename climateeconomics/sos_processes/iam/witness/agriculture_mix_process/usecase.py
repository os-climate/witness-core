...
AGRI_MIX_TECHNOLOGIES_LIST_FOR_OPT = ["ManagedWood", "CropEnergy"]
...
class Study(StudyManager):
    ...
    def setup_usecase(self, study_folder_path=None):
        ...
        self.energy_prices = pd.DataFrame({GlossaryCore.Years: years, "electricity": 16.0})
        ...
        population_df = pd.DataFrame({GlossaryCore.Years: years, GlossaryCore.PopulationValue: population})
        ...
        self.red_meat_ca_per_day = pd.DataFrame(
            {GlossaryCore.Years: years, "red_meat_calories_per_day": [red_meat_average_ca_daily_intake] * year_range}
        )
        ...
        self.fish_ca_per_day = pd.DataFrame(
            {GlossaryCore.Years: years, GlossaryCore.FishDailyCal: [fish_average_ca_daily_intake] * year_range}
        )
        self.other_ca_per_day = pd.DataFrame(
            {GlossaryCore.Years: years, GlossaryCore.OtherDailyCal: [other_average_ca_daily_intake] * year_range}
        )
        ...
        self.transport = pd.DataFrame({GlossaryCore.Years: years, "transport": np.ones(len(years)) * 7.6})

        self.energy_carbon_emissions = pd.DataFrame(
            {
                GlossaryCore.Years: years,
                "biomass_dry": -0.64 / 4.86,
                "solid_fuel": 0.64 / 4.86,
                "electricity": 0.0,
                "methane": 0.123 / 15.4,
                "syngas": 0.0,
        ...
        self.crop_investment = pd.DataFrame({GlossaryCore.Years: years, GlossaryCore.InvestmentsValue: crop_invest})
        ...
        deforest_invest_df = pd.DataFrame(
            {GlossaryCore.Years: years, GlossaryCore.InvestmentsValue: np.linspace(114, 39, len(self.years))}
        )

        co2_taxes_year = [2018, 2020, 2025, 2030, 2035, 2040, 2045, 2050]
        co2_taxes = [14.86, 17.22, 20.27, 29.01, 34.05, 39.08, 44.69, 50.29]
        func = sc.interp1d(co2_taxes_year, co2_taxes, kind="linear", fill_value="extrapolate")

        self.co2_taxes = pd.DataFrame({GlossaryCore.Years: years, GlossaryCore.CO2Tax: func(years)})

        techno_capital = pd.DataFrame(
            {GlossaryCore.Years: self.years, GlossaryCore.Capital: 20000 * np.ones_like(self.years)}
        )
        ...
        design_space_ctrl_dict["vegetables_and_carbs_calories_per_day_ctrl"] = (
    ...
    def setup_design_space_ctrl_new(self):
        ...
        update_dspace_dict_with(
            ...
            [1.0e-6] * self.nb_poles,
            [3000.0] * self.nb_poles,
            activated_elem=[True] * self.nb_poles,
        )
        if "CropEnergy" in self.techno_list:
            update_dspace_dict_with(
