    ...
    def setup_usecase(self, study_folder_path=None):
        ...
        for energy in self.witness_uc.energy_list:
            ...
            for technology in self.witness_uc.dict_technos[energy]:
                ...
                dvar_value = dspace_df[f"{energy}.{technology}.{energy_wo_dot}_{technology_wo_dot}_array_mix"]["value"]
                activated_dvar = dspace_df[f"{energy}.{technology}.{energy_wo_dot}_{technology_wo_dot}_array_mix"][
                    "activated_elem"
                ]
                ...
                dv_arrays_dict[
                    f"{self.witness_uc.study_name}.{self.energy_mix_name}.{energy}_{technology}_utilization_ratio_array"
                ] = design_var_utilization_ratio_value
                dv_arrays_dict[
                    f"{self.witness_uc.study_name}.{self.energy_mix_name}.{energy}.{technology}.{GlossaryCore.UtilisationRatioValue}"
                ] = pd.DataFrame(data={GlossaryCore.Years: years, GlossaryCore.UtilisationRatioValue: 100.0})
        ...
        for ccs in self.witness_uc.ccs_list:
            ...
            for technology in self.witness_uc.dict_technos[ccs]:
                technology_wo_dot = technology.replace(".", "_")
                dvar_value = dspace_df[f"{ccs}.{technology}.{ccs_wo_dot}_{technology_wo_dot}_array_mix"]["value"]
                activated_dvar = dspace_df[f"{ccs}.{technology}.{ccs_wo_dot}_{technology_wo_dot}_array_mix"][
                    "activated_elem"
                ]
                activated_value = np.array([elem for i, elem in enumerate(dvar_value) if activated_dvar[i]])
                ...
                dv_arrays_dict[
                    f"{self.witness_uc.study_name}.{GlossaryCore.CCUS}.{ccs}.{technology}_utilization_ratio_array"
                ] = design_var_utilization_ratio_value
                dv_arrays_dict[
                    f"{self.witness_uc.study_name}.{GlossaryCore.CCUS}.{ccs}.{technology}.{GlossaryCore.UtilisationRatioValue}"
                ] = pd.DataFrame(data={GlossaryCore.Years: years, GlossaryCore.UtilisationRatioValue: 100.0})
                # add design variable for utilization ratio per technology
        ...
        dv_arrays_dict[f"{self.witness_uc.study_name}.red_meat_calories_per_day_ctrl"] = np.array(
        ...
        dv_arrays_dict[f"{self.witness_uc.study_name}.white_meat_calories_per_day_ctrl"] = np.array(
        ...
        dv_arrays_dict[f"{self.witness_uc.study_name}.vegetables_and_carbs_calories_per_day_ctrl"] = np.array(
        ...
        dv_arrays_dict[f"{self.witness_uc.study_name}.milk_and_eggs_calories_per_day_ctrl"] = np.array(
        ...
        func_df.loc[func_df["variable"] == "land_demand_constraint", "weight"] = 0.0
        func_df.loc[func_df["variable"] == "calories_per_day_constraint", "weight"] = 0.0
        func_df.loc[func_df["variable"] == "total_prod_minus_min_prod_constraint_df", "weight"] = 0.0
        ...
        """
        values_dict[f"{self.witness_uc.study_name}.{self.coupling_name}.{GlossaryCore.energy_list}"] = (
            self.witness_uc.energy_list
        )
