...
class Study(StudyManager):
    ...
    def setup_usecase(self, study_folder_path=None):
        ...
        total_invests = pd.DataFrame({GlossaryCore.Years: years, GlossaryCore.InvestmentsValue: total_invest_start})

        # Energy
        energy_investment_wo_tax = pd.DataFrame(
            {GlossaryCore.Years: years, GlossaryCore.EnergyInvestmentsWoTaxValue: 1000.0}
        )
        ...
        if self.main_study:
            ...
            demand_per_capita_agriculture = pd.DataFrame(
                {
                    GlossaryCore.Years: years,
            ...
            demand_per_capita_industry = pd.DataFrame(
                {
                    GlossaryCore.Years: years,
            ...
            demand_per_capita_services = pd.DataFrame(
                {
                    GlossaryCore.Years: years,
            ...
            invest_indus = pd.DataFrame({GlossaryCore.Years: years, GlossaryCore.ShareInvestment: invest_indus_start})

            invest_services = pd.DataFrame(
                {GlossaryCore.Years: years, GlossaryCore.ShareInvestment: invest_services_start}
            )

            invest_agriculture = pd.DataFrame(
                {GlossaryCore.Years: years, GlossaryCore.ShareInvestment: invest_agri_start}
            )
            ...
            share_energy_agriculture = pd.DataFrame(
                {GlossaryCore.Years: years, GlossaryCore.ShareSectorEnergy: share_energy_agri_2020}
            )
            share_energy_services = pd.DataFrame(
                {GlossaryCore.Years: years, GlossaryCore.ShareSectorEnergy: share_energy_services_2020}
            )
            share_energy_resi = pd.DataFrame(
                {GlossaryCore.Years: years, GlossaryCore.ShareSectorEnergy: share_energy_resi_2020}
            )
            share_energy_other = pd.DataFrame(
                {GlossaryCore.Years: years, GlossaryCore.ShareSectorEnergy: share_energy_other_2020}
            )
            ...
            energy_mean_price = pd.DataFrame({GlossaryCore.Years: years, GlossaryCore.EnergyPriceValue: energy_price})
            ...
            workforce_share = pd.DataFrame(
                {
                    GlossaryCore.Years: years,
                    GlossaryCore.SectorAgriculture: agrishare,
            ...
            CO2_indus_emissions_df = pd.DataFrame({GlossaryCore.Years: years, "indus_emissions": 0.0})
            ...
            for sector in GlossaryCore.SectorsPossibleValues:
                ...
                subsector_share_dict = {
                    ...
                    **dict(
                        zip(
                            section_non_energy_emission_gdp_df.columns[1:],
                ...
                cons_input[
                    f"{self.study_name}.{GHGemissionsDiscipline.name}.{GlossaryCore.EconomicSectors}.{sector}.{GlossaryCore.SectionNonEnergyEmissionGdpDfValue}"
