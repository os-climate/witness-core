...
class Study(ClimateEconomicsStudyManager):
    ...
    def setup_usecase(self, study_folder_path=None):
        ...
        scenario_dict = {
            usecase_ms_mda.USECASE2: usecase2(execution_engine=self.execution_engine),
    ...
    def specific_check_outputs(self):
        ...
        ref_value_temp_increase = {self.USECASE2: 4.23, self.USECASE2B: 4.07, self.USECASE4: 3.34, self.USECASE7: 2.41}
        ref_temperature_2020 = 1.3

        all_co2_taxes = dm.get_all_namespaces_from_var_name("CO2_taxes")
        ref_value_co2_tax = {self.USECASE2: 0, self.USECASE2B: 0, self.USECASE4: 0, self.USECASE7: 1192}
        ...
        all_net_energy_productions = dm.get_all_namespaces_from_var_name(
            f"EnergyMix.{GlossaryCore.EnergyProductionDetailedValue}"
        )
        ...
        all_energy_investment_without_tax = dm.get_all_namespaces_from_var_name(
        ...
        for scenario in list_scenario:
            ...
            for scenario_temp_increase in all_temp_increase:
                if scenario in scenario_temp_increase:
                    ...
                    error_msg += self.should_be_greater(
                        value_temp_increase, ref_value_temp_increase[scenario] * 0.8, f"{scenario_temp_increase}[2100]"
                    )
                    error_msg += self.should_be_lower(
                        value_temp_increase, ref_value_temp_increase[scenario] * 1.2, f"{scenario_temp_increase}[2100]"
                    )
                    error_msg += self.should_be_greater(
                        value_temp_2020,
                        ref_temperature_2020 * tolerance_low_ref_2020,
                        f"{scenario_temp_increase}[2020]",
                    )
                    error_msg += self.should_be_lower(
                        value_temp_2020,
                        ref_temperature_2020 * tolerance_high_ref_2020,
                        f"{scenario_temp_increase}[2020]",
                    )
            ...
            for scenario_co2_tax in all_co2_taxes:
                if scenario in scenario_co2_tax:
                    ...
                    error_msg += self.should_be_greater(
                        value_co2_tax, ref_value_co2_tax[scenario] * 0.8, f"{scenario_co2_tax}[2100]"
                    )
                    error_msg += self.should_be_lower(
                        value_co2_tax, ref_value_co2_tax[scenario] * 1.2, f"{scenario_co2_tax}[2100]"
                    )
            ...
            for scenario_gdp in all_gdps:
                if scenario in scenario_gdp:
                    ...
                    error_msg += self.should_be_greater(
                        value_gdp, ref_value_world_gdp_net_of_damage[scenario] * 0.8, f"{scenario_gdp}[2100]"
                    )
                    error_msg += self.should_be_lower(
                        value_gdp, ref_value_world_gdp_net_of_damage[scenario] * 1.2, f"{scenario_gdp}[2100]"
                    )
                    # assert on value of start, tolerance at 1.5%
                    error_msg += self.should_be_greater(
                        value_gdp_2020, ref_gdp_2020 * tolerance_low_ref_2020, f"{scenario_gdp}[2020]"
                    )
                    error_msg += self.should_be_lower(
                        value_gdp_2020, ref_gdp_2020 * tolerance_high_ref_2020, f"{scenario_gdp}[2020]"
                    )
            for scenario_emissions in all_co2_emissions:
                if scenario in scenario_emissions:
                    emissions_df = dm.get_value(scenario_emissions)
                    value_emissions = emissions_df.loc[emissions_df["years"] == 2100]["CO2_20"].values[
                        0
                    ]  # todo : replace by GWP in futur
                    value_emissions_2020 = emissions_df.loc[emissions_df["years"] == 2020]["CO2_20"].values[
                        0
                    ]  # todo : replace by GWP in futur
                    if self.USECASE7 in scenario:
                        error_msg += self.should_be_lower(
                            value_emissions, ref_value_co2_emissions[scenario] * 0.8, f"{scenario_emissions}[2100]"
                        )
                        error_msg += self.should_be_greater(
                            value_emissions, ref_value_co2_emissions[scenario] * 1.2, f"{scenario_emissions}[2100]"
                        )
                    else:
                        error_msg += self.should_be_greater(
                            value_emissions, ref_value_co2_emissions[scenario] * 0.8, f"{scenario_emissions}[2100]"
                    ...
                    error_msg += self.should_be_greater(
                        value_emissions_2020, ref_co2_2020 * tolerance_low_ref_2020, f"{scenario_emissions}[2020]"
                    )
                    error_msg += self.should_be_lower(
                        value_emissions_2020, ref_co2_2020 * tolerance_high_ref_2020, f"{scenario_emissions}[2020]"
                    )
            ...
            for scenario_net_energy_production in all_net_energy_productions:
                if scenario in scenario_net_energy_production:
                    ...
                    value_net_energy_production = net_energy_production_df.loc[
                        net_energy_production_df["years"] == 2100
                    ]["Total production (uncut)"].values[0]
                    value_net_energy_production_2020 = net_energy_production_df.loc[
                        net_energy_production_df["years"] == 2020
                    ]["Total production (uncut)"].values[0]
                    error_msg += self.should_be_greater(
                        value_net_energy_production,
                        ref_value_net_energy_production[scenario] * 0.8,
                        f"{scenario_net_energy_production}[2100]",
                    )
                    error_msg += self.should_be_lower(
                        value_net_energy_production,
                        ref_value_net_energy_production[scenario] * 1.2,
                        f"{scenario_net_energy_production}[2100]",
                    )
                    error_msg += self.should_be_greater(
                        value_net_energy_production_2020,
                        ref_net_prod_2020 * tolerance_low_ref_2020,
                        f"{scenario_net_energy_production}[2020]",
                    )
                    error_msg += self.should_be_lower(
                        value_net_energy_production_2020,
                        ref_net_prod_2020 * tolerance_high_ref_2020,
                        f"{scenario_net_energy_production}[2020]",
                    )
            ...
            for scenario_population in all_populations:
                if scenario in scenario_population:
                    ...
                    error_msg += self.should_be_greater(
                        value_population, ref_value_populations[scenario] * 0.8, f"{scenario_population}[2100]"
                    )
                    error_msg += self.should_be_lower(
                        value_population, ref_value_populations[scenario] * 1.2, f"{scenario_population}[2100]"
                    )
                    error_msg += self.should_be_greater(
                        value_population_2020,
                        ref_population_2020 * tolerance_low_ref_2020,
                        f"{scenario_population}[2020]",
                    )
                    error_msg += self.should_be_lower(
                        value_population_2020,
                        ref_population_2020 * tolerance_high_ref_2020,
                        f"{scenario_population}[2020]",
                    )
            ...
            for scenario_energy_investment_without_tax in all_energy_investment_without_tax:
                if scenario in scenario_energy_investment_without_tax:
                    ...
                    value_energy_investment_without_tax = energy_investment_without_tax_df.loc[
                        energy_investment_without_tax_df["years"] == 2100
                    ][GlossaryCore.EnergyInvestmentsWoTaxValue].values[0]
                    error_msg += self.should_be_greater(
                        value_energy_investment_without_tax,
                        ref_value_energy_investment_without_tax[scenario] * 0.8,
                        scenario_energy_investment_without_tax,
                    )
                    error_msg += self.should_be_lower(
                        value_energy_investment_without_tax,
                        ref_value_energy_investment_without_tax[scenario] * 1.2,
                        scenario_energy_investment_without_tax,
                    )
