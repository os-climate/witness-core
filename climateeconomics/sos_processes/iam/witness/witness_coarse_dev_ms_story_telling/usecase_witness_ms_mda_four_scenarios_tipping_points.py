...
class Study(ClimateEconomicsStudyManager):
    ...
    USECASE4_TP_REF = uc_ms_mda.USECASE4 + ", " + TIPPING_POINT + SEP + TIPPING_POINT_LIST[0] + UNIT
    USECASE4_TP1 = uc_ms_mda.USECASE4 + ", " + TIPPING_POINT + SEP + TIPPING_POINT_LIST[1] + UNIT
    USECASE4_TP2 = uc_ms_mda.USECASE4 + ", " + TIPPING_POINT + SEP + TIPPING_POINT_LIST[2] + UNIT
    USECASE7_TP_REF = uc_ms_mda.USECASE7 + ", " + TIPPING_POINT + SEP + TIPPING_POINT_LIST[0] + UNIT
    USECASE7_TP1 = uc_ms_mda.USECASE7 + ", " + TIPPING_POINT + SEP + TIPPING_POINT_LIST[1] + UNIT
    USECASE7_TP2 = uc_ms_mda.USECASE7 + ", " + TIPPING_POINT + SEP + TIPPING_POINT_LIST[2] + UNIT
    ...
    def setup_usecase(self, study_folder_path=None):
        ...
        values_dict.update(
            {
                f"{self.study_name}.{self.scatter_scenario}.{self.USECASE4_TP1}.{tipping_point_variable}": float(
                    self.TIPPING_POINT_LIST[1]
                ),
                f"{self.study_name}.{self.scatter_scenario}.{self.USECASE4_TP2}.{tipping_point_variable}": float(
                    self.TIPPING_POINT_LIST[2]
                ),
                f"{self.study_name}.{self.scatter_scenario}.{self.USECASE7_TP1}.{tipping_point_variable}": float(
                    self.TIPPING_POINT_LIST[1]
                ),
                f"{self.study_name}.{self.scatter_scenario}.{self.USECASE7_TP2}.{tipping_point_variable}": float(
                    self.TIPPING_POINT_LIST[2]
                ),
            }
        )
        # Inputs were optimized manually through the sostrades GUI and saved in csv files  => recover inputs
        invest_gdp_uc4_tp1 = pd.read_csv(
            join(
                dirname(__file__),
                pardir,
        ...
        invest_gdp_uc4_tp2 = pd.read_csv(
            join(
                dirname(__file__),
                pardir,
                "witness_coarse_dev_story_telling",
        ...
        invest_gdp_uc7_tp1 = pd.read_csv(
            join(
                dirname(__file__),
                pardir,
                "witness_coarse_dev_story_telling",
        ...
        invest_gdp_uc7_tp2 = pd.read_csv(
            join(
                dirname(__file__),
                pardir,
                "witness_coarse_dev_story_telling",
